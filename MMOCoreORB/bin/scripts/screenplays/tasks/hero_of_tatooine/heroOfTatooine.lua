...
HeroOfTatooineScreenPlay = ScreenPlay:new {
		...
		ring = "object/tangible/wearables/ring/ring_mark_hero.iff"
}
...
function HeroOfTatooineScreenPlay:start()
	if (isZoneEnabled("tatooine")) then
		...
		local pAltCave = getSceneObject(9995371)
...
--[[ Infinity Custom allowing players to reset ]]
function HeroOfTatooineScreenPlay:reset(pPlayer)
	if pPlayer ~= nil then
		local states = {
		...
		for i=1,#states do
			if util:hasQuestState(pPlayer, "hero_of_tatooine", QS[i]) then
				util:removeQuestState(pPlayer, "hero_of_tatooine", QS[i])
			end
		end
		local others = {
			{"Bounty Hunter", "hero_of_tatooine_intellect"},
			{"Farmer","hero_of_tatooine_altruism"},
			{"Rancher","hero_of_tatooine_honor"},
		}
		for j=1,#others do
			for i=1, 2 do
				if util:hasQuestState(pPlayer, others[j][2], QS[i]) then
					util:removeQuestState(pPlayer, others[j][2], QS[i])
				end
			end
		end
		local marks = {
		...
		for i=1,#marks do
			if util:hasQuestState(pPlayer, "hero_of_tatooine_missing_marks", QS[i]) then
				util:removeQuestState(pPlayer, "hero_of_tatooine_missing_marks", QS[i])
			end
		end
...
function HeroOfTatooineScreenPlay:spawnAltruismObjects()
	...
	--HeroOfTatooineScreenPlay:despawnAltruismObjects
	util:cw("HOT altruism spawns", 1)
	...
	-- infinity moved, conflict with mando sawtooth queen
	local pCrate = spawnSceneObject("tatooine", "object/tangible/item/quest/hero_of_tatooine/explosives_crate.iff", 95.4,-45.7,-95.9, 5995569, .985, 0, .1714, 0)
	...
	if (pCrate ~= nil) then
		...
		util:cw("HOT crate spawned", 1)
	else
		util:cw("HOT FAIL - no crate spawned", 1)
	end
	...
	if (pWall ~= nil) then
		...
		util:cw("HOT wall spawned", 1)
	else
		util:cw("HOT FAIL - no wall spawned", 1)
	end
	...
	if (pCrevice ~= nil) then
		...
		util:cw("HOT crevice spawned", 1)
	else
		util:cw("HOT FAIL - no crevice spawned", 1)
	end
...
function HeroOfTatooineScreenPlay:doCourageChange()
	...
	elseif (pCourageMob ~= nil) then
		...
		if SceneObject(pCourageMob):isAiAgent() then
			AiAgent(pCourageMob):doDespawn()
		else
	...
	if (pBoar ~= nil) then
		AiAgent(pBoar):addCreatureFlag(AI_NOAIAGGRO)
...
function HeroOfTatooineScreenPlay:doAltruismChange()
	...
	if (pFarmer ~= nil) then
		...
		if SceneObject(pFarmer):isAiAgent() then
			AiAgent(pFarmer):doDespawn()
		else
	...
	if spawnPoint == nil then
		createEvent(60 * 1000, "HeroOfTatooineScreenPlay", "doAltruismChange", nil, "")		
		printLuaError("Failed to create leader in HeroOfTatooineScreenPlay:doAltruismChange().")
	end
	local z = getWorldFloor(spawnPoint[1], spawnPoint[3], "tatooine")
...
function HeroOfTatooineScreenPlay:destroyIntellectMobs()
	...
	if (pBountyHunter ~= nil) then
		...
		if SceneObject(pBountyHunter):isAiAgent() then
			AiAgent(pBountyHunter):doDespawn()
		else
	...
	for i = 1, 6, 1 do
		...
		if (pLiar ~= nil) then
			...
			if SceneObject(pLiar):isAiAgent() then
				AiAgent(pLiar):doDespawn()
			else
...
function HeroOfTatooineScreenPlay:giveAltruismWaypoint(pPlayer)
	...
	local waypointID = PlayerObject(pGhost):addWaypoint("tatooine", "Kidnapped Family", "", 6555, -1311, WAYPOINTPURPLE, true, true, 0, 1)
...
function HeroOfTatooineScreenPlay:doGiverDespawn(pGiver)
	...
	if (CreatureObject(pGiver):isAiAgent()) then
		...
		if SceneObject(pGiver):isAiAgent() then
			AiAgent(pGiver):doDespawn()
		else
...
function HeroOfTatooineScreenPlay:doHonorChange()
	...
	if (pPirate1 ~= nil) then
		if SceneObject(pPirate1):isAiAgent() then
			AiAgent(pPirate1):doDespawn()
		else
	...
	if (pPirate2 ~= nil) then
		if SceneObject(pPirate2):isAiAgent() then
			AiAgent(pPirate2):doDespawn()
		else
	...
	if (pLeader ~= nil) then
		if SceneObject(pLeader):isAiAgent() then
			AiAgent(pLeader):doDespawn()
		else
	...
	local spawnPoint = getSpawnPoint("tatooine", x, y, 10, 512)
	if spawnPoint == nil then
		createEvent(60 * 1000, "HeroOfTatooineScreenPlay", "doHonorChange", nil, "")
	...
	AiAgent(pLeader):addCreatureFlag(AI_NOAIAGGRO)
	...
	if (pPirate1 ~= nil) then
		AiAgent(pPirate1):addCreatureFlag(AI_NOAIAGGRO)
	...
	if (pPirate2 ~= nil) then
		AiAgent(pPirate2):addCreatureFlag(AI_NOAIAGGRO)
...
function HeroOfTatooineScreenPlay:pirateLeaderDamage(pLeader, pPlayer, damage)
	...
	if (not SceneObject(pPlayer):isPlayerCreature() or not CreatureObject(pPlayer):hasScreenPlayState(2, "hero_of_tatooine") or CreatureObject(pPlayer):hasScreenPlayState(32, "hero_of_tatooine")
...
function HeroOfTatooineScreenPlay:spawnAggroLeader(pOldLeader, pPlayer, screenID)
	...
	if SceneObject(pOldLeader):isAiAgent() then
		AiAgent(pOldLeader):doDespawn()
	else
...
function HeroOfTatooineScreenPlay:giveHonorWaypoint(pPlayer)
	...
	local waypointID = PlayerObject(pGhost):addWaypoint("tatooine", "Ranch House", "", 4993, -4682, WAYPOINTPURPLE, true, true, 0, 1)
...
function HeroOfTatooineScreenPlay:doHonorStep(pAgent)
	...
	if (nextPoint == nil) then
		if (agentID ~= rancherID) then
			if SceneObject(pAgent):isAiAgent() then
				AiAgent(pAgent):doDespawn()
			else
