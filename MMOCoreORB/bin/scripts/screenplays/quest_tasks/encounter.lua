...
function Encounter:isPlayerInABuilding(pPlayer)
    ...
    if pPlayer == nil then
        return true                                     -- Infinity:  Fail check if pPlayer is nil
    else
...
function Encounter:handleEncounterInRangeCheck(pPlayer)
	...
	if spawnedObjects == nil or not SpawnMobiles.isValidMobile(spawnedObjects[1]) then
		return
	end
...
function Encounter:handleDespawnEvent(pPlayer)
	...
	for i = 1, #spawnedObjects, 1 do
		if SpawnMobiles.isValidMobile(spawnedObjects[i]) then
			...
			AiAgent(spawnedObjects[i]):removeCreatureFlag(AI_ESCORT)
			AiAgent(spawnedObjects[i]):removeCreatureFlag(AI_FOLLOW)
...
function Encounter:doRunAway(pAiAgent)
    if not SpawnMobiles.isValidMobile(pAiAgent) then
		return
	end
