...
function JantaCaveScreenPlay:unbuffedAFKcheck()

	local caveID = 529322
	local playerTable = util:getPlayersInBuilding(caveID)

	if playerTable ~= nil and #playerTable ~= 0 then

		local ejectionPoints = {
			{"dantooine", 7065.38, 48.5574, -4110.88},
			{"dantooine", 7063.11, 48.2184, -4097.16},
			{"dantooine", 7069.71, 51.0272, -4107.04},
			{"dantooine", 7064.85, 48.6967, -4112.6}
		}

		for i=1, #playerTable do
			local player = playerTable[i]

			if player ~= nil and SceneObject(player):isPlayerCreature() and not CreatureObject(player):isDead() and not CreatureObject(player):isIncapacitated() and not util:isAdmin(player) then
				if not CreatureObject(player):hasBuff(MEDICAL_ENHANCE_HEALTH) then
					local roll = getRandomNumber(1,#ejectionPoints)
					SceneObject(player):switchZone(ejectionPoints[roll][1], ejectionPoints[roll][2], ejectionPoints[roll][3], ejectionPoints[roll][4], 0)
				end
			end
		end
	end

	local delay = getRandomNumber(10, 30)
	createEvent(delay * 60 * 1000, "JantaCaveScreenPlay","unbuffedAFKcheck", nil , "")
