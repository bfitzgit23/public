...
function CorellianCorvette:sendAuthorizationSui(pPlayer, pLeader, pCorvette)
	...
	createEvent(30 * 1000, "CorellianCorvette", "closeAuthorizationSui", pPlayer, tostring(pageId))
end
...
function CorellianCorvette:removePrisoner(pPrisoner)
	...
	if SceneObject(pPrisoner):isAiAgent() then
		AiAgent(pPrisoner):doDespawn()
	else
	...
	for i = 1, 66, 1 do
		...
		if (pCell ~= nil) then
			for j = SceneObject(pCell):getContainerObjectsSize() - 1, 0, -1 do
				...
				if pObject ~= nil and not SceneObject(pObject):isPlayerCreature() and not SceneObject(pObject):isPlayerCreature():isPet() then
					if SceneObject(pObject):isAiAgent() then
						AiAgent(pObject):doDespawn()
					else
