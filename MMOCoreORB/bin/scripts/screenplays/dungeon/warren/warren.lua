...
function Warren:notifyEnteredWarren(pBuilding, pPlayer)
	...
	if (pInventory ~= nil) then
        ...
        local privilegedPlayer = pGhost ~= nil and PlayerObject(pGhost):isPrivileged()

		if (pItem ~= nil or privilegedPlayer) then
...
function Warren:isInWarren(pPlayer)
	...
	return SceneObject(pPlayer):getRootParentID() == self.buildingID
end
...
function Warren:spawnMobiles()
	...
	--At St room
	spawnMobile("dantooine", "warren_altered_atst", 300, 25.7, -84, -28.8, 0, 8575745)-- /tele 25.7 -28.8 dantooine -84 8575745
...
function Warren:getBuildingObject()
	return getSceneObject(self.buildingID)
end
