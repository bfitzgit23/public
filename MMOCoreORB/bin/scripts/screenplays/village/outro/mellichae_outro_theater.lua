...
function MellichaeOutroTheater:onMellichaeKilled(pMellichae, pKiller)
	...
	VillageJediManagerCommon.setJediProgressionScreenPlayState(pOwner, VILLAGE_JEDI_PROGRESSION_DEFEATED_MELLICHAE) -- Killed him.
	FsOutro:setCurrentStep(pOwner, 4)
	PadawanTrials:doPadawanTrialsSetup(pOwner)

	local pActiveArea = spawnActiveArea(CreatureObject(pOwner):getZoneName(), "object/active_area.iff", SceneObject(pTheater):getWorldPositionX(), 0, SceneObject(pTheater):getWorldPositionY(), 400, 0)
	...
	for i = 1, #self.theater, 1 do
		...
		if (pObject ~= nil) then
			...
			if (color ~= nil) then
				...
				createObserver(DAMAGERECEIVED, self.taskName, "onPowerShrineHit", pObject)
...
-- custom Infinity stuff
function MellichaeOutroTheater:onPowerShrineHit(pSceneObject, pAttacker, damage)
	if pSceneObject == nil or pAttacker == nil then
		util:cw("nil object or attacker in onPowerShrineHit", 1)
		return 0
	end

	if not util:isPlayerOrPet(pAttacker) then
		util:cw("attacker is not player or pet in onPowerShrineHit", 1)
		return 0
	end

	local ownerID = readData(SceneObject(pSceneObject):getObjectID() .. "questOwner")
	local pPlayer = util:petOwner(pAttacker)

	if ownerID ~= 0 and pPlayer ~= nil then
		local attackerID = SceneObject(pPlayer):getObjectID()
		util:cw("ownerID=" .. ownerID, 1)
		util:cw("killerID=" .. attackerID, 1)
		if ownerID ~= attackerID then
			...
			util:cw("damage player that doesn't own this", 1)
			CreatureObject(pPlayer):inflictDamage(pPlayer, 0, dmg, 0)
			CreatureObject(pPlayer):inflictDamage(pPlayer, 3, dmg, 0)
			CreatureObject(pPlayer):inflictDamage(pPlayer, 6, dmg, 0)
			self:healToFull(pSceneObject)
		end
	else
		util:cw("owner or pPlayeris nil", 1)
	end
...
function MellichaeOutroTheater:onPowerShrineDestroyed(pSceneObject, pKiller, nothing)
	if (pSceneObject == nil or pKiller == nil) then
			util:cw("nil object or killer onPowerShrineDestroyed", 1)
		return 1
	end

	dropObserver(DAMAGERECEIVED, self.taskName, "onPowerShrineHit", pSceneObject)

	local ownerID = readData(SceneObject(pSceneObject):getObjectID() .. "questOwner")
	local pOwner = getSceneObject(ownerID)

	if pOwner == nil then
		printLuaError("pOwner = nil in onPowerShrineDestroyed")
	...
	if (SceneObject(pSceneObject):getTemplateObjectPath() == "object/tangible/jedi/power_shrine_red.iff") then
		...
		if (numOfShrines == 0) then
			self:helpCrystal(pOwner, "red", ownerID)
		end
	elseif (SceneObject(pSceneObject):getTemplateObjectPath() == "object/tangible/jedi/power_shrine.iff") then
		...
		if (numOfShrines == 0) then
			...
			self:helpCrystal(pOwner, "green", ownerID)
		end
...
function MellichaeOutroTheater:helpCrystal(pOwner, color, ownerID)
	...
	if (pDaktar ~= nil and not CreatureObject(pDaktar):isDead()) then
		...
		AiAgent(pDaktar):setDefender(pOwner)
	end
	...
	if (color == "red") then
		...
		if (pMellichae ~= nil and not CreatureObject(pMellichae):isDead()) then
			...
			AiAgent(pMellichae):setDefender(pOwner)
		end
	end
	...
	for i = 1, 4, 1 do
		local zoneName = CreatureObject(pOwner):getZoneName()
		...
		local zLoc = getTerrainHeight(pOwner, xLoc, yLoc)

		local pMobile = spawnMobile(zoneName, mobTemplate, 0, xLoc, zLoc, yLoc, 0, 0)

		if (pMobile ~= nil) then
			AiAgent(pMobile):setDefender(pOwner)
		end
	end
end
