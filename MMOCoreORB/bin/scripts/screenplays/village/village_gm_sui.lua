        ...
        local msg =
            ...
            " ID: " ..
                SceneObject(pPlayer):getObjectID() ..
                    " non Privileged player attempted to acces GMFSVillage -- VillageGmSui:showMainPage."
...
function VillageGmSui.changePhase(pPlayer)
    ...
    sui.setPrompt(
        "Are you sure you want to change the village to the next phase? Doing so will reset the progress of all players in the current phase."
    )
...
function VillageGmSui.playerInfo(pPlayer, targetID)
    ...
    local promptBuf =
        " \\#pcontrast1 " ..
        "Player:" .. " \\#pcontrast2 " .. SceneObject(pTarget):getCustomObjectName() .. " (" .. targetID .. ")\n"
    ...
    elseif (tonumber(readScreenPlayData(pTarget, "PadawanTrials", "completedTrials")) == 1) then
        ...
        if (JediTrials:isOnKnightTrials(pTarget)) then
            ...
            local trialNumber = JediTrials:getCurrentTrial(pTarget)
            if trialNumber == nil then
                trialNumber = "nil"
            end
            local trialName = JediTrials:getTrialStateName(pTarget, trialNumber)
            if trialName == nil then
                trialName = "JediTrial:nil.  trialNumber not found."
            end
            promptBuf =
                promptBuf ..
                "    Current Trial #:  " .. trialNumber .. "  Trial Name:  " .. string.gsub(trialName, "JediTrial:", "") .. "\n"

            local trialData = knightTrialQuests[trialNumber]

            if (trialData ~= nil and (trialData.trialType == TRIAL_HUNT or trialData.trialType == TRIAL_HUNT_FACTION)) then
                local huntTarget = readScreenPlayData(pTarget, "JediTrials", "huntTarget")
                if huntTarget == nil or huntTarget == "" then
                    huntTarget = "nil or blank"
                end
                local targetCountString = readScreenPlayData(pTarget, "JediTrials", "huntTargetCount")
                local targetCount = tonumber(targetCountString)
                if targetCount == nil then
                    targetCount = "nil or blank"
                end
                local targetGoalString = readScreenPlayData(pTarget, "JediTrials", "huntTargetGoal")
                local targetGoal = tonumber(targetGoalString)
                if targetGoal == nil then
                    targetGoal = "nil or blank"
                end
                promptBuf =
                    promptBuf ..
                    "    Hunt Target:  " ..
                        huntTarget .. "  Targets Killed:  " .. targetCount .. "  Target Goal: " .. targetGoal .. "\n"
            end
        else
            promptBuf = promptBuf .. "Padawan Trials Completed\n"
        end
    elseif (VillageJediManagerCommon.hasJediProgressionScreenPlayState(pTarget, VILLAGE_JEDI_PROGRESSION_DEFEATED_MELLICHAE)) then
        if (JediTrials:isOnPadawanTrials(pTarget)) then
            ...
            if (trialData ~= nil and trialData.trialType ~= TRIAL_HUNT) then
                local huntTarget = readScreenPlayData(pTarget, "JediTrials", "huntTarget")
                local targetCountString = readScreenPlayData(pTarget, "JediTrials", "huntTargetCount")
                local targetCount = tonumber(targetCountString)
                if targetCount == nil then
                ...
                promptBuf =
                    promptBuf ..
                    "    Current Trial #:  " .. trialNumber .. "  Trial Name:  " .. string.gsub(trialName, "JediTrial:", "") .. "\n"
                if (huntTarget ~= nil and huntTarget ~= "") then
                    promptBuf =
                        promptBuf ..
                        "    Hunt Target:  " ..
                            huntTarget .. "  Targets Killed:  " .. targetCount .. "  Target Goal: " .. targetGoal .. "\n"
        ...
        if (curStep == FsOutro.OLDMANWAIT) then
            ...
            elseif (timeTilVisit > 0) then
                promptBuf =
                    promptBuf ..
                    " \\#pcontrast1 " ..
                        "Time until visit:" .. " \\#pcontrast2 " .. VillageGmSui:getTimeString(timeTilVisit * 1000) .. "\n"
            else
    ...
    elseif (FsIntro:isOnIntro(pTarget)) then
        ...
        elseif (curStep == FsIntro.SITHWAIT) then
            ...
            elseif (timeTilAttack > 0) then
                promptBuf =
                    promptBuf ..
                    " \\#pcontrast1 " ..
                        "Time Until Attack:" .. " \\#pcontrast2 " .. VillageGmSui:getTimeString(timeTilAttack * 1000) .. "\n"
            else
        ...
        if
            (curStep == FsIntro.OLDMANWAIT or curStep == FsIntro.OLDMANMEET or curStep == FsIntro.SITHWAIT or
                curStep == FsIntro.SITHATTACK)
         then
            promptBuf =
                promptBuf ..
                " -- Encounter Checks --\n" ..
                    "\\#pcontrast1 " ..
                        "InPositionForEncounter:" ..
                            " \\#pcontrast2 " .. tostring(Encounter:isPlayerInPositionForEncounter(pTarget)) .. "\n"

            if (not Encounter:isPlayerInPositionForEncounter(pTarget)) then
                promptBuf =
                    promptBuf ..
                    " \\#pcontrast1 " ..
                        "Player Online:" .. " \\#pcontrast2 " .. tostring(Encounter:isPlayerOnline(pTarget)) .. "\n"
                if (PlayerObject(pGhost):isOnline()) then
                    promptBuf =
                        promptBuf ..
                        " \\#pcontrast1 " ..
                            "Player In a Building:" .. " \\#pcontrast2 " .. tostring(Encounter:isPlayerInABuilding(pTarget)) .. "\n"
                    promptBuf =
                        promptBuf ..
                        " \\#pcontrast1 " ..
                            "Player In NPC City:" .. " \\#pcontrast2 " .. tostring(Encounter:isPlayerInNpcCity(pTarget)) .. "\n"
                end
            end
            promptBuf = promptBuf .. " ----\n"
        end
    ...
    if
        (VillageJediManagerCommon.hasJediProgressionScreenPlayState(pTarget, VILLAGE_JEDI_PROGRESSION_HAS_VILLAGE_ACCESS) or
            VillageJediManagerCommon.getUnlockedBranchCount(pTarget) > 0)
     then
        promptBuf =
            promptBuf ..
            " \\#pcontrast1 " ..
                "Unlocked Branches:" .. " \\#pcontrast2 " .. VillageJediManagerCommon.getUnlockedBranchCount(pTarget) .. "\n"
    end
    ...
    if
        (jediState >= 4 or CreatureObject(pTarget):hasSkill("force_title_jedi_rank_03") or
            CreatureObject(pTarget):hasSkill("force_rank_dark_novice") or
            CreatureObject(pTarget):hasSkill("force_rank_light_novice"))
     then
    ...
    if
        (CreatureObject(pTarget):hasSkill("combat_bountyhunter_investigation_03") or
            CreatureObject(pTarget):hasSkill("mandalorian_master"))
     then
        sui.add("Cancel Player Bounty Mission", "cancelPlayerBountyMission" .. targetID)
    end
...
function VillageGmSui.manageActiveVillageQuest(pPlayer, targetID)
    ...
    if
        (not VillageJediManagerCommon.hasActiveQuestThisPhase(pTarget) or
            VillageJediManagerCommon.hasCompletedQuestThisPhase(pTarget))
     then
    ...
    elseif (curQuest == VILLAGE_PHASE2_DAGEERIN) then
        ...
        promptAdd =
            " \\#pcontrast1 " ..
            "Tasks towards limit:" ..
                " \\#pcontrast2 " .. readScreenPlayData(pTarget, "VillageJediProgression", "FsSadTasksSinceLastTimestamp") .. "\n"
    ...
    elseif (curQuest == VILLAGE_PHASE3_DAGEERIN) then
        ...
        promptAdd =
            " \\#pcontrast1 " ..
            "Tasks towards limit:" ..
                " \\#pcontrast2 " .. readScreenPlayData(pTarget, "VillageJediProgression", "FsSad2TasksSinceLastTimestamp") .. "\n"
...
function VillageGmSui.forceIntroSithAttackEvent(pPlayer, targetID)
    ...
    if (not FsIntro:isOnIntro(pTarget) or (curStep ~= FsIntro.SITHWAIT and curStep ~= FsIntro.SITHATTACK)) then
        CreatureObject(pPlayer):sendSystemMessage(
            "Unable to force the sith attack intro event for " .. playerName .. ", they are not on the correct step."
        )
        return
    end
...
function VillageGmSui.forceIntroOldManEvent(pPlayer, targetID)
    ...
    if (not FsIntro:isOnIntro(pTarget) or (curStep ~= FsIntro.OLDMANWAIT and curStep ~= FsIntro.OLDMANMEET)) then
        CreatureObject(pPlayer):sendSystemMessage(
            "Unable to force the old man intro event for " .. playerName .. ", they are not on the correct step."
        )
        return
    end
...
function VillageGmSui.forceOutroOldManEvent(pPlayer, targetID)
    ...
    if (not FsOutro:isOnOutro(pTarget) or (curStep ~= FsOutro.OLDMANWAIT and curStep ~= FsOutro.OLDMANMEET)) then
        CreatureObject(pPlayer):sendSystemMessage(
            "Unable to force the old man outro event for " ..
                CreatureObject(pTarget):getFirstName() .. ", they are not on the correct step."
        )
        return
    end

    CreatureObject(pPlayer):sendSystemMessage(
        "Now forcing the old man event outro to start for " .. CreatureObject(pTarget):getFirstName() .. "."
    )
...
function VillageGmSui.resetActiveQuest(pPlayer, targetID)
    ...
    sui.setPrompt(
        "Are you sure you want to reset the player's active quest status? This could potentially allow a player to have two active quests at the same time."
    )
    sui.setOkButtonText("Yes")
    sui.setCancelButtonText("No")
...
function VillageGmSui.resetCompletedQuest(pPlayer, targetID)
    ...
    sui.setPrompt(
        "Are you sure you want to reset the player's completed quest status? This will allow the player to complete a second quest this phase."
    )
    sui.setOkButtonText("Yes")
    sui.setCancelButtonText("No")
    sui.setTargetNetworkId(targetID)
...
function VillageGmSui.manageVisibility(pPlayer, targetID)
    ...
    promptBuf =
        promptBuf ..
        " \\#pcontrast1 " .. "Current Visibility: " .. " \\#pcontrast2 " .. PlayerObject(pGhost):getVisibility() .. "\n"
...
function VillageGmSui.frsManagement(pPlayer, targetID)
    ...
    if (luaCouncil == nil or councilType == nil or luaCouncil ~= councilType) then
        promptBuf =
            ...
            " \\#pcontrast1 " ..
                "WARNING:" ..
                    " \\#pcontrast2 Trials council choice does not match council type stored on player. Use Change/Fix Council Type menu option below to fix.\n"
    end
    ...
    promptBuf =
        promptBuf ..
        " \\#pcontrast1 " ..
            "FRS XP:" .. " \\#pcontrast2 " .. withCommas(PlayerObject(pGhost):getExperience("force_rank_xp")) .. "\n"

    sui.setPrompt(promptBuf)

    sui.add("Set FRS Rank", "setFrsRank" .. targetID)
    sui.add("Set FRS XP", "setFrsXp" .. targetID)
    sui.add("Change/Fix Council Type", "changeCouncilType" .. targetID)
    sui.add("Remove from FRS", "removeFromFrs" .. targetID)
...
function VillageGmSui:frsManageCallback(pPlayer, pSui, eventIndex, args)
    ...
    if (pTarget == nil or not SceneObject(pTarget):isPlayerCreature()) then
        Logger:log(
            "Unable to find player for VillageGmSui function frsManageCallback - " .. menuOption .. " using oid " .. targetID,
            LT_ERROR
        )
        return
    end
    ...
    elseif (menuOption == "setFrsXp") then
        ...
        sui.sendTo(pPlayer)
    elseif (menuOption == "changeCouncilType") then
        local sui = SuiListBox.new("VillageGmSui", "changeCouncilTypeCallback")
        sui.setTitle("Change Council Type")
        sui.setPrompt(
            "Choose " ..
                SceneObject(pTarget):getCustomObjectName() ..
                    "'s new council type below. Note: Changing a player's council will set them back to rank 0."
        )
        ...
        sui.sendTo(pPlayer)
    elseif (menuOption == "removeFromFrs") then
        -- Infinity:  Remove from FRS
        ...
        PlayerObject(pGhost):removeFromFrs(CreatureObject(pTarget))

        local msg =
            SceneObject(pPlayer):getCustomObjectName() ..
            " ID: " ..
                SceneObject(pPlayer):getObjectID() ..
                    " used GMFSVillage:suiSetFrsRankCallback on " ..
                        SceneObject(pTarget):getCustomObjectName() .. " ID: " .. targetID
        msg = msg .. " removing them from the FRS."
        Logger:log(msg, LT_WARNING)
...
function VillageGmSui:suiSetFrsRankCallback(pPlayer, pSui, eventIndex, args)
    ...
    if (pTarget == nil or not SceneObject(pTarget):isPlayerCreature()) then
        local msg =
            SceneObject(pPlayer):getCustomObjectName() ..
            " ID: " ..
                SceneObject(pPlayer):getObjectID() .. " attempted to use GMFSVillage:suiSetFrsRankCallback on an improper target."
    ...
    if (rank == nil or rank < 0 or rank > 11) then
        local msg =
            SceneObject(pPlayer):getCustomObjectName() ..
            " ID: " ..
                SceneObject(pPlayer):getObjectID() ..
                    " attempted to use GMFSVillage:suiSetFrsRankCallback and input an improper rank number."
        Logger:log(msg, LT_ERROR)
    ...
    -- Infinity:  Set the rank and adjust FRS xp
    PlayerObject(pGhost):setFrsRank(rank, true)

    local msg =
        SceneObject(pPlayer):getCustomObjectName() ..
        " ID: " ..
            SceneObject(pPlayer):getObjectID() ..
                " used GMFSVillage:suiSetFrsRankCallback on " .. SceneObject(pTarget):getCustomObjectName() .. " ID: " .. targetID
...
function VillageGmSui:suiSetFrsXpCallback(pPlayer, pSui, eventIndex, args)
    ...
    local currentXp = PlayerObject(pTargetGhost):getExperience("force_rank_xp")

    local delta = amount - currentXp

    CreatureObject(pTarget):awardExperience("force_rank_xp", delta, true, false) -- Don't use modifiers

    if (delta < 0) then
        local messageString = LuaStringIdChatParameter("@base_player:prose_revoke_xp")
        messageString:setTO(getStringId("@exp_n:force_rank_xp"))
        messageString:setDI(delta * -1)
        CreatureObject(pTarget):sendSystemMessage(messageString:_getObject())
    end
    ...
    local msg =
        ...
        " ID: " ..
            SceneObject(pPlayer):getObjectID() ..
                " used GMFSVillage:suiSetFrsXpCallback on " .. SceneObject(pTarget):getCustomObjectName() .. " ID: " .. targetID
    msg = msg .. " setting them to " .. amount .. " Force Rank Experience"
    Logger:log(msg, LT_WARNING)
...
function VillageGmSui.unlockLightFrs(pPlayer, targetID)
    ...
    if (jediState ~= 4) then
        local msg =
            ...
            " ID: " ..
                SceneObject(pPlayer):getObjectID() ..
                    " attempted to use GMFSVillage:unlockLightFrs on Player: " ..
                        SceneObject(pTarget):getCustomObjectName() .. " ID: " .. targetID
        ...
        CreatureObject(pPlayer):sendSystemMessage(
            SceneObject(pTarget):getCustomObjectName() ..
                " ID: " ..
                    targetID .. " must have have completed the Jedi Knight Trials and have a Jedi State of 4 to Unlock Light FRS."
        )
        return
    end
    ...
    local msg =
        ...
        " ID: " ..
            SceneObject(pPlayer):getObjectID() ..
                " used GMFSVillage:unlockLightFrs on Player: " .. SceneObject(pTarget):getCustomObjectName() .. " ID: " .. targetID
    Logger:log(msg, LT_WARNING)

    CreatureObject(pPlayer):sendSystemMessage(
        SceneObject(pTarget):getCustomObjectName() .. " ID: " .. targetID .. " is now a member of the Light Jedi Enclave."
    )
end
...
function VillageGmSui.unlockDarkFrs(pPlayer, targetID)
    ...
    if (jediState ~= 8) then
        local msg =
            ...
            " ID: " ..
                SceneObject(pPlayer):getObjectID() ..
                    " attempted to use GMFSVillage:unlockDarkFrs on Player: " ..
                        SceneObject(pTarget):getCustomObjectName() .. " ID: " .. targetID
        msg = msg .. " with an invalid Jedi State of " .. jediState
        Logger:log(msg, LT_ERROR)
        ...
        CreatureObject(pPlayer):sendSystemMessage(
            SceneObject(pTarget):getCustomObjectName() ..
                " ID: " ..
                    targetID .. " must have have completed the Jedi Knight Trials and have a Jedi State of 8 to Unlock Dark FRS."
        )
        return
    end
    ...
    local msg =
        ...
        " ID: " ..
            SceneObject(pPlayer):getObjectID() ..
                " used GMFSVillage:unlockDarkFrs on Player: " .. SceneObject(pTarget):getCustomObjectName() .. " ID: " .. targetID
    Logger:log(msg, LT_WARNING)

    CreatureObject(pPlayer):sendSystemMessage(
        SceneObject(pTarget):getCustomObjectName() .. " ID: " .. targetID .. " is now a member of the Dark Jedi Enclave."
    )
end
...
function VillageGmSui.showLightRanks(pPlayer)
    ...
    local msg =
        SceneObject(pPlayer):getCustomObjectName() ..
        " ID: " .. SceneObject(pPlayer):getObjectID() .. " used GMFSVillage:showLightRanks."
    Logger:log(msg, LT_WARNING)
end
...
function VillageGmSui.showDarkRanks(pPlayer)
    ...
    local msg =
        SceneObject(pPlayer):getCustomObjectName() ..
        " ID: " .. SceneObject(pPlayer):getObjectID() .. " used GMFSVillage:showDarkRanks."
    Logger:log(msg, LT_WARNING)
end
...
function VillageGmSui.branchManagement(pPlayer, targetID)
    ...
    sui.setPrompt(
        "The statuses of " ..
            SceneObject(pTarget):getCustomObjectName() ..
                "'s force sensitive branches are listed below. Branches can be locked and unlocked from this window."
    )
...
function VillageGmSui:branchManagementCallback(pPlayer, pSui, eventIndex, args)
    ...
    if (VillageJediManagerCommon.hasUnlockedBranch(pTarget, branchName)) then
        CreatureObject(pPlayer):sendSystemMessage(
            SceneObject(pTarget):getCustomObjectName() ..
                "'s " .. getStringId("@quest/force_sensitive/utils:" .. branchName) .. " has been LOCKED."
        )
        CreatureObject(pTarget):sendSystemMessage(
            "Your unlock of the branch " ..
                getStringId("@quest/force_sensitive/utils:" .. branchName) .. " has been removed by a GM."
        )
        CreatureObject(pTarget):removeScreenPlayState(2, "VillageUnlockScreenPlay:" .. branchName)
    else
        CreatureObject(pPlayer):sendSystemMessage(
            SceneObject(pTarget):getCustomObjectName() ..
                "'s " .. getStringId("@quest/force_sensitive/utils:" .. branchName) .. " has been UNLOCKED."
        )
...
function VillageGmSui:manageCounterStrikeBasesCallback(pPlayer, pSui, eventIndex, args)
    ...
    if (not FsCsBaseControl:isShieldPoweredDown(pTheater)) then
        ...
        suiPrompt =
            suiPrompt ..
            " \\#pcontrast1 " ..
                "Shield Taken Down At:" .. " \\#pcontrast2 " .. os.date("%c", powerDownTime) .. " (" .. diffString .. " ago)\n"
        ...
        if (pAttacker == nil) then
            ...
            suiPrompt =
                suiPrompt ..
                " \\#pcontrast1 " .. "Current Attacker:" .. " \\#pcontrast2 " .. CreatureObject(pAttacker):getFirstName() .. "\n"
        end
    end
...
function VillageGmSui.cancelPlayerBountyMission(pPlayer, targetID)
    local pTarget = getSceneObject(targetID)
    ...
    if
        (CreatureObject(pTarget):hasSkill("combat_bountyhunter_investigation_03") or
            CreatureObject(pTarget):hasSkill("mandalorian_master"))
     then
        util:cout(pPlayer, "Canceling any player bounty mission for " .. CreatureObject(pTarget):getFirstName(), "green")
        CreatureObject(pTarget):failPlayerBountyMission()
    end
end
...
function VillageGmSui:changeCouncilTypeCallback(pPlayer, pSui, eventIndex, args)
    ...
    local cancelPressed = (eventIndex == 1)
    if (cancelPressed or args == "" or args == nil) then
        return
    end
    ...
    if (councilType == JediTrials.COUNCIL_LIGHT) then
        ...
        CreatureObject(pTarget):surrenderSkill("force_title_jedi_rank_03") -- This should remove them from FRS and clear FRS XP
