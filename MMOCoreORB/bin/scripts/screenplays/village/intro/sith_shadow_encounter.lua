...
function SithShadowEncounter:isTheFirstSithShadowOfThePlayer(pSithShadow, pPlayer)
    ...
    if pPlayer == nil or pSithShadow == nil then
        return false
    end

 	local spawnedSithShadows = SpawnMobiles.getSpawnedMobiles(pPlayer, self.taskName)

	return spawnedSithShadows ~= nil and SpawnMobiles.isValidMobile(spawnedSithShadows[1]) and CreatureObject(spawnedSithShadows[1]):getObjectID() == CreatureObject(pSithShadow):getObjectID()
end
...
function SithShadowEncounter:onEncounterSpawned(pPlayer, spawnedObjects)
	...
	if pPlayer == nil or spawnedObjects == nil or not SpawnMobiles.isValidMobile(spawnedObjects[1]) then
		return
	end
