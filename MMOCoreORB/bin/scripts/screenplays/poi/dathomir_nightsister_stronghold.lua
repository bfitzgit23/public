NightSisterStrongholdScreenPlay = ScreenPlay:new {
	...
	-- original cave cells cave id 4115618: 4115619 - 4115629
}
...
function NightSisterStrongholdScreenPlay:spawnMobiles()
	...
	-- Infinity customized this, the cell id's below do not exist now
	...
	spawnMobile("dathomir", "nightsister_enraged_rancor",720,-64.9,-69.9,-178.3,165,4115626)
	...
	-- blockers to prevent hitting axkva from above
	local obstruct = "object/static/terrain/corellia/rock_crystl_shrpbush_med.iff"
	--local obstruct2 = "object/static/structure/dathomir/nsister_guard_tower_style_01.iff"
	local obstruct2 = "object/tangible/furniture/city/custom_429_0000000000000000000.iff"
	local obLocs = {
		{-95.3, -73.5, -158.7, 4115626},
		{-93.5, -73.5, -159.4, 4115626},
		{-91.8, -73.6, -160.1, 4115626},
		{-90.0, -73.8, -160.7, 4115626},
		{-88.8, -73.9, -161.3, 4115626},
		{-88.2, -73.1, -162.7, 4115626},
		{-74.3, -71.1, -156.3, 4115626},
		{-74.2, -71.1, -157.7, 4115626},
		{-69.1, -71.2, -159.8, 4115626},
		{-73.8, -71.1, -159.6, 4115626},
		{-74.5, -71.2, -161.6, 4115626},
		{-74.7, -71.2, -163.5, 4115626},
		{-87.1, -72.8, -164.8, 4115626},
		{-86.2, -72.4, -166.1, 4115626},
		{-84.5, -72.0, -167.1, 4115626},
	}
	local obLocs2 = {
		{-79.4, -70.2, -170.7, 4115626},
		{-100.3, -72.4, -153.4, 4115626},
		{-110, -69.8, -170.3, 4115626},
		{-72.5, -70.1, -166.7, 4115626},
		{-72.5, -70.1, -166.7, 4115626},
		{-101, -69, -136.8, 4115626},
		{-98.3, -68.9, -136.8, 4115626},
		{-64.5, -69.9, -160.4, 4115626},
	}
	for i=1, #obLocs do
		spawnSceneObject("dathomir", obstruct, obLocs[i][1], obLocs[i][2], obLocs[i][3], obLocs[i][4], math.rad(getRandomNumber(180)-180))
	end
	for i=1, #obLocs2 do
		spawnSceneObject("dathomir", obstruct2, obLocs2[i][1], obLocs2[i][2], obLocs2[i][3], obLocs2[i][4], math.rad(getRandomNumber(180)-180))
	end
...
function NightSisterStrongholdScreenPlay:axkvaKilled(pAxkvaMin)
	createEvent(45 * 60 * 1000, "NightSisterStrongholdScreenPlay", "respawnAxkvaMin", nil, "")
...
function NightSisterStrongholdScreenPlay:spawnGuards(pAxkvaMin)
	...
	local targetID = CreatureObject(pAxkvaMin):getTargetID()
	if targetID ~= nil then
		util:cw("axkva target = " .. targetID, 1)
		local sceno = getSceneObject(targetID)
		if sceno ~= nil then
			local playerTarget = util:petOwner(sceno)
			if playerTarget ~= nil then
				util:cw("pet owner = " .. CreatureObject(playerTarget):getFirstName(), 1)
				local dist = util:distanceObjects(pAxkvaMin, playerTarget)
				if dist ~= nil then
					util:cw("distance = " .. dist, 1)
				end
			end
		end
	end
