BestineElection = ScreenPlay:new {
	...
	-- Infinity sets duration to 14 days
	electionDuration = 14 * 24 * 60 * 60 * 1000, -- Duration of the election phase
	timeTilNextElection = 14 * 24 * 60 * 60 * 1000, -- Time until next election, starts immediately after an election ends
...
-- Infinity custom code to debug and shorten election cycles
function BestineElection:get_electionDuration()
	if util:isDev() then
		--return 5 * 60 * 1000 -- 5 mins for dev
	end
	return self.electionDuration
end
function BestineElection:get_time_Til_Next_Election()
	if util:isDev() then
		return 5 * 60 * 1000 -- 5 mins for dev
	else
		return self.timeTilNextElection
	end
end
function BestineElection:phaseReset()
...
function BestineElection:doPhaseInit()
	...
	--util:cw("BestineElection:doPhaseInit()", 1)
	if (not hasServerEvent("ElectionPhaseChange")) then
		self:setCurrentPhase(1)
		self:createNewVoterList()
		createServerEvent(self:get_electionDuration(), "BestineElection", "doPhaseChange", "ElectionPhaseChange")
	else
		...
		if eventID ~= nil and getServerEventTimeLeft(eventID) <= 0 then
			util:l("Bestine Election timer hit 0 and was fixed on startup or GMScreenplay", nil, 3)
			self:doPhaseChange()
		end
	end
end
...
function BestineElection:forceReset(pTime)
	--util:cw("BestineElection:forceReset", 1)
	self:setCurrentPhase(1)
	self:createNewVoterList()
	if(pTime == 0) then
		rescheduleServerEvent("ElectionPhaseChange", self:get_electionDuration())
	else
...
function BestineElection:doPhaseChange()
	...
	--util:cw("BestineElection:doPhaseChange()", 1)
	...
	if (currentPhase == 1) then
		...
		if (not hasServerEvent("ElectionPhaseChange")) then
			createServerEvent(self:get_time_Til_Next_Election(), "BestineElection", "doPhaseChange", "ElectionPhaseChange")
		else
			rescheduleServerEvent("ElectionPhaseChange", self:get_time_Til_Next_Election())
		end
...
function BestineElection:getCurrentPhase()
	...
	--util:cw("BestineElection:getCurrentPhase() = " .. curPhase, 1)
	return curPhase
end
...
function BestineElection:getElectionNumber()
	...
	--util:cw("BestineElection:getElectionNumber() = " .. electionNum, 1)
	return electionNum
end

function BestineElection:setElectionNumber(newNum)
	setQuestStatus("bestineElection:electionNumber", newNum)
	--util:cw("BestineElection:setElectionNumber() = " .. newNum, 1)
end
