...
function tourAryonConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	local MGtimeLeftInSecs = BestineElection:getPhaseTimeLeft()

	if (screenID == "init_not_election_phase") then
		if (PlayerObject(pGhost):hasGodMode()) then
				if MGtimeLeftInSecs < 0 then
					clonedConversation:addOption("MGMT: PROBLEM - Second Left: " .. MGtimeLeftInSecs, "reset_no")
				else
					clonedConversation:addOption("MGMT: Second Left: " .. MGtimeLeftInSecs, "reset_no")
				end
			clonedConversation:addOption("MGMT: Reset Now", "reset_now")
			clonedConversation:addOption("MGMT: Reset Default", "reset_def")
		end
	...
	elseif (screenID == "reset_now") then
		BestineElection:forceReset(1)
	elseif (screenID == "reset_def") then
		BestineElection:forceReset(0)
		local resetLeftInSecs = BestineElection:getPhaseTimeLeft()
	elseif (screenID == "init_election_phase") then
		if (PlayerObject(pGhost):hasGodMode()) then
			if MGtimeLeftInSecs ~= nil then
				if MGtimeLeftInSecs < 0 then
					clonedConversation:addOption("MGMT: PROBLEM - Second Left: " .. MGtimeLeftInSecs, "reset_no")
				else
					clonedConversation:addOption("MGMT: Second Left: " .. MGtimeLeftInSecs, "reset_no")
				...
				clonedConversation:addOption("MGMT: Second Left is nil", "reset_no")
			end
			clonedConversation:addOption("MGMT: Reset Now", "reset_now")
			clonedConversation:addOption("MGMT: Reset Default", "reset_def")
		end
