...
function RecruiterConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	util:cw("recruiter screenID=" .. screenID, 1)

	if screenID == "greet_member_start_covert" or screenID == "greet_member_start_overt" then
		if not util:hasQuestState(pPlayer, "faction:tradein", 1) then
			util:cw("adding option for faction pet trade in")
			clonedConversation:addOption("Faction Pet Trade-in","faction_pet_tradein")
		end
	end
	...
	elseif screenID == "faction_pet_tradein_agree" then
		local itemImperial = {}
		local itemRebel = {}
		local ni = 1
		local nr = 1
		local pInventory = CreatureObject(pPlayer):getSlottedObject("datapad")
		if pInventory ~= nil then
			for i = 0, SceneObject(pInventory):getContainerObjectsSize() - 1, 1 do
				local pItem = SceneObject(pInventory):getContainerObject(i)
				if (pItem ~= nil and SceneObject(pItem):isPetControlDevice()) then
					local controlledObject = IntangibleObject(pItem):getControlledObject()
					if (controlledObject ~= nil) then
						local iName = AiAgent(controlledObject):getCreatureTemplateName()  -- Wicked this will show rebel_droideka in the string
						util:cw("looking for " .. iName, 1)

						local foundit = false
						for i, v in pairs(imperialRewardData['hirelings']) do
							if iName == v.controlledObjectTemplate then
								createEvent(ni * 1000, "RecruiterConvoHandler", "tradeImperialStuffDestroy", pPlayer, tostring(SceneObject(pItem):getObjectID()))
								util:l("Faction Trade Imp - Destroyed " .. i, pPlayer, 2)
								ni = ni + 1
								table.insert(itemImperial, i)
								foundit = true
							end
						end
					
						if not foundit then
							for i, v in pairs(rebelRewardData['hirelings']) do
								if iName == v.controlledObjectTemplate then
									createEvent((nr + ni) * 1000, "RecruiterConvoHandler", "tradeRebelStuffDestroy", pPlayer, tostring(SceneObject(pItem):getObjectID()))
									util:l("Faction Trade Reb - Destroyed " .. i, pPlayer, 2)
									setQuestStatus("faction:tradein:reb:dest" .. nr, i)
									nr = nr + 1
									table.insert(itemRebel, i)
									foundit = true
								end
							end
						end
		...
		setQuestStatus("faction:tradein:impqty", #itemImperial)
		setQuestStatus("faction:tradein:rebqty", #itemRebel)
		setQuestStatus("faction:tradein:impgiven", 0)
		setQuestStatus("faction:tradein:rebgiven", 0)
		setQuestStatus("faction:tradein:impdest", 0)
		setQuestStatus("faction:tradein:rebdest", 0)
		local nis = ni + nr + 2
		util:cw("nis=" .. nis, 1)
		for i=1, #itemImperial do
			createEvent(nis * 1000, "RecruiterConvoHandler", "tradeImperialStuff", pPlayer, itemImperial[i])
			nis = nis + 1
		end
		for i=1, #itemRebel do
			createEvent(nis * 1000, "RecruiterConvoHandler", "tradeRebelStuff", pPlayer, itemRebel[i])
			nis = nis + 1
		end
		-- set quest state
		if util:isDev() then
			util:cw("faction trade in flag not set on dev", 1)
		else
			util:setQuestState(pPlayer, "faction:tradein", 1)
		end
...
function RecruiterConvoHandler:tradeImperialStuff(pPlayer, pItem)
	if pPlayer ~= nil and pItem ~= nil then
		local count = tonumber(getQuestStatus("faction:tradein:impgiven"))
		count = count + 1
		util:cw("count = " .. count, 1)
		setQuestStatus("faction:tradein:impgiven", count)
		setQuestStatus("faction:tradein:imp:give" .. count, pItem)
		util:l("Faction Trade Imp - Received " .. pItem, pPlayer, 2)
		util:giveQuestReward(pPlayer, "object/tangible/item/quest/faction_pet_token.iff", pItem)
		--recruiterScreenplay:awardData(pPlayer, "imperial", pItem, 0, true)
	end
end
function RecruiterConvoHandler:tradeRebelStuff(pPlayer, pItem)
	if pPlayer ~= nil and pItem ~= nil then
		local count = tonumber(getQuestStatus("faction:tradein:rebgiven"))
		count = count + 1
		setQuestStatus("faction:tradein:rebgiven", count)
		setQuestStatus("faction:tradein:reb:give" .. count, pItem)
		util:l("Faction Trade Reb - Received " .. pItem, pPlayer, 2)
		util:giveQuestReward(pPlayer, "object/tangible/item/quest/faction_pet_token2.iff", pItem)
		--recruiterScreenplay:awardData(pPlayer, "rebel", pItem, 0, true)
	end
end
function RecruiterConvoHandler:tradeImperialStuffDestroy(pPlayer, pItem)
	if pPlayer ~= nil and pItem ~= nil then
		local sceno = getSceneObject(pItem)
		if (sceno ~= nil and SceneObject(sceno):isPetControlDevice()) then
			local iName = SceneObject(sceno):getObjectName()
			IntangibleObject(sceno):destroyPCD(pPlayer)
			util:l("Faction Trade Imp - Destroyed " .. iName, pPlayer, 2)
			local count = tonumber(getQuestStatus("faction:tradein:impdest"))
			count = count + 1
			util:cw("count = " .. count, 1)
			setQuestStatus("faction:tradein:impdest", count)
			setQuestStatus("faction:tradein:imp:dest" .. count, iName)
		end
	end
end
function RecruiterConvoHandler:tradeRebelStuffDestroy(pPlayer, pItem)
	if pPlayer ~= nil and pItem ~= nil then
		local sceno = getSceneObject(pItem)
		if (sceno ~= nil and SceneObject(sceno):isPetControlDevice()) then
			local iName = SceneObject(sceno):getObjectName()
			IntangibleObject(sceno):destroyPCD(pPlayer)
			util:l("Faction Trade Reb - Destroyed " .. iName, pPlayer, 2)
			local count = tonumber(getQuestStatus("faction:tradein:rebdest"))
			count = count + 1
			setQuestStatus("faction:tradein:rebdest", count)
			setQuestStatus("faction:tradein:reb:dest" .. count, iName)
		end
	end
end