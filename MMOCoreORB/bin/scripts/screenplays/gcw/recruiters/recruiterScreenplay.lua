...
function recruiterScreenplay:awardData(pPlayer, faction, itemString, gcwDiscount, tradein)
	-- Infinity custom code for the trade in
	-- if players are trading they don't need the checks or charges
	if tradein == nil then
		tradein = false
	else
		tradein = true
	...
	if not tradein and factionStanding < (itemCost + self.minimumFactionStanding) then
		return self.errorCodes.NOTENOUGHFACTION
	end
