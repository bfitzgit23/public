...
function CityControlLanding:chanceToSpawn()
	...
	if (chance <= 15) then -- 15% Chance
		createEvent(20 * 60 * 1000, "CityControlLanding", "spawnCityLanding", nil, "false") --20min after chance roll to allow for load time
	else
		createEvent(6 * 60 * 60 * 1000, "CityControlLanding", "chanceToSpawn", nil, "") --6hrs
	end
end
...
function CityControlLanding:spawnCityLanding(spawnCity, manual)
	...
	if (pShuttle ~= nil) then
		...
		if (#playerTable > 0) then
			...
			createEvent(20 * 1000, "CityControlLanding", "cleanUp", nil, "")
		end
	end
end
...
function CityControlLanding:handleShuttlePosture(shuttle)
	...
	if (landingType == "IMPERIAL") then
		if (shuttlePosture == "UPRIGHT") then
			...
			createEvent(19000, "CityControlLanding", "spawnLandingParty", nil, "")
		else
			CreatureObject(pShuttle):setPosture(UPRIGHT)
		end
	elseif (landingType == "REBEL") then
		if (shuttlePosture == "PRONE") then
			...
			createEvent(25000, "CityControlLanding", "spawnLandingParty", nil, "")
		else
			CreatureObject(pShuttle):setPosture(PRONE)
		end
	else
...
function CityControlLanding:despawnLanding()
	...
	createEvent(5 * 1000, "CityControlLanding", "despawnMobiles", nil, "")
	...
	createEvent(5 * 60 * 1000, "CityControlLanding", "cleanUp", nil, "")
end
...
function CityControlLanding:cleanUp()
	...
	if (manualControl == "false") then
		createEvent(4 * 60 * 60 * 1000, "CityControlLanding", "chanceToSpawn", nil, "") -- 4 hrs
	end
