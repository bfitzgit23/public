...
function KnightTrials:notifyKilledHuntTarget(pPlayer, pVictim, pKillerID)
	if (pVictim == nil or pPlayer == nil) then
		return 0
	end

    -- Infinity Zone and Distance checks
	local zp = SceneObject(pPlayer):getZoneName()
	local zv = SceneObject(pVictim):getZoneName()
	if zp ~= zv then
		return 0
	end
	local player = util:petOwner(pPlayer)
	local dist = util:distanceObjects(player, pVictim)
	if dist > 80 then
		return 0
	end
    ...
    --[[ Changed to our Infinity checks above
...
-- Infinity Custom screenplay SUI
function KnightTrials:suiShowCurrentTrial(pPlayerID)
	if pPlayerID ~= nil then
		local pPlayer = getSceneObject(pPlayerID)
		if pPlayer ~= nil then
			local admin = readData("gm:sui:adminID")
			if admin ~= nil then
				local gm = getSceneObject(admin)
				if gm ~= nil then
					...
					if trialNumber == nil then
						util:cout(gm, "Not on knight trials", "orange")
						return
					end
					local trialsCompleted = JediTrials:getTrialsCompleted(pPlayer)
