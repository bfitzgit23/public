...
function SpawnMobiles.isValidMobile(pMobile)
	return pMobile ~= nil and type(pMobile) == "userdata" and SceneObject(pMobile):isAiAgent() and not SceneObject(pMobile):isPet()
end
	...
	for spawnNumber = 1, #spawnPoints, 1 do
		...
		if SpawnMobiles.isValidMobile(spawnedObject) then
			if mobileList[spawnNumber]["setNotAttackable"] then
                AiAgent(spawnedObject):addCreatureFlag(AI_NOAIAGGRO) --Infinity:  Removed setNoAiAggro hook
			end
...
function SpawnMobiles.getSpawnedMobilePointersList(pSceneObject, prefix)
	...
	for i = 1, numberOfSpawns, 1 do
		...
		spawnedMobiles[i] = getSceneObject(mobileID)
	end
...
function SpawnMobiles.despawnMobilesInList(pSceneObject, spawnedMobilesList, prefix, storeRespawn)
	...
	if spawnedMobilesList ~= nil then
		...
		for i = 1, #spawnedMobilesList, 1 do
			if storeRespawn and (not SpawnMobiles.isValidMobile(spawnedMobilesList[i]) or CreatureObject(spawnedMobilesList[i]):isDead()) then
				writeData(playerID .. prefix .. SPAWN_MOBILES_STRING .. i .. ":noRespawn", 1)
			end

			if SpawnMobiles.isValidMobile(spawnedMobilesList[i]) then
...
function SpawnMobiles.isFromSpawn(pSceneObject, prefix, pMobile)
    ...
    if pSceneObject == nil or not Spawnmobiles.isValidMobile(pMobile) then
        return false
    end
