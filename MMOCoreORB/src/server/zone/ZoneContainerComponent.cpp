...
bool ZoneContainerComponent::transferObject(SceneObject* sceneObject, SceneObject* object, int containmentType, bool notifyClient, bool allowOverflow, bool notifyRoot) const {
	...
	if (parent == nullptr) {
		...
		StringBuffer msg;
		msg << "Attempting to insert object to zone and parent != nullptr." << endl;
		msg << "  Zone: " << zone->getZoneName() << endl; 
		msg << "  Object ID: " << object->getObjectID() << "  Object Name: " << object->getDisplayedName() << endl;
		msg << "  Parent OID: " << parent->getObjectID() << "  Parent Name: " << parent->getDisplayedName() << endl;
	}
