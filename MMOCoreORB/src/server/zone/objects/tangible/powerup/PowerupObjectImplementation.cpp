...
void PowerupObjectImplementation::updateCraftingValues(CraftingValues* values, bool firstUpdate) {
	...
	if (firstUpdate) {
		...
		uses = 500; // Powerups are always 100 uses  //Infinity:  Increased to 500 uses
	...
	// Handle subsequent crafting updates derived from experimentation
...
#ifdef DEBUG_POWERUPS
		info(true) << "has secondary attribute -- Current Effect = " << effect << " with a roll of " << roll << ". modifiers.size() = " << modifiers.size() << ".";
#endif // DEBUG_POWERUPS
		...
		// 2nd+ secondary stats -- Effect is greater than 50% check to apply thestat - roll chance 10%
			...
			addSecondaryStat(values, pupTemplate);
		// 3rd secondary stat
		} else if ((effect >= 70.f) && modifiers.size() == 3 && roll < 25) { 
			addSecondaryStat(values, pupTemplate);
		} else if ((effect >= 90.f) && modifiers.size() == 4 && roll < 25) { 
			addSecondaryStat(values, pupTemplate);
		}
	}
	...
	float currentValue = values->getCurrentValue("effect");
	...
	for (int i = 0; i < modifiers.size(); ++i) {
		...
		} else if (i == 1 && (modifiers.size() > 2) && (stat->getValue() < 8.0f)) {
#ifdef DEBUG_POWERUPS
		...
		float newValue = (currentValue / values->getMaxValue("effect")) * MAXSECONDARY;
		float valueChange = (newValue - stat->getValue()) / statsLeft;
		lastSecondary = stat->getValue() + valueChange;
