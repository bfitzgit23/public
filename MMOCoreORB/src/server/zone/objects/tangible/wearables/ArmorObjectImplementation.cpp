...
void ArmorObjectImplementation::fillAttributeList(AttributeListMessage* alm, CreatureObject* object) {
	// Condition
	if (maxCondition > 0) {
        int currentCondition = maxCondition-(int)conditionDamage;
		StringBuffer cond;
		cond << currentCondition << "/" << maxCondition << " (" << floor((float)currentCondition / (float)maxCondition * 100.0f)  << "%)";
...
float ArmorObjectImplementation::getTypeValue(int type, float value) const {
	...
	float newValue = 0;             //Infinity:  Going to allow float for calculations

	if (vulnerabilites & type) {
		newValue = value;
		if (newValue > 80)          //Infinity:  Vulnerable moved to special should cap at 80%
		    newValue = 80;
    }
	...
	auto armor = const_cast<ArmorObjectImplementation*>(this);

	if (armor != nullptr) {
		if (type == SharedWeaponObjectTemplate::LIGHTSABER && newValue > 39.0f && !armor->isPsgArmorObject()) { // Infinity:  Hardcap for Mando LS Resist until better resources avaiable
			// error("Someone using armor with over 39% lightsaber resist.  " + String::valueOf(newValue) + "%.  Armor capped at 39%.");
			newValue = 39;
		}

		if (type == SharedWeaponObjectTemplate::ACID && newValue > 45.0f && !armor->isPsgArmorObject()) { // Infinity:  Hardcap of 45% acid resist for Tantel armor for balance.
			if (armor->getObjectTemplate()->getTemplateFileName().contains("tantel")) {
				//error("Someone using Tantel armor with over 45% acid resist.  " + String::valueOf(newValue) + "%.  Armor capped at 45%.");
				newValue = 45;
			}
		}
	}
	...
	if (newValue > 90) {
		newValue = 90; // Infinity:  Hardcap unconditionally
	}

	return floor(newValue);    //Infinity:  Going to truncate
}

int ArmorObjectImplementation::handleObjectMenuSelect(CreatureObject* player, byte selectedID) {
	if (selectedID == 69 && player->getSkillMod("slicing") >= 35 && isASubChildOf(player)) {//Infinity:  Skillmod check instead of skill box
		if (isSliced()) {
