...
void ContainerImplementation::fillObjectMenuResponse(ObjectMenuResponse* menuResponse, CreatureObject* player) {
	...
	if (isSliceable() && isContainerLocked() && player->getSkillMod("slicing") >= 5) //Infinity:  Skillmod check instead of skill box
...
int ContainerImplementation::handleObjectMenuSelect(CreatureObject* player, byte selectedID) {
	...
	} else if (selectedID == 69 && player->getSkillMod("slicing") >= 5) {  //Infinity:  Skillmod check instead of skill box
