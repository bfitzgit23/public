...
void VendorDataComponent::payMaintenance(CreatureObject* player) {
	...
	ManagedReference<CreatureObject*> user = nullptr;

	if (player != nullptr) {
		user = player;
	}
	else {
		user = zoneServer->getObject(getOwnerId()).castTo<CreatureObject*>();
	}

	if (user == nullptr)
		return;

	ManagedReference<SuiInputBox*> input = new SuiInputBox(user, SuiWindowType::STRUCTURE_VENDOR_PAY);
	...
	user->getPlayerObject()->addSuiBox(input);
	user->sendMessage(input->generateMessage());

}

void VendorDataComponent::handlePayMaintenance(int value, CreatureObject* player) {
	ManagedReference<SceneObject*> strongParent = parent.get();
	if (strongParent == nullptr || strongParent->getZoneServer() == nullptr)
		return;
	...
	ManagedReference<CreatureObject*> owner = zoneServer->getObject(getOwnerId()).castTo<CreatureObject*>();
	...
	if(value > 100000) {
		user->sendSystemMessage("@player_structure:vendor_maint_invalid");
		return;
	}

	if(value <= 0) {
		user->sendSystemMessage("@player_structure:amt_greater_than_zero");
		return;
	}

	unsigned long totalCredits = (uint64)user->getBankCredits() + (uint64)user->getCashCredits();

	if(totalCredits >= value) {
		if (user->getBankCredits() > value) {
			TransactionLog trx(user, strongParent, TrxCode::VENDORMAINTANENCE, value, false);
			maintAmount += value;
			user->subtractBankCredits(value);
		} else {
			TransactionLog trxCash(user, strongParent, TrxCode::VENDORMAINTANENCE, value - user->getBankCredits(), true);
			user->subtractCashCredits(value - user->getBankCredits());
			maintAmount += value - user->getBankCredits();

			TransactionLog trxBank(user, strongParent, TrxCode::VENDORMAINTANENCE, user->getBankCredits(), false);
			trxBank.groupWith(trxCash);
			user->subtractBankCredits(user->getBankCredits());
			maintAmount += user->getBankCredits();
		}
		...
		user->sendSystemMessage("@player_structure:vendor_maint_denied");
	}
}

void VendorDataComponent::withdrawMaintenance() {
	ManagedReference<SceneObject*> strongParent = parent.get();
	if (strongParent == nullptr || strongParent->getZoneServer() == nullptr)
		return;
...
void VendorDataComponent::handleWithdrawMaintenance(int value) {
	ManagedReference<SceneObject*> strongParent = parent.get();
	if (strongParent == nullptr || strongParent->getZoneServer() == nullptr)
		return;
