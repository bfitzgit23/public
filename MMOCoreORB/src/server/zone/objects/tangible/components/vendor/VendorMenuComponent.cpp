...
void VendorMenuComponent::fillObjectMenuResponse(SceneObject* sceneObject, ObjectMenuResponse* menuResponse, CreatureObject* player) const {
	...
	if (!playerIsOwner) {
		...
		if (ghost->isAdmin()) { // Infinity:  Added additional admin-only options
	...
	if (!vendorData->isInitialized()) {
		...
		if (vendorData->isVendorSearchEnabled()) {
			...
			ManagedReference<SceneObject*> scenoRootParent = sceneObject->getRootParent();
			if (scenoRootParent != nullptr) {
				ManagedReference<BuildingObject*> building = cast<BuildingObject*>(scenoRootParent.get());
...
int VendorMenuComponent::handleObjectMenuSelect(SceneObject* sceneObject, CreatureObject* player, byte selectedID) const {
	...
	if (!owner) {
		if (ghost->isPrivileged()) {
			...
			if (ghost->isAdmin()) { // Infinity:  Added additional admin-only options
				if (selectedID == 73) {
					vendorData->payMaintenance(player);
					return 0;
				} else if (selectedID == 78) {
	...
	switch (selectedID) {
		...
		case 73: {
			vendorData->payMaintenance();
			return 0;
		}

		case 74: {
			vendorData->withdrawMaintenance();
			return 0;
		}
