...
public:
	...
	inline void setInitialized(bool val) {
		...
		setVendorSearchEnabled(false);  //Infinity:  Vendor Search disabled by default at initialization
	}
	...
	void payMaintenance(CreatureObject* player = nullptr);

	void withdrawMaintenance();

	void handlePayMaintenance(int value, CreatureObject* player);

	void handleWithdrawMaintenance(int value);
