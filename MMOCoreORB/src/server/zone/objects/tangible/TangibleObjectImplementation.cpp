...
void TangibleObjectImplementation::setFactionStatus(int status) {
	...
	if (isPlayerCreature()) {
		...
		Reference<StoreSpawnedChildrenTask*> task = new StoreSpawnedChildrenTask(creature, std::move(petsToStore));
		task->execute();
...
void TangibleObjectImplementation::notifyInsert(QuadTreeEntry* object) {
	if (object == nullptr)
		return;
