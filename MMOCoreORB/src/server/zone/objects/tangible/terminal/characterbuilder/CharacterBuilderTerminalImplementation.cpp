...
void CharacterBuilderTerminalImplementation::gmenhanceCharacter(CreatureObject* player) {
	PlayerManager* pm = player->getZoneServer()->getPlayerManager();
	...
	pm->gmenhanceCharacter(player,buffAmount);
	...
	for (int i = 0; i < ghost->getActivePetsSize(); i++) {
		...
		if (pet != nullptr) {
			...
			pm->gmenhanceCharacter(pet,buffAmount);
		}
	}
}