...
void WeaponObjectImplementation::fillAttributeList(AttributeListMessage* alm, CreatureObject* object) {
	...
	//Infinity:  Saber Force Cost, should be min 1.0 for lightsabers
	if (isJediWeapon()) {
		alm->insertAttribute("forcecost", String::format("%.1f", Math::max(1.0f, getForceCost())));  //Infinity:  Show one decimal on force cost, minimum of 1.0 FC
	}
	...
	if ((speedSlice > 1.0 && damageSlice > 1.0) && object != nullptr && object->checkCooldownRecovery("WeaponSlice:" + String::valueOf(getObjectID()))) {
		...
		StringBuffer msg;
		msg << "A suspicious weapon has been identified.\n\n";
		msg << "Player Viewing: \t\t" << object->getFirstName() << "\n";
		msg << "Crafter's Name: \t\t" << getCraftersName() << "\n";
		msg << "Crafter's ID: \t\t\t" << getCraftersID() << "\n";
		msg << "Weapon Name: \t\t" << getDisplayedName() << "\n";
		msg << "Weapon Template: \t" << getObjectTemplate()->getTemplateFileName() << "\n";
		msg << "Weapon OID: \t\t" << getObjectID() << "\n";
		msg << "Damage Slice: \t\t" << damageSlice << "\n";
		msg << "Speed Slice: \t\t" << speedSlice << "\n";

		String sender = "Infinity Secret Police";
		String subject = "Suspicious Weapon Identified";
		...
		object->updateCooldownTimer("WeaponSlice:" + String::valueOf(getObjectID()), 60 * 60 * 1000);		
	}


	if (isJediWeapon() && getMaxDamage() > 750 && getParentRecursively(SceneObjectType::NPCCREATURE) != nullptr && object != nullptr && object->checkCooldownRecovery("Lightsaber:" + String::valueOf(getObjectID()))) {
		...
		StringBuffer msg;
		msg << "A suspicious lightsaber has been identified.\n\n";
		msg << "Player Viewing: \t\t" << object->getFirstName() << "\n";
		msg << "Crafter's Name: \t\t" << getCraftersName() << "\n";
		msg << "Crafter's ID: \t\t\t" << getCraftersID() << "\n";
		msg << "Weapon Name: \t\t" << getDisplayedName() << "\n";
		msg << "Weapon Template: \t" << getObjectTemplate()->getTemplateFileName() << "\n";
		msg << "Weapon OID: \t\t" << getObjectID() << "\n";
		msg << "Min Damage: \t\t\t" << getMinDamage() << "\n";
		msg << "Max Damage: \t\t" << getMaxDamage() << "\n";

		String sender = "Infinity Secret Police";
		String subject = "Suspicious Lightsaber Identified";
		String body = msg.toString();
		...
		object->updateCooldownTimer("Lightsaber:" + String::valueOf(getObjectID()), 60 * 60 * 1000);		
	}
}
...
void WeaponObjectImplementation::decay(CreatureObject* user) {
	...
	if (hasPowerup())
		chance += 25;	//Infinity: Increase chance of weapon decay by 50%, not 200%
		...
		if (isJediWeapon()) {
			...
			for (int i = 0; i < saberInv->getContainerObjectsSize(); i++) {
				...
				if (crystal != nullptr) {
					...
					//Infinity:  Roll again with a 25% chance to fail to reduce durability loss on LS components.  Random chance per crystal
					int crystalRoll = System::random(100);
					if (crystalRoll < 75) {
