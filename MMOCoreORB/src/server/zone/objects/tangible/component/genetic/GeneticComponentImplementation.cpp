...
void GeneticComponentImplementation::updateCraftingValues(CraftingValues* values, bool firstUpdate) {
	...
	info(true) << " Electricity = " << elecResist << " Acid = " << acidResist << " Stun = " << stunResist << " Saber = " << saberResist;
#endif
	...
	health = ((hardiness * 15) + (dexterity * 3)) * 2;
	action = ((dexterity * 15) + (intellect * 3)) * 2;
	mind   = ((intellect * 15) + (hardiness * 3)) * 2;
	stamina = ((dexterity * 15) + (endurance * 3)) * 2;
	willPower = ((intellect * 15) + (cleverness * 3)) * 2;
	constitution = ((hardiness * 15) + (fortitude * 3)) * 2;
	focus = ((intellect * 15) + (dependability * 3)) * 2;
	strength = ((hardiness * 15) + (dependability * 3 * 2)) * 2;
	quickness = ((dexterity * 15) + (dependability * 3 * 2)) * 2;
	...
	// toHit Calculation
	hit = 0.75 + (cleverness/1000.0f);	//Infinity:  Custom calc

#ifdef DEBUG_GENETIC_LAB
	info(true) << "ToHit: " << hit;
#endif

	float damageVar = ceil(15.0f + (735.0f * power / 1000.0f));

#ifdef DEBUG_GENETIC_LAB
	info(true) << "damageVar: " << damageVar;
#endif

	speed = Math::min(2.4f,Math::max(2.0f, (2.7f - round(courage/1000.0f * 10.0f) / 10.0f)));

#ifdef DEBUG_GENETIC_LAB
	info(true) << "Speed: " << speed;
#endif

	// Calculate damage  Infinity
	maxDam = Math::min(1800.0f, round(damageVar * speed));
...
float GeneticComponentImplementation::getEffectiveArmor() {
	float effectiveCalc = (kinResist + energyResist + blastResist + heatResist + coldResist + elecResist + acidResist + stunResist + (saberResist / 35.0f * 0.90f)) * 2.0f;
