...
CreatureObject* ThreatMap::getHighestDamageGroupLeader() {
	...
	for (int i = 0; i < size(); ++i) {
		...
		uint32 totalDamage = entry->getTotalDamage();  //Infinity:   DOTs count toward loot rights, changed from getLootDamage
...
uint32 ThreatMap::getTotalDamageByAttacker(TangibleObject* attacker) {
	Locker locker(&lockMutex);
	...
	for (int i = 0; i < size(); ++i) {
		...
		ManagedReference<TangibleObject*> tano = elementAt(i).getKey();

		if (tano != nullptr && tano == attacker)
