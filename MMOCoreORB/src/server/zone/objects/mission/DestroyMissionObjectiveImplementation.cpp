...
void DestroyMissionObjectiveImplementation::spawnLair() {
	...
	if (lairObject == nullptr) {
		...
		lairObject->setObjectName("@lair_n:" + lairName, false);
	 	...
	 	lairObject->setMaxCondition(condition);  //Infinity:  Custom lair condition based on highest CL mobile (not boss)	
...
void DestroyMissionObjectiveImplementation::abort() {
	...
	if (hasObservers()) {
		...
		if (lair != nullptr) {
			...
			//Infinity:  Let's destroy all nearby faction NPCs when a faction destroy missions is aborted
			int lairFaction = lair->getFaction();

			auto zone = lair->getZone();

			if (zone != nullptr && (lairFaction == Factions::FACTIONIMPERIAL || lairFaction == Factions::FACTIONREBEL)) {

				CloseObjectsVector* closeObjectsVector = lair->getCloseObjects();
				SortedVector<ManagedReference<QuadTreeEntry*> > closeObjects;

				if (closeObjectsVector != nullptr) {
					closeObjectsVector->safeCopyReceiversTo(closeObjects, CloseObjectsVector::CREOTYPE);
				} else {
					zone->getInRangeObjects(lair->getWorldPositionX(), lair->getWorldPositionY(), 75.0f, &closeObjects, true);
				}
				...
				for (int i = 0; i < closeObjects.size(); ++i) {
					...
					if (object == nullptr || !object->isAiAgent() || object->isPet() || (closeObjectsVector != nullptr && !object->isInRange(lair, 75.0f))) {
						continue;
					}

					ManagedReference<AiAgent*> agent = cast<AiAgent*>(object.get());

					if (agent != nullptr && agent->getFaction() == lairFaction) {
						Locker locker(agent);
