...
class FindTargetTask : public Task, public Logger {
	...
	bool findAndTrackSuccess(CreatureObject* player, BountyMissionObjective* objective) {\
		...
		if (zoneServer != nullptr && mission != nullptr) {
			...
			if (missionManager != nullptr && missionManager->hasPlayerBountyTargetInList(targetId)) {
				target = zoneServer->getObject(targetId).castTo<CreatureObject*>();
			}
		}
		...
		if (target != nullptr && target->isPlayerCreature()) {
			...
			String targetName = target->getDisplayedName();

			if (arakyd) { //Infinity: Log all of these
				Logger::console.error("FindTargetTask:  Hunter " + playerName + " successfully tracked player " + targetName + " with an Arakyd probe droid");
			}
			else if (objective->getLastTrackingLogTimestampSeconds() >= 300) {  //Infinity:  Only log once every 5 minutes
				Logger::console.error("FindTargetTask:  Hunter " + playerName + " successfully tracked player " + targetName + " with a seeker droid.");
				Locker locker(objective);
				objective->updateLastTrackingLogTimestamp();
			}

			//Infinity:  Modified anonymous
			if (ConfigManager::instance()->getBool("Core3.MissionManager.AnonymousBountyTerminals", false) && mission->getTargetName() == "Unidentified Player Target") {
				Locker clocker(mission, player);
				mission->setMissionTitle("Player Bounty", " Eliminate the player " + targetName);
