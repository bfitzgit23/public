...
void ManufactureSchematicImplementation::fillAttributeList(AttributeListMessage* alm, CreatureObject* object) {
	...
	alm->insertAttribute("data_volume", 1.0f);  //Infinity:  dataSize always 1

	alm->insertAttribute("complexity", complexity);   // Added to understand factory times
...
void ManufactureSchematicImplementation::setPrototype(TangibleObject* tano) {
	...
	prototype = tano;
	crafter = nullptr;
	dataSize = 1; //draftSchematic->getSize();  //Infinity:  All manufacturing schematics to be dataSize 1
