...
void BuildingObjectImplementation::notifyDissapear(QuadTreeEntry* object) {
	...
	if (object == nullptr) { //Infinity:
		error("object is nullptr in BuildingObjectImplementation::notifyDissapear");
		return;
	}
    ...
    if (sceneO == nullptr) {
        error("sceneO is nullptr after cast in BuildingObjectImplementation::notifyDissapear");
		return;
    }
	uint64 scnoID = sceneO->getObjectID();
...
#ifdef DEBUG_COV
		if (sceneO->isPlayerCreature()) {
			info(true) << getObjectName() << " - BuildingObjectImplementation::notifyDissapear for Player: " << sceneO->getDisplayedName() << " ID: " << scnoID;
		}
#endif // DEBUG_COV
