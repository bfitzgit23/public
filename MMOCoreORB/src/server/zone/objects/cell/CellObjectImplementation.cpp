...
void CellObjectImplementation::onContainerLoaded() {
	...
	for (int j = 0; j < getContainerObjectsSize(); ++j) {
		...
		/*Infinity:  Let's check pre-2.0.0 vendors and turn off their AI and movement
		if (child != nullptr && child->isVendor() && child->isAiAgent()) {

			ManagedReference<AiAgent*> agent = child->asAiAgent();
			if (agent != nullptr && ((agent->getOptionsBitmask() & OptionBitmask::AIENABLED) || (agent->getMovementState() != AiAgent::OBLIVIOUS) || !(agent->getCreatureBitmask() & CreatureFlag::STATIONARY))) {			
				Logger::console.error("Fixed vendor : " + String::valueOf(agent->getObjectID()) + ", Name: " + agent->getDisplayedName() + ".");
				Locker locker(agent);
				agent->setOblivious();
				agent->wipeBlackboard();
				agent->clearOptionBit(OptionBitmask::AIENABLED, false);
				agent->setCreatureBitmask(CreatureFlag::STATIONARY);
			}
		}
		*/
	}
