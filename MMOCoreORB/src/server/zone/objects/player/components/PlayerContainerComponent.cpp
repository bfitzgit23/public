...
int PlayerContainerComponent::notifyObjectInserted(SceneObject* sceneObject, SceneObject* object) const {
	...
	if (object->isWeaponObject()) {
        ...
        if (weapon != nullptr && weapon->isOneHandMeleeWeapon()) {
            if (weapon->getObjectName()->getStringID() == "sword_02" || weapon->getObjectName()->getStringID() == "forge_hammer") {
                if (weapon->getArmorPiercing() == 0) {
                    Locker locker(weapon);
                    weapon->setArmorPiercing(1);
                    locker.release();
                    creo->sendSystemMessage("Your 1-handed weapon has been upgraded to Light armor piercing.");
                }
            }
        }
        ...
        if (weapon != nullptr && weapon->isUnarmedWeapon()) {
            if (weapon->getObjectName()->getStringID() == "knuckler_blasterfist") {
                if (weapon->getArmorPiercing() == 2) {
                    ...
                    weapon->setArmorPiercing(1);
                    locker.release();
                    creo->sendSystemMessage("Your Blasterfist has been changed to Light armor piercing.");
                }
            }
        }
    }
