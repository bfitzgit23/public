...
#include "server/zone/objects/mission/events/BhMissionTimeOutTask.h"
...
void PlayerObjectImplementation::initializeTransientMembers() {
	...
	miliSecsSession = 0;
	//Infinity:  Initialize Primary TEF
	primaryTef = NONE;
}
		...
		if (player != nullptr) {
			parent->executeOrderedTask([player] () {
					...
					if (ghost != nullptr) {
						...
						String checkComplete = ghost->getScreenPlayData("utils", "permissionGroupCheck");

						if (checkComplete != "true") { //Infinity:  Only going to run this once per player

							ghost->setScreenPlayData("utils", "permissionGroupCheck", "true");

							//Infinity:  Cleanup permissionGroups and verify Enclave permissions
							...
							if (groups != nullptr && groups->size() > 0) {	
								...
								for (int i = 0; i < groups->size(); ++i) {
									...
									while (ghost->hasPermissionGroup(group)) { //Infinity:  Remove duplicates
										ghost->removePermissionGroup(group, true);
									}

									//Infinity:  Check Enclave permission for switchers
									int jediState = ghost->getJediState();
									if ((jediState == 4 && group.beginsWith("DarkEnclave")) || (jediState == 8 && group.beginsWith("LightEnclave"))) {
										continue;
									}
									
									//Infinity:  Check Enclave permission for those who have dropped FRS
									if ((jediState < 4 && group.contains("Enclave")) || group.contains("EnclaveRank-1")) {
										continue;
									}

									//Infinity:  Fix themepark permissionGroups
									if (group.beginsWith("jabba_palace")) {
										String newGroup = "@tatooine_region_names:" + group;
										if (!ghost->hasPermissionGroup(newGroup)) {
											ghost->addPermissionGroup(newGroup, true);
										}
										continue;
									}

									if (group == "emperors_retreat2") {
										String newGroup = "@naboo_region_names:" + group;
										if (!ghost->hasPermissionGroup(newGroup)) {
											ghost->addPermissionGroup(newGroup, true);
										}
										continue;
									...
									if (group == "temple_of_exar_kun2") {
										String newGroup = "@yavin4_region_names:" + group;
										if (!ghost->hasPermissionGroup(newGroup)) {
											ghost->addPermissionGroup(newGroup, true);
										}
										continue;
									...
									ghost->addPermissionGroup(group, true);
								}
							}
						}

                        //Infinity:   Cleanup owned structure list

                        auto zoneServer = player->getZoneServer();

                        if (zoneServer != nullptr) {

                            for (int i = ghost->getTotalOwnedStructureCount() - 1; i >= 0; i--) {
                                ...
                                if (structure == nullptr) {
                                    Logger::console.error("Found and removed nullptr structure for player " + player->getFirstName() + ".  OID: " + String::valueOf(oid));
                                    ghost->removeOwnedStructureByID(oid);
                                }
                            }
                        }
					}
...
void PlayerObjectImplementation::unloadSpawnedChildren() {
	...
	Reference<StoreSpawnedChildrenTask*> task = new StoreSpawnedChildrenTask(creo, std::move(childrenToStore));
	task->execute();
}
...
int PlayerObjectImplementation::calculateBhReward() { //Infinity:  Heavily customized

	int minReward = 25000; 			// Minimum reward for a player bounty
	int maxSkillReward = 250000;

	int skillPoints = getSpentJediSkillPoints() + getNumSpecificSkills("force_sensitive");
	int reward = skillPoints * 1000;

	if (reward < minReward) {
		reward = minReward;
	}
	else if (reward > maxSkillReward) {
		reward = maxSkillReward;
	}

	ManagedReference<CreatureObject*> player = getParentRecursively(SceneObjectType::PLAYERCREATURE).castTo<CreatureObject*>();
	if (player != nullptr && player->hasSkill("mandalorian_master") && reward < 100000) {
		reward = 100000;
	}
...
void PlayerObjectImplementation::notifySceneReady() {
	...
	if (zone != nullptr  && creature->getFirstName().toLowerCase() != "discord") {  // Infinity:  Not Discord Bot
	...
	for (int i = chatRooms.size() - 1; i >= 0; i--) {
		...
		if (room != nullptr && creature->getFirstName().toLowerCase() != "discord") {    // Infinity:  Not Discord Bot
	...
	// Infinity:  Let's make Discord Chat Bot Join the Infinity Channel
	...
	if (ConfigManager::instance()->getBool("Core3.EnableHelperDroid", false))
		createHelperDroid();
}
...
int PlayerObjectImplementation::addExperience(TransactionLog& trx, const String& xpType, int xp, bool notifyClient) {
	...
	if (experienceList.contains(xpType)) {
		...
		if (xp <= 0 && xpType != "jedi_general") {
			...
			return 0;
		// Infinity:  4 million experience cap for Jedi experience loss
		}
...
void PlayerObjectImplementation::addFriend(const String& name, bool notifyClient) {
	...
	if (notifyClient && strongParent != nullptr) {
		...
		bool appearOffline = playerToAddGhost->getScreenPlayData("addfriend_appear_offline", "status") == "true";

		// other player is online and is not ignoring this player?
		if (playerToAdd->isOnline() && (isPrivileged() || (!appearOffline && !playerToAddGhost->isIgnoring(firstName)))) {
...
void PlayerObjectImplementation::notifyOnline() {
	...
	if (chatManager == nullptr || zoneServer == nullptr)
		return;

	String lowerFirstName = playerCreature->getFirstName();
	lowerFirstName = lowerFirstName.toLowerCase();

	//Infinity:  Credit logging
	auto playerManager = server->getPlayerManager();
	if (playerManager != nullptr) {
		StringBuffer msg;
		playerManager->writePlayerLog(playerCreature, msg.toString(), Logger::LogLevel::LOG);
	}

	ManagedReference<Account*> account = getAccount();

	String accountCreationDate = "Unknown";
	String accountName = "Unknown";

	if (account != nullptr) {
		Time createdTime(account->getTimeCreated());
		accountCreationDate = createdTime.getFormattedTime();
		accountName = account->getUsername();
	}

	//Infinity:  Detailed player logging, turned off for now
	//Logger::console.error("Player Logged In = " + playerCreature->getFirstName() + ", Account = " + accountName + ", AccountID = " + String::valueOf(getAccountID()) + ", Creation Date = " +  accountCreationDate + ", IP = " + sessionStatsIPAddress);
	
	//Infinity:  Checking for helps on test servers

	if (zoneServer->getGalaxyName() != "SWG Infinity") {

		SkillManager* skillManager = zoneServer->getSkillManager();

		if (playerManager != nullptr && skillManager != nullptr) {
			// Infinity dev admin access for TC
			if (lowerFirstName == "bbq" || lowerFirstName == "stepo" || lowerFirstName == "lafa" || lowerFirstName == "gingen" || lowerFirstName == "prada" || lowerFirstName == "volinor") {
					skillManager->addAbility(asPlayerObject(), "admin");
					playerManager->updatePermissionLevel(playerCreature, 15);
			}

			// Infinity tester access for TC
			if (lowerFirstName == "dahren") {
					skillManager->addAbility(asPlayerObject(), "admin");
					playerManager->updatePermissionLevel(playerCreature, 10);
			}
		}
	}
	...
	for (int i = 0; i < friendList.reversePlayerCount(); ++i) {
		...
		bool appearOffline = getScreenPlayData("addfriend_appear_offline", "status") == "true";

		if (otherGhost->isPrivileged() || (!appearOffline && !isIgnoring(otherName))) {
	...
	for (int i = 0; i < friendList.size(); ++i) {
		...
		bool appearOffline = otherGhost->getScreenPlayData("addfriend_appear_offline", "status") == "true";

		if (isPrivileged() || (!appearOffline && !otherGhost->isIgnoring(lowerFirstName))) {
	...
	if (missionManager != nullptr) {//Infinity:  Let's add them to the bounty list

		uint64 oid = playerCreature->getObjectID();

		if (!missionManager->hasPlayerBountyTargetInList(oid)) {
			if (playerCreature->hasSkill("force_title_jedi_rank_02") || getVisibility() > 0) {  //Infinity:  Add any Jedi or player with visibility if not already in list
				missionManager->addPlayerToBountyList(oid, calculateBhRewardWithExisting());
			}
		}
		else {
			missionManager->updatePlayerBountyReward(oid, calculateBhRewardWithExisting());
		}
		missionManager->updatePlayerBountyOnlineStatus(oid, true);	//Infinity:  If they aren't in bounty list after checks, it will be ignored
	}
	...
	*luaOnPlayerLoggedIn << playerCreature;
	...
	if (bank != nullptr) {
		...
		if (currentItemLimit < maxBankItemLimit)
			bank->setContainerVolumeLimit(maxBankItemLimit);
	}

    //Infinity:  Validate badges
    if (playerManager != nullptr) {
        playerManager->validateBadges(asPlayerObject(), true);
    }

	//Infinity:  Let's unequip uncertified and zero condition armor and clothing.  Also check invalid backpacks.
		...
		for (int i = 0; i < playerCreature->getSlottedObjectsSize(); ++i) {
			...
			if (object->isContainerObject()) {
				if (object->getObjectName()->getStringID().contains("backpack") && object->getContainerVolumeLimit() == 65) {
					int capacity = 50;
					Locker locker(object, asPlayerObject());
			...
			if (object->isWearableObject()) {
				...
				if (object->isBroken()) {    //Infinity: Let's remove any wearables with condition < 2 (Broken)
					objectController->transferObject(object, inventory, -1, true, true); // -1 is the containmentType to unequip items
					error("Removing destroyed wearable " + object->getObjectName()->getStringID() + " for player " + playerCreature->getDisplayedName());
					continue;
				}
				...
				if (skillsRequired.size() > 0) {   // We have a wearable item with certifications required
					...
					if (!hasSkill) { // Remove uncertified wearable item
						objectController->transferObject(object, inventory, -1, true, true); // -1 is the containmentType to unequip items
						error("Removing uncertified wearable " + object->getObjectName()->getStringID() + " for player " + playerCreature->getDisplayedName());
						itemsRemain  = true;
					}
				}
			}
	        ...
	        if (object->isWeaponObject()) {
                ...
                if (weapon != nullptr && weapon->isUnarmedWeapon()) {
                    if (weapon->getObjectName()->getStringID() == "knuckler_blasterfist") {
                        if (weapon->getArmorPiercing() == 2) {
                            Locker locker(weapon, asPlayerObject());
...
void PlayerObjectImplementation::notifyOffline(bool appearOnly) {
	debug("notifyOffline");
	...
	*luaOnPlayerLoggedOut << playerCreature;
	...
	auto zoneServer = playerCreature->getZoneServer();
	if (zoneServer != nullptr) {
		...
		if (missionManager != nullptr) {
			...
			if (playerCreature->hasSkill("combat_bountyhunter_investigation_03") || playerCreature->hasSkill("mandalorian_master")) {
				
				ManagedReference<MissionObject*> bhMission = missionManager->getBountyHunterMission(playerCreature);
				if (bhMission != nullptr) {
					BhMissionTimeoutTask* timeoutTask = new BhMissionTimeoutTask(bhMission);
					int64 scheduledCheck = 1000 * 60 * 10;  //10 minutes
					timeoutTask->schedule(scheduledCheck);
				}
			}
		}
	}
...
void PlayerObjectImplementation::logSessionStats(bool isSessionEnd) {
	...
	if (ServerCore::getSchemaVersion() >= 1003) {
		...
		query << "INSERT INTO `session_stats` ("
			...
			<< ", '" << sessionStatsIPAddress << "'"
			<< ", " << isSessionEnd
			<< ", " << (uint64)(miliSecsSession / 1000.0f)
			<< ", " << (uint64)(sessionStatsMiliSecs / 1000.0f)
			<< ", " << creditsDelta
			<< ", " << skillPointDelta
		...
		logMsg << "SessionStats:"
			...
			<< " sessionSeconds: " << (uint64)(miliSecsSession / 1000.0f)
			<< " logSeconds: " << (uint64)(sessionStatsMiliSecs / 1000.0f)
...
void PlayerObjectImplementation::doForceRegen() {
	...
	if (creature->isMeditating()) {
		...
		if (medTask != nullptr)
			modifier = 5;   //Infinity:  Custom change from 3 to 5
	}
...
void PlayerObjectImplementation::updateLastCombatActionTimestamp(bool updateGcwCrackdownAction, bool updateGcwAction, bool updateBhAction, bool healOnly) {
    ...
    bool alreadyHasPvpTef = hasPvpTef();    //Infinity:  for PVPTEFACTIvATED observer
	bool alreadyHasBhTef = hasBhTef();
	bool alreadyHasGcwTef = hasGcwTef();
	...
	if (updateGcwAction && !(healOnly && alreadyHasBhTef)) {
		lastGcwPvpCombatActionTimestamp.updateToCurrentTime();
		lastGcwPvpCombatActionTimestamp.addMiliTime(FactionManager::TEFTIMER);

		if (!parent->checkCooldownRecovery("shuttleImmunity")) {
			parent->updateCooldownTimer("shuttleImmunity", 0);  //Infinity:  Clear shuttle immunity
			if (parent->containsPendingTask("shuttle_immunity")) {
				parent->removePendingTask("shuttle_immunity");
			}
			parent->broadcastPvpStatusBitmask();
			parent->sendSystemMessage("Removing shuttle PVP immunity due to updated TEF.");
		}
		
		if (!alreadyHasBhTef) {	//Infinity:  Primary TEF
			primaryTef = GCWTEF;
		}
	}
	...
	if (updateBhAction) {
		...
		//Infinity:  Only notify observer for SL buffs if Primary TEF is not GCWTEF (BHTEF or NONE)
		if (!alreadyHasBhTef && primaryTef != GCWTEF) {
	...
	if (!alreadyHasPvpTef && (updateGcwAction || updateBhAction)) {
...
void PlayerObjectImplementation::recalculateForcePower() {
	...
	//Infinity:  We are going to recalculate force power max from skill boxes and wearable items	
	const SkillList* skillList = player->getSkillList();
	if (skillList == nullptr)
		return;

	unsigned int forcePowerMax = 0;

	for (int i = 0; i < skillList->size(); ++i) {
		Skill* skill = skillList->get(i);
		if (skill->getSkillName().beginsWith("force_")) {
			//Add skill modifiers
			...
			for (int j = 0; j < skillModifiers->size(); ++j) {
				...
				if (entry->getKey() == "jedi_force_power_max")
					forcePowerMax += entry->getValue();
			}
		}
	}

	forcePowerMax += player->getSkillModOfType("jedi_force_power_max", SkillModManager::WEARABLE);

	setForcePowerMax(forcePowerMax, true);
}
...
void PlayerObjectImplementation::revokeBadge(uint32 badge) {
    ...
    if (zoneServer != nullptr) {
        ...
        if (playerManager != nullptr) {
            playerManager->revokeBadge(asPlayerObject(), badge);
        }
    }   
}

int PlayerObjectImplementation::getNumSpecificSkills(const String& reqSkillName) {
...
int PlayerObjectImplementation::calculateBhRewardWithExisting() { //Infinity:  Custom

	int minReward = 25000; 	
	int maxReward = 2500000;

	int skillReward = calculateBhReward();
	int totalReward = skillReward;
	...
	int playerBountyReward = 0;
	int currentReward = missionManager->getPlayerBounty(player->getObjectID());

	if (currentReward > skillReward) {
		playerBountyReward = currentReward - skillReward;
		totalReward += playerBountyReward;
	}

	if (totalReward < minReward) {
		totalReward = minReward;
	}
	else if (totalReward > maxReward) {
