...
class Badges : public Serializable, public ReadWriteLock {
	uint32 badgeBitmask[10]; //Infinity: increase the number of bitmasks added to the code from 5 to 10 (change to the number you'll use)
	uint8 badgeTypeCounts[6];
	uint8 badgeTotal;
public:
	Badges() {
		for (int i = 0; i < 10; ++i)  //Infinity: increase the number of bitmasks added to the code from 5 to 10 (change to the number you'll use)
			badgeBitmask[i] = 0;
	...
	inline void addSerializableVariables() {
		...
		//Infinity: add additional bitmask (don't forget to add the total count in the commented section above)
		//you will also need to edit MMOCoreORB\src\server\zone\packets\player\BadgesResponseMessage.h
...
public:
	...
	friend void to_json(nlohmann::json& j, const Badges& b) {
		...
		for (int i = 0; i < 10; ++i) {  //Infinity:  5 to 10
	...
	void setBadge(const Badge* badge) {
		...
		if (bitmaskNumber > 9 || bitmaskNumber < 0) {   //Infinity:  4 to 9
	...
	void unsetBadge(const Badge* badge) {
		...
		if (badgeBitmask[bitmaskNumber] & value) {
			//badgeBitmask[bitmaskNumber] -= value;
            badgeBitmask[bitmaskNumber] &= ~value;  //Infinity:  Unset the bit using AND with NOT of value
			const int badgeType = badge->getTypeInt();

            if (badgeTypeCounts[badgeType] > 0) { //Infinity:  Prevent underflow in case of error conditions
                badgeTypeCounts[badgeType]--;
            }

            if (badgeTotal > 0) {   //Infinity:  Prevent underflow in case of error conditions
			    badgeTotal--;
            }
		}
	}
	...
	void setBitmask(int index, uint32 bitmask) {
		if (index > 9 || index < 0) {  //Infinity:  4 to 9
