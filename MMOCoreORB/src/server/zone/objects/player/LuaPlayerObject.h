...
namespace player {
	...
	public:
		...
		int updateLastCombatActionTimestamp(lua_State* L);
		int getAccountAgeInDays(lua_State* L);
		int getMaximumLots(lua_State* L);
		int setMaximumLots(lua_State* L);
		int storePets(lua_State* L);
		int getLotsRemaining(lua_State* L);
		int hasBhTef(lua_State* L);
		int isAdmin(lua_State* L);
		int hasActivePets(lua_State* L);
		int removeFromFrs(lua_State* L);
		int getServerMovementTimeDelta(lua_State* L);
		int getActivePetsSize(lua_State* L);
		int removeTefs(lua_State* L);
		int hasTef(lua_State* L);
		int clearActivePets(lua_State* L);

		//String getCallerInfo(lua_State* L);
