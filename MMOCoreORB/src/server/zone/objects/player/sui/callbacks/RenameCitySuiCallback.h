...
public:
	...
	void run(CreatureObject* creature, SuiBox* sui, uint32 eventIndex, Vector<UnicodeString>* args) {
		...
		int result = (ghost->isAdmin()) ? NameManagerResult::ACCEPTED : nameManager->validateCityName(cityName);
		...
		if (!ghost->isAdmin() && (!planetManager->validateRegionName(cityName) || !cityManager->validateCityName(cityName))) {
