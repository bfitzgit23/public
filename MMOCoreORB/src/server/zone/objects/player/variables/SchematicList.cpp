		...
		for(int i = 0; i < rewardedSchematics.size(); ++i) {
			if(rewardedSchematics.elementAt(i).getKey() == schematic) {
				if(rewardedSchematics.get(i) > 1) {
					...
					return true;
				} else if (schematic->getObjectTemplate()->getTemplateFileName().contains("lightsaber_pike_s1")) {
					Logger::console.error("Found lightsaber pike schematic with unlimited uses, deleting it.");
					removeRewardedSchematic(schematic);
					return true;					
				}
			}
