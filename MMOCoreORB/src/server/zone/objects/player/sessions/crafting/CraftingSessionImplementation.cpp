...
void CraftingSessionImplementation::initialAssembly(int clientCounter) {
	...
	for (int i = 0; i < colorVariables.size(); ++i) {
		...
		/* Infinity:  Allow players to color droid head
...
void CraftingSessionImplementation::startCreationTasks(int timer, bool practice) {
	...
	if (crafter == nullptr || craftingTool == nullptr)  //Infinity stability checks
		return;

	auto zoneServer = crafter->getZoneServer();   //Infinity:  Save and nullcheck
	if (zoneServer == nullptr)
		return;
	...
	//Infinity:  Instant crafting on test servers
