...
int DestroyStructureSessionImplementation::destroyStructure() {
	...
	if (structureObject->isGCWBase()) {
		...
		if (lua != nullptr) {
			Reference<LuaFunction*> luaObtainData = lua->createFunction("shovel", "redeed", 0);

			if (luaObtainData != nullptr) {
                *luaObtainData << creatureObject;
                *luaObtainData << structureObject;
                *luaObtainData << structureObject->getSurplusMaintenance();
