...
void EntertainingSessionImplementation::addHealingXpGroup(int xp) {
	...
	auto zoneServer = entertainer->getZoneServer();
	...
	for (int i = 0; i < group->getGroupSize(); ++i) {
		...
		if (groupMember != nullptr && groupMember->isPlayerCreature() && groupMember->hasSkill("social_entertainer_novice") && groupMember->isEntertaining() && groupMember->isInRange(entertainer, 40.0f)) {
		
			Core::getTaskManager()->executeTask([groupMember, playerManager, xp] () {

				if (groupMember == nullptr || playerManager == nullptr)
					return;
				
				playerManager->awardExperience(groupMember, "entertainer_healing", xp, true);

			}, "AddHealingXpGroupLambdaTask");
		}
	}
}
...
void EntertainingSessionImplementation::doPerformanceAction() {
	...
	//Infinity:  Going to reduce action cost for /startdance and /startmusic to be in line with flourish action cost
	...
	if (playerCreationManager != nullptr) {
		...
		if (quickness < 650 && maxQuickness < 650) { //Infinity:  Let's bump all races up to Rodian-level quickness at least
...
void EntertainingSessionImplementation::doPerformEffect(int effectId, int effectLevel) {
	...
	/* Infinity:  Let's allow entertainers to spam effects to burn action
...
void EntertainingSessionImplementation::doFlourish(int flourishNumber, bool grantXp) {
	...
	//Infinity:  Boosting quickness for low attribute races
	int quickness = entertainer->getHAM(CreatureAttribute::QUICKNESS);
	...
	//Infinity:  Custom action drain values
...
void EntertainingSessionImplementation::addEntertainerBuffStrength(CreatureObject* creature, int performanceType, float strength) {
	...
	//Infinity:  Custom city strength buff for ent buffs in relevant specializations
	float citySpecStrength = entertainer->getSkillMod("private_spec_buff_mind");	

	//Infinity:  Check for city ban
...
void EntertainingSessionImplementation::activateEntertainerBuff(CreatureObject* creature, int performanceType) {
	...
	try {
		...
		//Infinity:  This is all custom combined dancer/musician buffs
		...
		mindBuff->setBuffName("performance_enhance_dance_mind");
		creature->addBuff(mindBuff);
		mlocker.release();

		Locker flocker(focusBuff);
		focusBuff->setBuffName("performance_enhance_music_focus");
		creature->addBuff(focusBuff);
		flocker.release();

		Locker wlocker(willBuff);
		willBuff->setBuffName("performance_enhance_music_willpower");
