...
void CreatureObjectImplementation::addShockWounds(int shockToAdd, bool notifyClient, bool sendSpam) {
    ...
    //Infinity:  Let's calculate available shockwounds to add with 1000 cap.
    int shockAvailable = 1000 - shockWounds;

    if (shockToAdd > shockAvailable) {  //Infinity:  Negative shockToAdd (heal) will generate false condition, otherwise update shockToAdd
        shockToAdd = shockAvailable;
    }
	...
	if (sendSpam && asCreatureObject()->isPlayerCreature() && shockToAdd > 0)
...
int CreatureObjectImplementation::inflictDamage(TangibleObject* attacker, int damageType, float damage, bool destroy, bool notifyClient, bool isCombatAction) {
	...
	//Infinity:  Change AI to 80% damage reduction
	if (isPlayerCreature() && getSkillMod("avoid_incapacitation") > 0) {
			float damageReduction = 0.80;
			if (hasBuff(BuffCRC::JEDI_FORCEBREACH)) { //20% reduction if breached
				damageReduction *= 0.80f;
			}
...
void CreatureObjectImplementation::notifyLoadFromDatabase() {
	...
	//Infinity:  Moved this up, need to check buffs on pets too
...
void CreatureObjectImplementation::notifyPositionUpdate(QuadTreeEntry* entry) {
	if (entry == nullptr) // Infinity:  Extra check here
		return;
...
void CreatureObjectImplementation::feignDeath() {
	...
	buff->setSkillModifier("private_damage_multiplier_x100", 125); //Infinity: Multiply by 100 so we can get to 25% additional damage output
...
void CreatureObjectImplementation::setStunnedState(int durationSeconds) {
	...
	if (hasBuff(buffCRC)) {
		...
		multBuff->setSkillModifier("private_damage_divisor_stun_x100", 125);   //Infinity:   Looking for 1.25 divisor, so multiplying times 100 and dividing in Combat Manager
...
void CreatureObjectImplementation::setIntimidatedState(int durationSeconds) {
	...
	if (hasBuff(buffCRC)) {
		...
		multBuff->setSkillModifier("private_damage_divisor_intimidate", 2);
...
bool CreatureObjectImplementation::isAttackableBy(CreatureObject* creature, bool bypassDeadCheck) {
	...
	if (isPlayerCreature()) {
		...
		if ((creature->isPlayerCreature() || creature->isPet()) && !checkCooldownRecovery("shuttleImmunity")) {		//Infinity:  No PVP attacks after shuttling
			return false;
		}
		...
		if (creature->isPlayerCreature() && creature->isRidingMount()) {
			ManagedReference<CreatureObject*> parentCreo = creature->getParent().get().castTo<CreatureObject*>();

			/*
			if (parentCreo != nullptr) {

				//Infinity:   Saving for future debugging.  Changing isSwimming() to isInWater() becauses mounts don't get updated swimming state
				StringBuffer msg;
				msg << "Player " << creature->getFirstName() << " is riding mount " << parentCreo->getDisplayedName() << " and attacking player " << getFirstName() << endl;
				msg << "Mount is swimming?:  " << (parentCreo->isSwimming() ? "Yes" : "No") << endl;
				msg << "Mount is in water?:  " << (parentCreo->isInWater() ? "Yes" : "No");
			...
			*/
			...
			if (parentCreo != nullptr && parentCreo->isInWater()) {  //Infinity: Owner can't attack if riding mount and mount is swimming (isInWater)
				return false;
			}	
		}
		...
		if (creature->isAiAgent()) {
			...
			if (creature->isPet() && (agentCreo != nullptr && !agentCreo->isMindTricked())) {
				...
				ManagedReference<CreatureObject*> parentCreo = owner->getParent().get().castTo<CreatureObject*>();
				if (parentCreo != nullptr && parentCreo->isVehicleObject()) {  // Infinity:  Pets can't attack if owner is on vehicle
					return false;
				}

				if (isPlayerCreature() || isPet()) {  //PVP Pet checks

					if (!owner->isInRange(asCreatureObject(), 96.0f))	// Infinity:  Pets can't attack  if owner more than 96 meters from target
						return false;

					if (owner->isSwimming())	// Infinity:  Pets can't attack if owner is swimming
						return false;

					ManagedReference<BuildingObject*> rootParent = cast<BuildingObject*>(asCreatureObject()->getRootParent());   // Pet's can't attack in buildings owner can't enter
					if (rootParent != nullptr && (!rootParent->isAllowedEntry(owner)))
						return false;

					if (parentCreo != nullptr && parentCreo->isInWater())   //Infinity: Pet's can't attack if owner is mounted and mount is swimming (isInWater())
						return false;
				}
...
bool CreatureObjectImplementation::isHealableBy(CreatureObject* healerCreo) {
	...
	*/
	...
	if (thisIsPlayer) {
		...
		// This player has a BH TEF and cannot be healed, unless they have a primary GCW TEF which supercedes
		if (thisGhost != nullptr && thisGhost->hasBhTef() && thisGhost->getPrimaryTef() != PlayerObject::GCWTEF) { //Can heal the target if target haS GCW TEF that is primary
			return false;
		}
	}
...
bool CreatureObjectImplementation::hasEffectImmunity(uint8 effectType) const {
	...
	AiAgent* agent = const_cast<CreatureObjectImplementation*>(this)->asAiAgent();

	switch (effectType) {
	case CommandEffect::BLIND:

		if (agent != nullptr && (agent->getCreatureBitmask() & CreatureFlag::NOBLIND)) 
			return true;
	...
	case CommandEffect::DIZZY:
		...
		if (agent != nullptr && (agent->getCreatureBitmask() & CreatureFlag::NODIZZY)) 
			return true;
		if (isDroidSpecies() || const_cast<CreatureObjectImplementation*>(this)->isVehicleObject() || isWalkerSpecies())
			return true;
		break;
	...
	case CommandEffect::INTIMIDATE:
		...
		if (agent != nullptr && (agent->getCreatureBitmask() & CreatureFlag::NOINTIMIDATE)) 
			return true;
		if (isDroidSpecies() || const_cast<CreatureObjectImplementation*>(this)->isVehicleObject() || isWalkerSpecies())
			return true;
		break;
	...
	case CommandEffect::STUN:
		...
		if (agent != nullptr && (agent->getCreatureBitmask() & CreatureFlag::NOSTUN)) 
			return true;
		if (isDroidSpecies() || const_cast<CreatureObjectImplementation*>(this)->isVehicleObject() || isWalkerSpecies())
			return true;
		break;
	...
	case CommandEffect::KNOCKDOWN:
		if (agent != nullptr && (agent->getCreatureBitmask() & CreatureFlag::NOKNOCKDOWN)) 
			return true;

		if (const_cast<CreatureObjectImplementation*>(this)->isVehicleObject())			//Infinity:  Vehicles cannot be knocked down
			return true;
		break;

	case CommandEffect::POSTUREUP:
		if (const_cast<CreatureObjectImplementation*>(this)->isVehicleObject() || isWalkerSpecies() || isDroideka())
			return true;
		break;

	case CommandEffect::POSTUREDOWN:
		if (agent != nullptr && (agent->getCreatureBitmask() & CreatureFlag::NOPOSTUREDOWN)) 
			return true;
...
bool CreatureObjectImplementation::hasDotImmunity(uint32 dotType) const {
	...
	//Infinity:  DOT Immunity Options bitmask

	AiAgent* agent = const_cast<CreatureObjectImplementation*>(this)->asAiAgent();
	if (agent != nullptr && (agent->getCreatureBitmask() & CreatureFlag::NODOT)) 
			return true;
...
void CreatureObjectImplementation::removeOutOfRangeObjects() {
	...
	if (creature->isPlayerCreature()) {
		...
		if (ghost != nullptr && countCov > ghost->getCountMaxCov()) {
            ...
            auto zone = getZone();
	        error() << "Player: " << getDisplayedName() << " ID: " << getObjectID() << " Reached Max COV Count: " << countCov << " Zone: " << (zone != nullptr ? zone->getZoneName() : "null") << " World Position: " << getWorldPosition().toString();
...
// Infinity: Custom FRS mod functions
...
// Infinity:  Player armor check
bool CreatureObjectImplementation::isWearingArmor() {
	if (asCreatureObject() != nullptr && isPlayerCreature()) {
		for (int i = 0; i < getSlottedObjectsSize(); ++i) {
...
// Infinity:  Mark of the Hunter hunted level check
int CreatureObjectImplementation::getHuntedLevel(){
	int hunted = 0;

	if (asCreatureObject() == nullptr)
		return 0;

	//Locker locker(player);  MrObvious, locker should not be needed here, please verify
	if (hasBuff(BuffCRC::SKILL_BUFF_HUNTED1))
		hunted++;
	if (hasBuff(BuffCRC::SKILL_BUFF_HUNTED2))
		hunted++;
	if (hasBuff(BuffCRC::SKILL_BUFF_HUNTED3))
		hunted++;
	return hunted;
}

// Infinity:  docBuff functions below for docBot
int CreatureObjectImplementation::docBuff(CreatureObject* enhancer, uint16 attribute, bool checkCrates) {
	...
	if (enhancer->hasAttackDelay() || enhancer->hasPostureChangeDelay() || enhancer->isProne() || enhancer->isMeditating() || enhancer->isSwimming() || enhancer->isRidingMount())
	...
	ManagedReference<CityRegion* > region = enhancer->getCityRegion().get();
	if (region != nullptr) {
		if (region->isBanned(enhancer->getObjectID())) {
				enhancer->sendSystemMessage("You cannot operate a buffBot within a city if you are city banned.");
				return DOCINVALIDSTATE;
		}
	}
	...
	if (enhancePack == nullptr) {//No buffpack found, let's check crates
		/*
		if (checkCrates) { // Never check crates
			for (int i = 0; i < inventory->getContainerObjectsSize(); ++i) {
				ManagedReference<SceneObject*> object = inventory->getContainerObject(i);
				...
				if (prototype == nullptr || !prototype->isPharmaceuticalObject())
					continue;
				PharmaceuticalObject* pharma = cast<PharmaceuticalObject*>(prototype);
				if (pharma == nullptr)
					continue;
				if (pharma->isEnhancePack()) {
					EnhancePack* enhancePackTemp = cast<EnhancePack*>(pharma);
					if (enhancePackTemp != nullptr && enhancePackTemp->getMedicineUseRequired() <= medicineUse && enhancePackTemp->getAttribute() == attribute) {
						buffCrateFound = crate->extractObjectToInventory(enhancer);
						break;
					}
				}
			}

			if (buffCrateFound) { //Found a crate and extracted a buff pack, let's find it
				for (int i = 0; i < inventory->getContainerObjectsSize(); ++i) {
					ManagedReference<SceneObject*> object = inventory->getContainerObject(i);
					if (object == nullptr || !object->isPharmaceuticalObject())
						continue;
	...
	if (!enhancer->hasBuff(BuffCRC::FOOD_DISH_BIVOLI_TEMPARI))
		enhancer->eatBivoli(checkCrates);
...
void CreatureObjectImplementation::eatBivoli(bool checkCrates) {
	...
	//bool bivoliCrateFound = false;

	for (int i = 0; i < inventory->getContainerObjectsSize(); ++i) {
		ManagedReference<SceneObject*> object = inventory->getContainerObject(i);
		if (object != nullptr && object->getGameObjectType() == SceneObjectType::FOOD) {
			ManagedReference<Consumable*> consumable = cast<Consumable*>(object.get());
			if (consumable == nullptr)
				continue;
			String templateName = object->getObjectTemplate()->getTemplateFileName();
	...
	if (bivoli == nullptr) { //No Bivoli found, let's check crates
		/*
		if (checkCrates) {
			for (int i = 0; i < inventory->getContainerObjectsSize(); ++i) {
				ManagedReference<SceneObject*> object = inventory->getContainerObject(i);
				...
				if (crate == nullptr)
					continue;
				TangibleObject* prototype = crate->getPrototype();
				if (prototype == nullptr || prototype->getGameObjectType() != SceneObjectType::FOOD)
					continue;
				ManagedReference<Consumable*> consumable = cast<Consumable*>(prototype);
				if (consumable == nullptr)
					continue;
				String templateName = consumable->getObjectTemplate()->getTemplateFileName();
				if (templateName == "dish_bivoli_tempari") {
						bivoliCrateFound = crate->extractObjectToInventory(enhancer);
						break;
				}
			}

			if (bivoliCrateFound) { //Found a crate and extracted a buff pack, let's find it
				for (int i = 0; i < inventory->getContainerObjectsSize(); ++i) {
					ManagedReference<SceneObject*> object = inventory->getContainerObject(i);
					if (object != nullptr && object->getGameObjectType() == SceneObjectType::FOOD) {
						ManagedReference<Consumable*> consumable = cast<Consumable*>(object.get());
...
void CreatureObjectImplementation::removeSpeedBuffs() {

	removeBuff(STRING_HASHCODE("burstrun"));
	removeBuff(STRING_HASHCODE("retreat"));
	removeBuff(STRING_HASHCODE("dash")); 
	removeBuff(STRING_HASHCODE("rocketboots"));
	removeBuff(BuffCRC::JEDI_FORCE_RUN_1);
	removeBuff(BuffCRC::JEDI_FORCE_RUN_2);
	removeBuff(BuffCRC::JEDI_FORCE_RUN_3);
	removeBuff(STRING_HASHCODE("luaSetSpeed"));
}
