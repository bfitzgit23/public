...
void VehicleObjectImplementation::fillAttributeList(AttributeListMessage* alm, CreatureObject* object){
	...
	if (linkedCreature != nullptr) {
		ownerName = linkedCreature->getFirstName();
		alm->insertAttribute("@obj_attr_n:owner", ownerName);
	}


	/* Infinity Vehicle Debug
	if (object != nullptr) {
		...
		if (ghost != nullptr && ghost->isPrivileged()) {
			...
			StringBuffer msg;
			msg << endl << "Vehicle Debug Info" << endl;
			msg << "--------------------------------------------------------------------------" << endl;
			msg << " Name = " << getDisplayedName();
			msg << " OID = " << getObjectID() << endl;
			msg << " Owner Name = " << ownerName << endl;
			msg << " Posture = " << getPosture() << endl;
			msg << " accelerationMultiplierBase = " << getAccelerationMultiplierBase() << endl;
			msg << " accelerationMultiplierMod = " << getAccelerationMultiplierMod() << endl;
			msg << " speedMultiplierBase = " << getSpeedMultiplierBase() << endl;
			msg << " speedMultiplierMod = " << getSpeedMultiplierMod() << endl;
			msg << " currentSpeed = " << getCurrentSpeed() << endl;
			msg << " speedMultiplierModChanges = " << getSpeedMultiplierModChanges() << endl;
			msg << " speedModifier = " << getSpeedModifier() << endl;
			msg << " accelerationModifier = " << getAccelerationModifier() << endl;
			msg << " runSpeed = " << getRunSpeed() << endl;
			msg << " walkSpeed = " << getWalkSpeed() << endl;
			msg << " stateBitmask = 0x" << String::hexvalueOf(getStateBitmask()) << endl;
			msg << "--------------------------------------------------------------------------";
