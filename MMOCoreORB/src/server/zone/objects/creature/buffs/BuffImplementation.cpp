...
void BuffImplementation::loadBuffDurationEvent(CreatureObject* creo) {
	if(nextExecutionTime.getTime() - time(0) > buffDuration) {
		error("Buff timer was messed up.  Serialized Time:" + String::valueOf((int)(nextExecutionTime.getTime() - time(0))) + " Duration: " + String::valueOf(buffDuration));
...
void BuffImplementation::activate(bool applyModifiers) {
	...
	try {
		...
		//Infinity:  Lookup buff name based on CRC if buffName is not set in Lua or cpp
		if (buffName.isEmpty()) {
			String name = BuffCRC::getBuffNameFromCRC(buffCRC);
			if (name != "Unknown CRC") {
				buffName = name;
			}
		}
