...
CommandReference<CommandQueueAction*> CommandQueue::getNextAction() {
	...
	for (int i = 0; i < queueVector.size(); i++) {
		...
		if (queueCommand != nullptr) {
			...
			if (priority == QueueCommand::IMMEDIATE && creature->isPlayerCreature()) {
				auto ghost = creature->getPlayerObject();

				if (ghost != nullptr && ghost->hasGcwTef() && ghost->getScreenPlayData("monitor", "immediateCommands") == "true") {
					Logger::console.error("PVP Player "  + creature->getFirstName() + " used immediate command: " + queueCommand->getName() + ".");
				}
			}
...
int CommandQueue::handleRunningState() {
	...
	bool logMe = false;
	if (creature->isAiAgent()) { //Infinity: Disable for now
		ManagedReference<AiAgent*> agent = creature->asAiAgent();
		if (agent != nullptr) {
			String mobileName = agent->getCreatureTemplate()->getTemplateName();
			logMe = mobileName == "laen_pieweto" || mobileName == "nabek_naga";
		}
	}
	logMe = false;
	...
	if (logMe) info(true) << "Command Name: " << queueCommand->getName() << " with a priority of " << priority;
...
#endif
		if (logMe) info(true) << "Next normal action is future returning time of " << remainingActionTime;
	...
	if (logMe) info(true) << "Command Queue activateCommand called -- time = " << time << " ---- Removing Action";
	...
	if (creature->isAiAgent()) {
		if (priority == QueueCommand::NORMAL) {
			...
			//Infinity:  Going to get weaponSpeed from agent, and not assume global 2.0 speed
			ManagedReference<AiAgent*> agent = creature->asAiAgent();	
			if (agent != nullptr) {
				time = agent->getWeaponSpeed();
			}
			else {
				time = CreatureTemplateManager::instance()->getGlobalAttackSpeedOverride();
			}
	...
	if (logMe) info(true) << "Command Queue returning -- time = " << time << "";
...
void CommandQueue::enqueueCommand(unsigned int actionCRC, unsigned int actionCount, uint64 targetID, const UnicodeString& arguments, int priority, int compareCounter) {
	...
	/* Infinity:  Save for testing
	if (creature->isPlayerCreature() || creature->isPet()) {
		creature->info(true) << "Enqueue Command " << queueCommand->getQueueCommandName() << ": with priority = " << priority << ", actionCount = " << actionCount <<
			 ", Queue Size = " << size << ", isCombat = " << queueCommand->isCombatCommand() << ", addToCombatQueue = " << queueCommand->addToCombatQueue();
	}
	*/

	if (logMe) creature->info(true) << "Enqueue Command " << queueCommand->getQueueCommandName() << ": with priority = " << priority << ", actionCount = " << actionCount <<
			 ", Queue Size = " << size << ", isCombat = " << queueCommand->isCombatCommand() << ", addToCombatQueue = " << queueCommand->addToCombatQueue();
	...
	if (logMe) {
		...
		for (int i = 0; i < queueVector.size(); ++i) {
			...
			const QueueCommand* queuedCommand = objectController->getQueueCommand(command->getCommand());
			if (queuedCommand == nullptr)
				continue;

			creature->info(true) << "Command # " << i+1 << " = "  << queuedCommand->getQueueCommandName();
		}
	}

	//  Infinity:  Keep for further command queue testing
#ifdef DEBUG_QUEUE
	if (creature->isPet()) {
		creature->info(true) << "Enqueue Pet Command " << queueCommand->getQueueCommandName() << ": with priority = " << priority << ", actionCount = " << actionCount <<
			 ", Queue Size = " << size << ", isCombat = " << queueCommand->isCombatCommand() << ", addToCombatQueue = " << queueCommand->addToCombatQueue();
	...
	if (creature->isPlayerCreature()) {
		creature->info(true) << "Enqueue Player Command " << queueCommand->getQueueCommandName() << ": with priority = " << priority << ", actionCount = " << actionCount <<
				", Queue Size = " << size << ", isCombat = " << queueCommand->isCombatCommand() << ", addToCombatQueue = " << queueCommand->addToCombatQueue();
	...
	if (priority == QueueCommand::NORMAL) {
		if ((creature->isAiAgent() && size >= 5) || size >= 16) {
#ifdef DEBUG_QUEUE
			creature->info(true) << "Ignoring Queue Command " << queueCommand->getQueueCommandName() << ": with priority - " << priority << " Queue Size = " << size;
#endif
			if (logMe) creature->info(true) << "Ignoring Queue Command " << queueCommand->getQueueCommandName() << ": with priority - " << priority << " Queue Size = " << size;
	...
	if (priority == QueueCommand::FRONT) {
		...
		if (creature->isPet() && size >= 8) {  //Let's make sure we don't overload pet combat queue with FRONT commands

#ifdef DEBUG_QUEUE
			creature->info(true) << "Ignoring Pet Command " << queueCommand->getQueueCommandName() << ": with priority - " << priority << " Queue Size = " << size;
#endif
			clearQueueAction(actionCount, 0, 0, 0);
			return;
		}
...
#endif
	if (logMe) info(true) << "Enqueuing Command " << queueCommand->getQueueCommandName() << ": with priority - " << priority << " compareCount of " << compareCounter << " Queue Size = " << queueVector.size();
...
void CommandQueue::clearQueueActions(bool combatOnly) {
	...
	for (int i = queueVector.size() - 1; i >= 0; i--) {
		...
		if (combatOnly) {
			...
			uint32 nameCrc = queueCommand->getNameCRC();
			if (!queueCommand->isCombatCommand() && !(nameCrc == STRING_HASHCODE("petattack") || nameCrc == STRING_HASHCODE("petspecialattack")))  { //Infinity:  Be sure to clear these on peace
				continue;
			}
		}
