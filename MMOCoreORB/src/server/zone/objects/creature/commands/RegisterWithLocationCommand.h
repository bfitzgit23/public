...
public:
	...
	bool isInMedicalBuilding(CreatureObject* player, BuildingObject* building) const {
		const PlanetMapCategory* planetMapCategory = building->getPlanetMapCategory();

		if (planetMapCategory == nullptr)
			return false;

		String categoryName = planetMapCategory->getName();
	...
	bool isInEntertainingBuilding(CreatureObject* player, BuildingObject* building) const {
		...
		if (categoryName == "hotel" || categoryName == "cantina" || categoryName == "theater" || categoryName == "tavern")
			return true;

		if (categoryName == "guild") {

			const PlanetMapSubCategory* planetMapSubCategory = building->getPlanetMapSubCategory();

			if (planetMapCategory != nullptr) {
				if (planetMapSubCategory->getName() == "guild_theater") {
					return true;
				}
			}
		}
