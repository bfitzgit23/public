...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		bool isTargetOwner = (targetID == structureObject->getOwnerObjectID());
		...
		if (structureObject->isOnBanList(targetID)) {
			...
			*/
			...
			/*
			if (listName == "ENTRY") {  Infinity:  This is an unnecessary check and message
			...
			*/
			...
			/* Infinity:  This is an unnecessary check and message
			if (listName == "HOPPER") {
		...
		if (returnCode == StructurePermissionList::GRANTED) {

			ManagedReference<CreatureObject*> targetCreo = nullptr;
			ZoneServer* zoneServer = server->getZoneServer();
			if (zoneServer != nullptr) {
				targetCreo = zoneServer->getObject(targetID).castTo<CreatureObject*>();
			}

			if (listName == "ADMIN" && structureObject != nullptr && structureObject->isGuildHall() && targetCreo != nullptr) {

				uint32 granterAccountID = 0;
				uint32 targetAccountID = 0;
				uint32 ownerAccountID = 0;

				auto structureZone = structureObject->getZone();
				...
				ManagedReference<CreatureObject*> ownerCreo = structureObject->getOwnerCreatureObject();
				...
				if (ghost != nullptr) {
					granterAccountID = ghost->getAccountID();
				}

				auto targetGhost = targetCreo->getPlayerObject();
				if (targetGhost != nullptr) {
					targetAccountID = targetGhost->getAccountID();
				}

				StringBuffer msg;

				msg << ") added player " << targetCreo->getFirstName() << " (AcctID:" << targetAccountID;
				msg << ") to the admin list of a guildhall.  Owner: " << ownerName << " (AcctID:" << ownerAccountID;
				msg << ").  Zone: " << zoneName << ", coordinates (x=";
				msg << ceil(structureObject->getWorldPositionX()) << ", y=" << ceil(structureObject->getWorldPositionY()) << ", z=" << ceil(structureObject->getWorldPositionZ()) << ").";
