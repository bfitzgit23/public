			...
			if (object->isTangibleObject()) {
				...
				if (object->isWeaponObject()) {
					...
					if (weapon != nullptr && weapon->isEquipped()) {
						creature->sendSystemMessage("The weapon object you are attempting to delete is equipped.   You must unequip the weapon before deleting it.");
						return GENERALERROR;
					}
				}

				if (object->isWearableObject()) {
					ManagedReference<WearableObject*> wearable = cast<WearableObject*>(object.get());

					if (wearable != nullptr && wearable->isEquipped()) {
						creature->sendSystemMessage("The wearable object you are attempting to delete is equipped.   You must unequip the item before deleting it.");
						return GENERALERROR;
					}
				}
				...
				if (tano != nullptr && tano->hasAntiDecayKit()) {
