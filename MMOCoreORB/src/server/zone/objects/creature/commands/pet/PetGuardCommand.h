...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		Reference<CreatureObject*> owner = pet->getLinkedCreature().get();
		if (owner == nullptr)
			return GENERALERROR;

		// target is passed from PetManagerImplementation as the player giving the pet the command
		Reference<CreatureObject*> commander = owner;  //Infinity:  We will only guard the owner as befriend is disabled
		...
		// Infinity:  Owner is the only target that m
		...
		if (targetObject == nullptr) { //Infinity:  Check for null
			targetObject = commander;
		}

		if (targetObject != pet) {  //Infinity:  Only valid guard targets are pet or owner
			targetObject = commander;
		}
