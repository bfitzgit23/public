...
public:
	...
	int static doTransferItemMisc(CreatureObject* creature, SceneObject* objectToTransfer, SceneObject* destinationObject, int transferType, TransactionLog& trx, float maxDistance = 16.5f) { //Infinity:  Support /loot area command up to 24 meters
		...
		if (objectToTransfer->getObjectTemplate()->getTemplateFileName() == "eta1_ship_usable") {
			trx.abort() << "can't tranfer ETA-1 Star Traveler control device";
			return GENERALERROR;
		}
		...
		for (ManagedReference<SceneObject*> parent = objectToTransfer->getParent().get(); parent != nullptr; parent = parent->getParent().get()) {
