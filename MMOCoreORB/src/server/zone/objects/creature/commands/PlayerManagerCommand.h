...
public:
	static int executeCommand(CreatureObject* creature, uint64 target, const UnicodeString& arguments) {
		...
		if (command == "path") {
            ...
            if (chatManager == nullptr) {
                player->sendSystemMessage("ChatManager not found.");
                return 1;
             }
            ...
            if (playerMap == nullptr) {
                player->sendSystemMessage("PlayerMap not found.");
                return 1;
            }
            ...
            while (playerMap->hasNext(false)) {
                ...
                ManagedReference<PlayerObject*> ghost = player->getPlayerObject();
                if (ghost != nullptr && !ghost->isLinkDead() && !ghost->isOffline()) {

                    playerCount++;
                    uint64 accountID = ghost->getAccountID();

                    if (accountID != 0 && !accountIDs.contains(accountID)) {
                        accountIDs.add(accountID);
                    }
                }
			}

            chatManagerLocker.release();

			StringBuffer message;
			message << "Number of users online = " << playerCount << ",  number of unique accounts = " << accountIDs.size() << ".";
			player->sendSystemMessage(message.toString());
	...
	static void sendSyntax(CreatureObject* player) {
		if (player != nullptr) {
            ...
            player->sendSystemMessage("Syntax: /server playermanager [playercount]");
	...
	static String dumpCOV(ZoneServer* zoneServer, uint64 oid, bool showAll) {
		...
		ManagedReference<SceneObject*> targetObject = zoneServer->getObject(oid);
