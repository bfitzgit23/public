...
#include "server/zone/objects/creature/events/RemoveShuttleImmunityTask.h"
...
public:
	...
	const int MAXIMUM_POSITION_TRIES = 10;
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		for (;tries > 0; --tries) {
			...
			if (arrivalPlayerCity || (!arrivalPlayerCity && (fabs(position.getPositionZ() - arrivalPointZ) <= 1.6))) {//Infinity:  Only check delta Z for NPC travel points
				break;
			}
		}
#endif
		...
		if (tries <= 0) {
			//creature->info(true) << "BoardShuttleCommand: Failed to find suitable arrival point at "
		...
		int timer = 15;							//Infinity:  Non-Interplanetary, usually player shuttles
		if (arrivalPoint->isInterplanetary()) {
			timer = 15;							//Infinity:  Starports and interplanetary shuttles
		}

		creature->updateCooldownTimer("shuttleImmunity", timer * 1000); 

		if (creature->containsPendingTask("shuttle_immunity")) {
			creature->removePendingTask("shuttle_immunity");
		}

		Reference<RemoveShuttleImmunityTask*> task = new RemoveShuttleImmunityTask(creature, "shuttle_immunity");

		if (task != nullptr) {
			creature->addPendingTask("shuttle_immunity", task, (timer * 1000) + 250);
		}
...
private:
	...
	Coordinate findRandomizedArrivalPoint(CreatureObject* targetShuttleObject, PlanetManager* planetManager, Zone* arrivalZone, PlanetTravelPoint* arrivalPoint, int tries) const {
		...
		if (region == nullptr || region->isClientRegion()) {
			...
			// Set the Z checking for collision at the randomized point
			//position.setPositionZ(arrivalPoint->getArrivalPositionZ());  //This is vanilla, and broken
			float collisionZ = CollisionManager::getWorldFloorCollision(position.getPositionX(), position.getPositionY(), arrivalZone, false);
			position.setPositionZ(collisionZ);
			...
			// Infinity:   the proper location for arrival is along a 30 degree arc centered on the shuttle's facing axis, between 16.5 and 18.5 meters out
			int arc = System::random(30) - 15;
			dirDegrees -= arc;
			float dirRadians = dirDegrees * M_PI / 180;

			float distance = (abs(arc) <= 5 ? 16.50 : 16.55) + System::random(20)/10.f; //Infinity:  Custom calcs
