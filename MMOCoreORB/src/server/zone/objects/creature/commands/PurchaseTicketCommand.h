...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		ManagedReference<SceneObject*> travelTerminal = nullptr;

		for (int i = 0; i < closeObjects.size(); i++) {
			ManagedReference<SceneObject*> object = cast<SceneObject*>(closeObjects.get(i));

			if (object == nullptr || (object->getGameObjectType() != SceneObjectType::TRAVELTERMINAL) || !checkDistance(creature, object, 30.f)) {  //Infinity:  Extended range to 30 meters
				continue;
			}
		...
		// Infinity:   Checks on ticket purchase height
		if (nearTravelTerminal && creature->getParent() == nullptr) {   //Make sure they are not outside on top of a structure
		...
		auto departureZone = zoneServer->getZone(departurePlanet);
		auto arrivalZone = zoneServer->getZone(arrivalPlanet);

		if (departureZone == nullptr) {
			creature->sendSystemMessage("Departure zone (" + departurePlanet + ") does not exist.");
			return GENERALERROR;
		}

		if (arrivalZone == nullptr) {
			creature->sendSystemMessage("Arrival zone (" + arrivalPlanet + ") does not exist.");
			return GENERALERROR;
		}

		auto pmDeparture = departureZone->getPlanetManager();
		auto pmArrival = arrivalZone->getPlanetManager();

		if (pmDeparture == nullptr || pmArrival == nullptr) {
			return GENERALERROR;
		}
		...
		auto destPoint = pmArrival->getPlanetTravelPoint(arrivalPoint);

		if (destPoint == nullptr) {
			creature->sendSystemMessage("The given arrival point was not found.");
			return GENERALERROR;
		}

		ManagedReference<CreatureObject*> arrivalShuttle = destPoint->getShuttle();
		if (arrivalShuttle == nullptr) {
			creature->sendSystemMessage("The destination shuttle could not be found.");
			return GENERALERROR;
		}
		...
		if (!roundTrip) {
			...
			for (int i = 0; i < inventory->getContainerObjectsSize() - 1; i++) {
				ManagedReference<SceneObject*> sceneO = inventory->getContainerObject(i);
			...
			if (hasCoupon && creature->isPlayerCreature()) {
				...
				couponSui->setForceCloseDistance(30);
