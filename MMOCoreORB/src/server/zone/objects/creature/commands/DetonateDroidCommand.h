...
: QueueCommand(name, server) {
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (!creature->hasSkill("combat_bountyhunter_novice") && !creature->hasSkill("combat_smuggler_novice") && !creature->hasSkill("mandalorian_master")) {//Infinity:  Added Death Watch Initiate
