...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		//Infinity:  Added nullptr check
		if (targetCreature == nullptr || (targetCreature->isAiAgent() && !targetCreature->isPet()) || targetCreature->isDroidObject() || targetCreature->isVehicleObject() || 
				targetCreature->isDead() || targetCreature->isRidingMount() || targetCreature->isAttackableBy(creature)) {
			targetCreature = creature;	
			clocker.release();	//Infinity:   Player is the heal target, release the lock on the target Creature.  Put the lock before because isDead() could be changed by another thread
		}
