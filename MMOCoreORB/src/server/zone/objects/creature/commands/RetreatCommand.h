...
public:
	...
	bool checkRetreat(CreatureObject* creature) const {
		...
		uint32 dashCRC = STRING_HASHCODE("dash");
		uint32 rocketbootsCRC = STRING_HASHCODE("rocketboots");
		uint32 luaSetSpeedCRC = STRING_HASHCODE("luaSetSpeed");
		...
		if (creature->hasBuff(burstCRC) || creature->hasBuff(forceRun1CRC) || creature->hasBuff(forceRun2CRC) || creature->hasBuff(forceRun3CRC) || 
				creature->hasBuff(luaSetSpeedCRC) || creature->hasBuff(rocketbootsCRC) || creature->hasBuff(dashCRC)) {
