...
//Infinity:  Heavily modified, removed all references to EMU FRS Mods
...
public:
							...
							if (targetCreature != nullptr) {
								if (targetCreature->isPlayerCreature()) {
									...
									if (!CombatManager::instance()->areInDuel(creature, targetCreature) && !targetCreature->hasBountyMissionFor(creature) && !creature->hasBountyMissionFor(targetCreature) && targetCreature->getFactionStatus() == FactionStatus::OVERT) {
										ManagedReference<GuildObject*> targetGuild = targetCreature->getGuildObject();
										if (targetGuild == nullptr || !targetGuild->isInWaringGuild(creature)) {
									...
									if (targetOwner != nullptr && !creature->hasBountyMissionFor(targetOwner) && !targetOwner->hasBountyMissionFor(creature) && !CombatManager::instance()->areInDuel(creature, targetOwner) && targetOwner->getFactionStatus() == FactionStatus::OVERT) {
										ManagedReference<GuildObject*> targetGuild = targetOwner->getGuildObject();
										if (targetGuild == nullptr || !targetGuild->isInWaringGuild(creature)) {
											ghost->doFieldFactionChange(FactionStatus::OVERT);
										}
									}
	...
	// this goes in command in order to allow for overriding for special commands like Fortify
	// Infinity:  Added override duration argument 
	virtual void applyEffect(CreatureObject* attacker, CreatureObject* defender, uint64 effectType, uint32 mod, uint32 overrideDuration = 0) const {
		...
		targetDefense *= defender->getFrsMod("manipulation",1.25);  //FRS Mod adjustment

		uint32 stateLength = (overrideDuration == 0) ? effect.getStateLength() : overrideDuration;
		uint32 duration = (uint32) Math::max(5.f, stateLength*(1.f-targetDefense/120.f));
		...
		case CommandEffect::BLIND:
			...
			if (defender->isAiAgent()) {
				...
				if (defenderAgent != nullptr && (defenderAgent->getCreatureBitmask() & CreatureFlag::NOBLIND)) 
					break;
			}
		...
		case CommandEffect::DIZZY:
			...
			if (defender->isAiAgent()) {
				...
				if (defenderAgent != nullptr && (defenderAgent->getCreatureBitmask() & CreatureFlag::NODIZZY))    //Infinity:  Custom creature flag
					break;
			}
		...
		case CommandEffect::STUN:
			...
			if (defender->isAiAgent()) {
				...
				if (defenderAgent != nullptr && (defenderAgent->getCreatureBitmask() & CreatureFlag::NOSTUN)) //Infinity:  Custom creature flag
					break;
			}
		...
		case CommandEffect::KNOCKDOWN:
			...
			if (defender->isAiAgent()) {
				...
				if (defenderAgent != nullptr && (defenderAgent->getCreatureBitmask() & CreatureFlag::NOKNOCKDOWN))   //Infinity:  Custom creature flag
					break;
			}
			...
			if (!defender->checkKnockdownRecovery()) {
				if (defender->getPosture() != CreaturePosture::UPRIGHT) {
					...
					if (defender->isPlayerCreature()) {
						String name = attacker->getDisplayedName();
						name[0] = toupper(name[0]);
						defender->sendSystemMessage(name + "'s knockdown attack somehow lifts you to your feet.");
					}
				}
				break;
			}
			...
			if (defender->isPlayerCreature()) {	//Only check players
				delay /= defender->getFrsMod("manipulation", 0.36);    //Infinity:   FRS Max divisor = 1.463 = 5 to 3.418 secs on KD
				//defender->sendSystemMessage("TC: You are under a knockdown posture change delay which expires in " + String::format("%.1f", delay) + " seconds.");
			}

			defender->setPostureChangeDelay(delay);

			defender->removeBuff(STRING_HASHCODE("burstrun"));
			defender->removeBuff(STRING_HASHCODE("retreat"));
			defender->removeBuff(STRING_HASHCODE("dash"));  		//Infinity:  Cancel speed buff
			defender->removeBuff(STRING_HASHCODE("rocketboots"));    //Infinity:  Cancel speed buff
		...
		case CommandEffect::POSTUREUP:
			if (!defender->checkPostureUpRecovery()) {
				if (defender->getPosture() != CreaturePosture::UPRIGHT) {
					...
					if (defender->isPlayerCreature()) {
						String name = attacker->getDisplayedName();
						name[0] = toupper(name[0]);
						defender->sendSystemMessage(name + "'s posture up attack somehow lifts you to your feet.");
					}
				}
				break;
			}
			...
			/*
			if (defender->isPlayerCreature()) {	//Only notify players
				defender->sendSystemMessage("TC:  You are under a posture up posture change delay which expires in 2.5 seconds.");
			}
			*/
		...
		case CommandEffect::POSTUREDOWN:
			...
			if (defender->isAiAgent()) {
				...
				if (defenderAgent != nullptr && (defenderAgent->getCreatureBitmask() & CreatureFlag::NOPOSTUREDOWN))  //Infinity:  Custom creature flag
					break;
			}
			...
			if (!defender->checkPostureDownRecovery()) {
				if (defender->getPosture() != CreaturePosture::UPRIGHT) {
					...
					if (defender->isPlayerCreature()) {
						String name = attacker->getDisplayedName();
						name[0] = toupper(name[0]);
						defender->sendSystemMessage(name + "'s posture down attack somehow lifts you to your feet.");
					}
				}
				break;
			}
			...
			/*
			if (defender->isPlayerCreature()) {	//Only notify players
				defender->sendSystemMessage("TC:  You are under a posture down posture change delay which expires in 2.5 seconds.");
			}
			*/
