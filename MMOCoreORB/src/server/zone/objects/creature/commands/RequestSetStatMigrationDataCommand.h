...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		//Infinity:  Stat migrate anywhere as long as not in combat

		if (!creature->isInCombat()) {
			session->migrateStats();
		}
		else {
			creature->sendSystemMessage("@image_designer:not_in_combat"); // "You cannot perform stat migration while in combat.")
			return GENERALERROR;
		}
