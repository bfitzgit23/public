...
void AiAgentImplementation::respawn(Zone* zone, int level) {
	...
	primaryWeaponCrc = 0;										//Clear weapon CRCs to spawn new random weapon each time
	secondaryWeaponCrc = 0;										//Clear weapon CRCs to spawn new random weapon each time
	destroyAllWeapons();										//Destroy all weapons just to be sure

    unsigned int originalCreatureBitmask = getCreatureBitmask();   //Infinity:  save original bitmasks
	unsigned int originalPvpMask = getPvpStatusBitmask();  

	ManagedReference<Creature*> creature = cast<Creature*>(asAiAgent());  //Infinity:  This is used in baby check and resetting transient members
	...
	if (npcTemplate != nullptr && creature != nullptr) {  //Infinity:  Want to have babies on static spawns also, so moving home nullptr check

		int chance = 1000;  //Infinity:  2000 to 1000 for better chance at static spawns
		int babiesSpawned = 0;

		if (home != nullptr) {//Infinity:  moved this check inside, so babies can spawn at static spawns
		...
		if ((creatureManager != nullptr && (creatureManager->checkSpawnAsBaby(npcTemplate->getTame(), babiesSpawned, chance) && 
			!(originalCreatureBitmask & CreatureFlag::ADULTONLY))) || (originalCreatureBitmask & CreatureFlag::BABYONLY)) {
			...
			if (originalCreatureBitmask & CreatureFlag::BABYONLY) {
				addCreatureFlag(CreatureFlag::BABYONLY);
			}
	...
	//Infinity:  Reload all of the agents info as adult (babies are handled separately).  also creates weapons for the agent
	if (!babySpawned) {

		reloadTemplate();

		if (originalCreatureBitmask & CreatureFlag::ADULTONLY) {
			addCreatureFlag(CreatureFlag::ADULTONLY);
		}

		if (!(originalCreatureBitmask & CreatureFlag::BABY)) {	//Only match PVP bitmask if last spawn was adult
			setPvpStatusBitmask(originalPvpMask);
		}
	}

	//Infinity:  Check for special creature flags
	if (originalCreatureBitmask & CreatureFlag::STATIONARY) {
		addCreatureFlag(CreatureFlag::STATIONARY);
	}

	if (originalCreatureBitmask & CreatureFlag::STATIC) {
		addCreatureFlag(CreatureFlag::STATIC);
	}

	if (originalCreatureBitmask & CreatureFlag::NOAIAGGRO) {
		addCreatureFlag(CreatureFlag::NOAIAGGRO);
	}

	setAITemplate();											//Not sure this is needed
	...
	// Infinity:  Let's reset the creature, make sure all DOTs, buffs, and wounds are cleared
	clearDots();
	...
	//Infinity:  Reset scout states (harvesting, DNA sampling, milking)
	if (creature != nullptr) {									
		creature->resetCreatureStates();
	}
...
float AiAgentImplementation::calculateAttackSpeed(int level) {
	// Infinity:  2.95f base
	float speed = 2.95f - ((float)level / 100.f);
	return speed;
}
...
void AiAgentImplementation::setupCombatStats() {
	...
	float globalSpeedOverride = CreatureTemplateManager::instance()->getGlobalAttackSpeedOverride();  //Infinity:  This is set in creature_manager.lua
	float customSpeed = npcTemplate->getAttackSpeed();

	if (globalSpeedOverride > 0.0f)	
		weaponSpeed = globalSpeedOverride;	
	
	if (customSpeed > 0.0f)						//Infinity:  We are going to allow override in lua for each creature template
...
void AiAgentImplementation::createWeaponsFromTemplate() {
	...
	/*
	if (npcTemplate->getTemplateName().contains("gaping_spider_recluse_queen") || npcTemplate->getTemplateName().contains("azure_belsavis_varactyl")) {

		StringBuffer msg;
		msg << "createWeaponFromTemplate - Template: " << npcTemplate->getTemplateName() << ", noneHash: 0x" << String::hexvalueOf(STRING_HASHCODE("none"));
		msg << ", unarmedHash: 0x" << String::hexvalueOf(STRING_HASHCODE("unarmed")) << endl;
		msg << ", primaryWeaponCrc: " << String::hexvalueOf(primaryWeaponCrc) << ", secondaryWeaponCrc: " << String::hexvalueOf(secondaryWeaponCrc);
...
void AiAgentImplementation::setupAttackMaps() {
	...
	if (petDeed != nullptr)								//Infinity:  Setup secondary attacks since we enabled secondary weapons.  Duplicating petDeed check for clarity
		attackMap = petDeed->getAttacks();
	else
	...
	for (int i = 0; i < attackMap->size(); i++) {
		...
		if (defaultWeap != nullptr && (attack->getWeaponType() & defaultWeap->getWeaponBitmask()) && !defaultAttackMap->containsCommand(attackMap->get(i).getCommand())) { //Infinity:  Don't add to default map twice
			defaultAttackMap->add(attackMap->get(i));
		}
	}
...
int AiAgentImplementation::calculateAttackMinDamage(int level) { // Infinity:  Heavily modified function
	...
	int minDmg = 5;
	if (petDeed != nullptr) { // BE Crafted Pet
		minDmg = petDeed->getMinDamage();
		if (level < petDeed->getLevel()) {
			// reduce by level range for baby
		...
		if (creatureTemplate == nullptr) {
			error("Creature template was null.");
			return 5;
		}
		minDmg = calculateAttackMaxDamage(level) - 10;
	}

	return minDmg;
}

int AiAgentImplementation::calculateAttackMaxDamage(int level) {  // Infinity:  Heavily modified function
	...
	if (petDeed != nullptr) { // BE Crafted Pet
		...
		int templateMaxDamage = creatureTemplate->getDamageMax();
		maxDmg = Math::max((float)templateMaxDamage, (50.0f + level * 14));
	}
...
void AiAgentImplementation::setDefender(SceneObject* defender) {
    ...
    setTargetID(defender->getObjectID(), true);  //Infinity:  Update the creo target ID with the new target
...
bool AiAgentImplementation::killPlayer(SceneObject* prospect) {
	...
	//Infinity:  A few tweaks here, AiAgent and prospect are prelocked

	if (prospect == nullptr || !prospect->isCreatureObject() || !prospect->isPlayerCreature()) {
		setFollowObject(nullptr);
		return false;
	}
	...
	if (prospect->isInRange(asAiAgent(), 6.f)) {
		...
		ManagedReference<CreatureObject*> prospectCreo = prospect->asCreatureObject();
		
		if (prospectCreo != nullptr && !prospectCreo->isDead()) { //Infinity:  Make sure they aren't already dead
			...
			playerManager->killPlayer(asCreatureObject(), prospectCreo, 0, false);
			return true;
		}
	}
...
void AiAgentImplementation::notifyDespawn(Zone* zone, bool forceNoRespawn) {
#ifdef DEBUG_AI_WEAPONS
	...
	if (!forceNoRespawn && respawnTimer > 0) {
        ...
        if (zone == nullptr) {
            error() << "AiAgent failed to create a RespawnCreature task due to nullptr zone." << __FILE__ << ":" << __LINE__ << ":" << __FUNCTION__ << ": " << *_this.getReferenceUnsafeStaticCast();
            return;
        }
...
int AiAgentImplementation::setDestination() {
	...
	case AiAgent::FOLLOWING: {
		...
		//Infinity: Make baby creatures leash earlier/closer to lair after failed taming or aggro
		if (!isPet() && isCreature() && followCopy->isPlayerCreature() && isInCombat()) {

			ManagedReference<Creature*> creature = cast<Creature*>(asAiAgent());
			if (creature != nullptr && creature->isBaby() && !homeLocation.isInRange(asAiAgent(), AiAgent::MAX_OOS_RANGE)) {
				leash();
...
//#define DEBUG_CAMOUFLAGE
	...
	if (concealed) {
		...
		camoDebug << "Concealed = true. Conceal Mod = " << concealMod << ". " << endl;

		mod = concealMod / 1.4044f;			//Should be 89 with 125 camo mod
	}
	else {

		maskscentMod = effectiveTarget->getSkillMod("mask_scent");
			
		camoDebug << "Concealed = false. Maskscent Mod = " << maskscentMod << ". " << endl;

		mod = maskscentMod / 1.4367f;		//Should be 87 with 125 mask_scent mod
	}

	camoDebug << " Adjusted Mod = " << mod << ". Using " << ((concealed) ? "concealMod." : "maskscentMod.") << endl;

	if (mod < 5)
		return false;

	// Player is masked or concealed, do not roll chance to break on non-aggressive Mobs
	if (!isAggressiveTo(effectiveTarget))  //Infinity:  Only check isAggressiveTo, not isStalker
		return true;

	/* Infinity:  using playerLevel or groupLevel makes no sense here
	...
	*/
	...
	camoDebug << " Adjusted Mod = " << mod << ".  Creature Level = " << creatureLevel << "." << endl;

	if ((mod - 60 - creatureLevel) >= 10) {     //125 Mask scent won't break if creature under CL 17, and 125 camo won't break if CL under CL19

		camoDebug << " (Adjusted mod - 60 - creatureLevel >= 10).  Your camo is too good for a low level creature like that.   Returning true.";
#ifdef DEBUG_CAMOUFLAGE
		effectiveTarget->sendSystemMessage(camoDebug.toString());
#endif
		camouflagedObjects.put(effectiveTargetID);		//Put them in vector so they aren't checked every time
		return true;
	}

	//Mobs are not going to see/smell you from over 96 meters away
	if (!effectiveTarget->isInRange(asAiAgent(), 96.0f)) {

#ifdef DEBUG_CAMOUFLAGE
		camoDebug << "Agent is over 80 meters away from player, returning camo = true";
		effectiveTarget->sendSystemMessage(camoDebug.toString());
#endif
		return true;
	}

	float roll = mod - (creatureLevel / 100.0f);  //Infinity

	camoDebug << " Initial Roll = " << roll << ". " << endl;

	if (effectiveTarget->isProne()) { //Infinity:  15 -> 10
		roll += 10;
	} else if (effectiveTarget->isRunning() || effectiveTarget->isRidingMount()) {
		roll -= 15;
	...
	camoDebug << "Roll after posture calcs = " << roll << ". " << endl;
	...
	camoDebug << "Roll after caps = " << roll << ". " << endl;
...
int AiAgentImplementation::inflictDamage(TangibleObject* attacker, int damageType, float damage, bool destroy, bool notifyClient, bool isCombatAction) {
	...
	if (damage > 0) {
		...
		// Infinity:  DOTs should absolutely count toward loot rights
		getThreatMap()->addDamage(attacker, damage);
	}
...
bool AiAgentImplementation::sendConversationStartTo(SceneObject* player) {
    ...
    // Target Player
    setTargetID(player->getObjectID(), true);  //Infinity:  Update the creo target ID with the new target
...
float AiAgentImplementation::getEffectiveResist() {
	...
	if (getElectricity() > 0) {
		totalResist += getElectricity();
		count++;
	}
	if (getEnergy() > 0) {
		totalResist += getEnergy();
		count++;
	}
	if (getHeat() > 0) {
		totalResist += getHeat();
	...
	if (getLightSaber() > 0) {
		totalResist += getLightSaber() / 35.0f * .90;  //Infinity: Custom calc for pets with LS resist
		count++;
	}

	return count > 0 ? totalResist / count : 0;
}
...
float AiAgentImplementation::getReducedResist(float value) {
	...
	int totalCurrentHAM = 0;
	int totalMaxHAM = 0;
	int i = 0;

	// Get total of max HAM pools
	while (i <= 6) {
		totalCurrentHAM += getHAM(i) - getWounds(i);   //Going to include wounds as "damage"
		totalMaxHAM += getMaxHAM(i);
		i += 3;
	}

#ifdef DEBUG_RESIST_DECAY

	int healthWounds = getWounds(0);
	int actionWounds = getWounds(3);
	int mindWounds = getWounds(6);

	StringBuffer msg;
	msg << endl << "=== DEBUG RESIST DECAY ===" << endl;
	msg << " Total Max HAM: " << totalMaxHAM << endl;
	msg << " Total Current HAM: " << totalCurrentHAM << endl;
	msg << " Total HAM Wounds: " << healthWounds + actionWounds + mindWounds << endl;
	msg << "   Health Wounds: " << healthWounds << endl;
	msg << "   Action Wounds: " << actionWounds << endl;
	msg << "   Mind Wounds: " << mindWounds << endl;
#endif

	//Infinity:  Current HAM percentage of max
	float percentHAM = totalCurrentHAM / (float)totalMaxHAM;

#ifdef DEBUG_RESIST_DECAY
	msg << " Percent HAM: " << percentHAM << endl;
#endif

	// Decay resists when total HAM percentage is less than 75%
	if (percentHAM < 0.75f) {  //Infinity: 
		// Reduce resists 1% for every 1% of percentage HAM below 75%, down to 25%
		// Reduction Range is from 75% to 25% of totalHAM. totaling a max 50% reduction of resists

		float reduction = Math::min((0.75f - percentHAM), 0.50f);

		newValue = value * (1.0f - reduction);

#ifdef DEBUG_RESIST_DECAY
		msg << "getReducedResist: Percent HAM: " << percentHAM << " Starting Resist: " << value << " New Resist: " << newValue << " Reduction percent: " << reduction;
#endif
	}
		...
		if (zoneServer != nullptr) {
			auto playerManager = zoneServer->getPlayerManager();
			if (playerManager != nullptr) {
				auto playerCreature = playerManager->getPlayer("MrObvious");
				if (playerCreature != nullptr) {
					playerCreature->sendSystemMessage(msg.toString());
				}
			}
		}
#endif
