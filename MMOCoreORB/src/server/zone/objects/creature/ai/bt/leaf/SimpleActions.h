...
public:
	...
	Behavior::Status execute(AiAgent* agent, unsigned int startIdx = 0) const {
		...
		if (weapon != nullptr && weapon->getAttackType() ==  SharedWeaponObjectTemplate::FORCEATTACK) {
			//agent->info("SelectAttack::execute is FORCEATTACK", true);
			return agent->selectSpecialAttack(-1) ? SUCCESS : FAILURE;
		}

		int specialPercentage = agent->getSpecialPercentage();

		if (specialPercentage == 0) {
			specialPercentage = AiAgent::DEFAULT_SPECIAL_PERCENTAGE;
		}

		if (System::random(99) + 1 <= specialPercentage) {
		...
		//agent->info("SelectAttack::execute has attackType attackNum = " + String::valueOf(attackNum), true);
...
public:
	...
	Behavior::Status execute(AiAgent* agent, unsigned int startIdx = 0) const {
		...
		//agent->info("Behavior::Status execute, res = " + String::valueOf(res), true);
