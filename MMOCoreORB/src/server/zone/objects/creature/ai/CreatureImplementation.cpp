...
void CreatureImplementation::initializeTransientMembers() {
	resetCreatureStates();	//Infinity:  Reset scout states (harvesting, DNA sampling, milking)
...
void CreatureImplementation::fillAttributeList(AttributeListMessage* alm, CreatureObject* player) {
	...
	if (creaKnowledge >= 60) {
		...
		//Infinity:   Removed creatureAreaAttack = unknown attack
...
bool CreatureImplementation::canHarvestMe(CreatureObject* player) {
	...
	float adjustedRange = 7.0f + Math::min(_this.getReferenceUnsafeStaticCast()->getTemplateRadius() / 1.5f, 17.0f);

	if(!player->isInRange(_this.getReferenceUnsafeStaticCast(), adjustedRange) || player->isInCombat() || !player->hasSkill("outdoors_scout_novice")
...
bool CreatureImplementation::canMilkMe(CreatureObject* player) {
	...
	if (!_this.getReferenceUnsafeStaticCast()->isAttackableBy(player)) //Infinity:  If you can't attack it, you can't milk it
		return false;

	if(!player->isInRange(_this.getReferenceUnsafeStaticCast(), 9.0f) || player->isInCombat() || player->isDead() || player->isIncapacitated() || !(player->hasState(CreatureState::MASKSCENT)))
		return false;
...
bool CreatureImplementation::hasSkillToSampleMe(CreatureObject* player) {
	...
	// Infinity:  Can sample up to CL 350 if Master BE
	if (skillMod < 1 || (cl > 75 && !player->hasSkill("outdoors_bio_engineer_master")) || (cl > 350))
		return false;
...
bool CreatureImplementation::canCollectDna(CreatureObject* player) {
	...
	// Infinity:  Custom checks here moved from sampleDNA and tasks
	if (_this.getReferenceUnsafeStaticCast()->getLevel() > 350  || _this.getReferenceUnsafeStaticCast()->isBaby()) {
		return false;
	}
...
void CreatureImplementation::loadTemplateDataForBaby(CreatureTemplate* templateData) {
	...
	addCreatureFlag(CreatureFlag::BABY);
...
void CreatureImplementation::setPetLevel(int newLevel) {
	...
	//Infinity:  Let's update he minDamage and maxDamage on the AI Agent
	minDamage = minDmg;
	maxDamage = maxDmg;
...
void CreatureImplementation::resetCreatureStates() {
	alreadyHarvested.removeAll();
	milkState = CreatureManager::NOTMILKED;
	dnaState = CreatureManager::HASDNA;
	dnaSampleCount = 0;
