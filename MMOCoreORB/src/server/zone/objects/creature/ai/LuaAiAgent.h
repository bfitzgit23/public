...
namespace ai {
	...
	public:
		...
		int getPatrolPointSize(lua_State* L);  //Infinity:  Removed S
		...
		int isBaby(lua_State* L);
		int getHomeLocationX(lua_State* L);
		int getHomeLocationY(lua_State* L);
		int getHomeLocationZ(lua_State* L);
		int getHomeLocationCellID(lua_State* L);
		int getHomeLocationDirection(lua_State* L);
	private:
		AiAgent* realObject;
	};
}
