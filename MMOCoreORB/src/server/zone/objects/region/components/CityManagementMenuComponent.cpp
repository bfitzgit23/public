...
void CityManagementMenuComponent::fillObjectMenuResponse(SceneObject* sceneObject, ObjectMenuResponse* menuResponse, CreatureObject* player) const {
    ...
    if (city->isMayor(player->getObjectID()) || city->isMilitiaMember(player->getObjectID()) ||  ghost->isStaff()) {//Infinity:  Banned Player Report
		menuResponse->addRadialMenuItemToRadialID(211, 231, 3, "Banned Player Report"); //Banned Player Report
	}
    ...
    */
	...
	menuResponse->addRadialMenuItemToRadialID(216, 232, 3, "@city/city:citypardon_all"); //City Pardon All City Banned Players
...
int CityManagementMenuComponent::handleObjectMenuSelect(SceneObject* sceneObject, CreatureObject* player, byte selectID) const {
	...
	case 226: //Toggle Zoning Enabled
		...
		break;
	case 231: //Banned Player Report
		cityManager->sendBannedReport(city, player, sceneObject);
		break;
	case 232: { //City pardon all city banned players
		Lua* lua = DirectorManager::instance()->getLuaInstance();
		Reference<LuaFunction*> pardonAllCityBannedPlayersExecute = lua->createFunction("pardonAllCityBannedPlayers", "execute", 0);
		*pardonAllCityBannedPlayersExecute << player;
		*pardonAllCityBannedPlayersExecute << sceneObject;
		pardonAllCityBannedPlayersExecute->callFunction();
		break;
	}
