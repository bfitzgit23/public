...
void CityVotingMenuComponent::fillObjectMenuResponse(SceneObject* sceneObject, ObjectMenuResponse* menuResponse, CreatureObject* player) const {
	...
	if (!city->isCandidate(player->getObjectID())) {
		...
		int hoursUntilUpdate = nextUpdateTime->miliDifference() / -3600000.f;  //Infinity:  Hours

		int deadline = ConfigManager::instance()->getInt("Core3.CityManager.MayoralRaceDeadlineInHours", 0); //Infinity:  Default behavior is no deadline

		if (hoursUntilUpdate > deadline) {
