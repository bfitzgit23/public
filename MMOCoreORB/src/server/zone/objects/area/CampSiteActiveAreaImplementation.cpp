...
void CampSiteActiveAreaImplementation::assumeOwnership(CreatureObject* player) {
	...
	CloseObjectsVector* closeObjectsVector = camp->getCloseObjects();
	SortedVector<ManagedReference<QuadTreeEntry*> > closeObjects;

	auto campRadius = campStructureData->getRadius();

	if (closeObjectsVector != nullptr) {
		closeObjectsVector->safeCopyReceiversTo(closeObjects, CloseObjectsVector::PLAYERTYPE);
	} else {
		zone->getInRangeObjects(camp->getWorldPositionX(), camp->getWorldPositionY(), campRadius, &closeObjects, true);
	}
	...
	for (int i = 0; i < closeObjects.size(); ++i) {
		...
		if (sceno == nullptr || !sceno->isPlayerCreature() || (closeObjectsVector != nullptr && !sceno->isInRange(camp, campRadius))) {
			continue;
		}

		visitors.add(sceno->getObjectID());
	}
