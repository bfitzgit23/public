...
void PetControlDeviceImplementation::callObject(CreatureObject* player) {
	...
	// Infinity:  combat check altered for Master CH
	...
	/* Infinity:  Can now call pets anywhere including private buildings
	...
	*/
	...
	if (ghost == nullptr || pet == nullptr || ghost->hasActivePet(pet))
		return;
	...
	//Infinity:  Let's update min/max damage on Agent for older pets
	int currentMin = pet->getDamageMin();
	int currentMax = pet->getDamageMax();

	if (pet->getMinDamage() < currentMin) {
		pet->setMinDamage(currentMin);
	}

	if (pet->getMaxDamage() < currentMax) {
		pet->setMaxDamage(currentMax);
	}

	/* Infinity: Pet weapon debug to save for later
	StringBuffer msg;
	msg << "----- Debug Pet weapons -----" << endl;

	if (pet->getCurrentWeapon() != nullptr) {
		msg << "Current Weapon: " << pet->getCurrentWeapon()->getObjectTemplate()->getTemplateFileName() << ", OID: " << pet->getCurrentWeapon()->getObjectID() << endl;
		msg << "Min Damage: " << pet->getCurrentWeapon()->getMinDamage() << endl;
		msg << "Max Damage: " << pet->getCurrentWeapon()->getMaxDamage() << endl << endl;
	}

	if (pet->getDefaultWeapon() != nullptr) {
		msg << "Default Weapon: " << pet->getDefaultWeapon()->getObjectTemplate()->getTemplateFileName() << ", OID: " << pet->getDefaultWeapon()->getObjectID() << endl;
		msg << "Min Damage: " << pet->getDefaultWeapon()->getMinDamage() << endl;
		msg << "Max Damage: " << pet->getDefaultWeapon()->getMaxDamage() << endl;;

		const CreatureAttackMap* attackMap = pet->getDefaultAttackMap();
		if (attackMap != nullptr && attackMap->size() > 0) {

			msg << "Default Weapon Attack Map" << endl;
			for (int i = 0; i < attackMap->size(); i++) {
				...
				msg << "  Attack #" << i+1 << ":  " << formattedName << endl;
			}
		}
		msg << endl;
	}

	if (pet->getPrimaryWeapon() != nullptr) {
		msg << "Primary Weapon: " << pet->getPrimaryWeapon()->getObjectTemplate()->getTemplateFileName() << ", OID: " << pet->getPrimaryWeapon()->getObjectID() << endl;
		msg << "Min Damage: " << pet->getPrimaryWeapon()->getMinDamage() << endl;
		msg << "Max Damage: " << pet->getPrimaryWeapon()->getMaxDamage() << endl;

		const CreatureAttackMap* attackMap = pet->getPrimaryAttackMap();
		if (attackMap != nullptr && attackMap->size() > 0) {

			msg << "Primary Weapon Attack Map" << endl;
			for (int i = 0; i < attackMap->size(); i++) {
	...
	if (pet->getSecondaryWeapon() != nullptr) {
		msg << "Secondary Weapon: " << pet->getSecondaryWeapon()->getObjectTemplate()->getTemplateFileName() << ", OID: " << pet->getSecondaryWeapon()->getObjectID() << endl;
		msg << "Min Damage: " << pet->getSecondaryWeapon()->getMinDamage() << endl;
		msg << "Max Damage: " << pet->getSecondaryWeapon()->getMaxDamage() << endl;

		const CreatureAttackMap* attackMap = pet->getSecondaryAttackMap();
		if (attackMap != nullptr && attackMap->size() > 0) {

			msg << "Secondary Weapon Attack Map" << endl;
			for (int i = 0; i < attackMap->size(); i++) {
	...
	*/
	...
	if (petType == PetManager::CREATUREPET) {
		const CreatureTemplate* activePetTemplate = pet->getCreatureTemplate(); 
		if (activePetTemplate != nullptr && (activePetTemplate->getTemplateName() != "at_st" && activePetTemplate->getTemplateName() != "rebel_droideka")) {
	
			ManagedReference<WeaponObject*> primaryWeapon = pet->getPrimaryWeapon();

			if (primaryWeapon != nullptr && primaryWeapon->isRangedWeapon() && !pet->hasPetDeed()) {

				ManagedReference<WeaponObject*> secondaryWeapon = pet->getSecondaryWeapon();
				ManagedReference<WeaponObject*> defaultWeapon = pet->getDefaultWeapon();

				if (secondaryWeapon == nullptr && defaultWeapon != nullptr) {
					player->sendSystemMessage("Your pet, " + pet->getDisplayedName() + ", has a ranged primary weapon and no secondary weapon.  Attempting to create a secondary melee weapon.  Please call your pet again.");
					pet->setSecondaryWeapon(defaultWeapon);
					return;
				}
			}
		}
	...
	if (zoneServer != nullptr) {
		...
		ManagedReference<FrsManager*> frsManager = zoneServer->getFrsManager();
	...
	for (int i = 0; i < ghost->getActivePetsSize(); ++i) {
		...
		if (object != nullptr) {
			if (object->isCreature() && petType == PetManager::CREATUREPET) {
				...
				const CreatureTemplate* activePetTemplate = object->getCreatureTemplate();  //Infinity:  Don't count the big faction pets which are creatures, not NPCs
				if (activePetTemplate != nullptr && (activePetTemplate->getTemplateName() == "at_st" || activePetTemplate->getTemplateName() == "rebel_droideka")) {
					continue;
				}
				...
				if (++currentlySpawned >= maxPets || (activePetTemplate->getTemplateName() == "at_st" && callingPetTemplate->getTemplateName() == "at_st") || 
						(activePetTemplate->getTemplateName() == "rebel_droideka" && callingPetTemplate->getTemplateName() == "rebel_droideka")) {
					player->sendSystemMessage("@pet/pet_menu:at_max"); // You already have the maximum number of pets of this type that you can call.
					return;
				}
			}
	...
	if (player->getCurrentCamp() == nullptr && player->getCityRegion() == nullptr && !ghost->isPrivileged()) {
		...
		// Infinity custom pet call time of 3 seconds
		int petCallTime = 3;
		...
		// Infinity
		message.setDI(petCallTime);
		player->sendSystemMessage(message);

		// Infinity
		player->addPendingTask("call_pet", callPet, petCallTime * 1000);

		if (!player->hasSkill("outdoors_creaturehandler_master") || petType != PetManager::CREATUREPET) { // Infinity: Cancel pet call on combat start unless Master CH calling creature pet
...
int PetControlDeviceImplementation::handleObjectMenuSelect(CreatureObject* player, byte selectedID) {
	...
	if (ghost == nullptr) {
		error("ghost is nullptr object in pet control device");
		return 1;
	}

	if (selectedID == 44) {  //Call
		if (pet == nullptr) {
...
void PetControlDeviceImplementation::storeObject(CreatureObject* player, bool force) {
	...
	if (player == nullptr || controlledObject == nullptr || !controlledObject->isAiAgent())
		return;
	...
	// Cross lock the player to the pet for dismount
	...
	if (task == nullptr) {
		player->sendSystemMessage("StorePetTask is nullptr, contact support.");
		return;
	}
	...
	if (getPetType() != PetManager::FACTIONPET || force || (ghost != nullptr && ghost->isPrivileged())) {
		...
		player->sendSystemMessage( "Pet will store in 60 seconds.");
...
bool PetControlDeviceImplementation::growPet(CreatureObject* player, bool force, bool adult) {
	...
	int stagesToGrow = timeDelta / 8640; // Infinity:  8640 per stage for live (24 hour growth, 144 minutes per stage)

	if (zoneServer->getGalaxyName() != "SWG Infinity") { //Infinity: on TC or dev box
		stagesToGrow = timeDelta / 30;	//Should be 5 minutes to adult growth
	}
...
bool PetControlDeviceImplementation::isValidPet(AiAgent* pet) {
	...
	if (deed != nullptr) {
		...
		if (deed->getLevel() <= 10 && deed->getMaxDamage() > 700)
			return true;
