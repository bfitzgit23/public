...
void StructureMaintenanceTask::run() {
	...
	if (oid == 0 || name == "") {
		StringBuffer msg;
		msg << "Found structure with no owner at " << "x = " << (int)strongRef->getPositionX() << ", y = " << (int)strongRef->getPositionY() << " on " << strongRef->getZone()->getZoneName();
		if (strongRef->isClientObject())
			msg << ".  Structure is a client object.";
		else
			msg << ".  Structure is not a client object.  Template is " <<  strongRef->getObjectTemplate()->getTemplateFileName() << ".";
		error(msg.toString());
	}
	...
	if (strongRef->getSurplusMaintenance() > 0) {
        ...
        /*
        if (strongRef->getSurplusMaintenance() > 9999999) {
            StringBuffer msg;
            msg << "Found structure with credits > 9,999,999 " << "x = " << (int)strongRef->getPositionX() << ", y = " << (int)strongRef->getPositionY() << " on " << strongRef->getZone()->getZoneName();
            msg << ".  Owner = " << name << ".  Surplus maintenance = " << strongRef->getSurplusMaintenance() << ".";
            error(msg.toString());
        }
        */
