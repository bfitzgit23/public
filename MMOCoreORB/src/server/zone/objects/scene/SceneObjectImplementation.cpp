...
bool SceneObjectImplementation::isNearBank() {
	...
	if (parentID != 0) {
		...
		if (isPlayerCreature() && parent != nullptr && (parent->isMount() || parent->isVehicleObject())) {
	...
	for (int i = 0; i < closeObjects.size(); ++i) {
		...
		if (closeObjectsVector != nullptr && !isInRange(sceneO, 15.f)) 
			continue;
...
float SceneObjectImplementation::getDistanceTo3d(SceneObject* targetCreature) {
	auto targetWorldPosition = targetCreature->getWorldPosition();
	float x = targetWorldPosition.getX();
	float y = targetWorldPosition.getY();
	...
	float deltaX = x - worldPosition.getX();
	float deltaY = y - worldPosition.getY();
	float deltaZ = z - worldPosition.getZ();
