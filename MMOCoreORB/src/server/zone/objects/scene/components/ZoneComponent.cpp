	...
	try {
		...
		if (closeObjects != nullptr) {
			...
			for (int i = 0; i < objects.size(); ++i) {
				...
				if (object != nullptr) { // Infinity:  Adding a nullptr check here as we are seeing some exceptions on this call
					try {
						object->notifyPositionUpdate(sceneObject);
					} catch (Exception& e) {
						object->error("exception while calling notifyPositionUpdate: " + e.getMessage() + "OID: " + String::valueOf(object->getObjectID()));
					}
				}
			}
		}
