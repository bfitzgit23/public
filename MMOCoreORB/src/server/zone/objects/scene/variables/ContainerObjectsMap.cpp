...
ManagedReference<SceneObject*> ContainerObjectsMap::get(int index) {
	...
	ManagedReference<SceneObject*> strongRef = container.get();

	if (strongRef != nullptr) {
		if (containerObjects.size() == 0) {
			uint64 oid = strongRef->getObjectID();
			String name = strongRef->getDisplayedName();
			StringBuffer msg;
			auto trace = StackTrace();
			msg << "-------------------------------------------------------------" << endl;
			msg << "Call stack:" << endl << trace.toStringData() << endl;
			msg << "ContainerObjectsMap::get - called while empty with index(" << index << ").  Container name: " << name << "  Container OID: " << oid << endl;
			msg << "-------------------------------------------------------------";
			Logger::console.error(msg.toString());
		}

		if (index < 0) { // This is going to generate an array out of bounds exception
			uint64 oid = strongRef->getObjectID();
			String name = strongRef->getDisplayedName();
			StringBuffer msg;
			auto trace = StackTrace();
			msg << "-------------------------------------------------------------" << endl;
			msg << "Call stack:" << endl << trace.toStringData() << endl;
			msg << "ContainerObjectsMap::get - called with negative index(" << index << ").  Container name: " << name << "  Container OID: " << oid << endl;
			msg << "-------------------------------------------------------------";
			Logger::console.error(msg.toString());
		}

		if (index > containerObjects.size() - 1) { // This is going to generate an array out of bounds exception
			uint64 oid = strongRef->getObjectID();
			String name = strongRef->getDisplayedName();
			StringBuffer msg;
			auto trace = StackTrace();
			msg << "-------------------------------------------------------------" << endl;
			msg << "Call stack:" << endl << trace.toStringData() << endl;
			msg << "ContainerObjectsMap::get - called with index(" << index << ") > containerObjects.size(" << containerObjects.size() << ") - 1.   Container name: " << name << "  Container OID: " << oid << endl;
			msg << "-------------------------------------------------------------";
			Logger::console.error(msg.toString());
		}
	}
