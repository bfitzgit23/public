...
void SuiManager::handleCharacterBuilderSelectItem(CreatureObject* player, SuiBox* suiBox, uint32 cancel, Vector<UnicodeString>* args) {
	...
	if (player == nullptr) {
		error("SuiManager::handleCharacterBuilderSelectItem - player is nullptr");
		return;
	}
	...
	if (node->hasChildNodes()) {
		...
		if (templatePath.indexOf(".iff") < 0) { // Non-item selections
			...
			if (templatePath == "unlearn_all_skills") {
				...
				SkillManager::instance()->surrenderAllSkills(player, true, true);  //Infinity:  Remove all but force_sensitive skills, including Force progression
			...
			} else if (templatePath == "reset_buffs") {  //Infinity
				if (!player->isInCombat()) {
					player->sendSystemMessage("Your buffs and stomach filling have been reset.");
					...
					player->sendSystemMessage("You cannot reset buffs while in combat.");
					return;
				}
			...
			} else if (templatePath == "gmenhance_character") { //Infinity:   Custom SUI option
				...
				player->sendSystemMessage("You have received 1,000,000 Credits.");  //Infinity:
				...
				auto pman = zserv->getPlayerManager();
				if (pman == nullptr)
					return;
			...
			} else if (templatePath.beginsWith("set_rank_")) {

				int rank = Integer::valueOf(templatePath.subString(9));
				...
				if (rank >=0 && rank <= 11) {
					...
					//Adjust experience to match rank
			...
			} else if (templatePath == "max_visibility") {
				ghost->setVisibility(8000);
				player->sendSystemMessage("You have maximized your visibility.");

			} else if (templatePath == "clear_visibility") {
				ghost->setVisibility(0);
				player->sendSystemMessage("You have cleared visibility.");
				...
				if (pman == nullptr)
					return;
				pman->unlockPadawanForTesting(player);
