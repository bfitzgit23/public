...
bool PlayerCreationManager::createCharacter(ClientCreateCharacterCallback* callback) const {
	...
	// Allow more characters in non-production servers
	bool liveServer = zoneServer.get()->getGalaxyName() == "SWG Infinity";   // Infinity:   Change behavior for lower environments

	// Infinity:  7 characters per account
	if (liveServer && client->getCharacterCount(zoneServer.get()->getGalaxyID()) >= maxchars) {
		ErrorMessage* errMsg = new ErrorMessage("Notice", "Infinity allows the creation of " + String::valueOf(maxchars) + " characters per account.", 0x0);
	...
	if (ghost != nullptr) {
		...
		if (!freeGodMode) {
			try {
				...
				// Allow more characters on test servers
				bool liveServer = zoneServer.get()->getGalaxyName() == "SWG Infinity";   // Infinity:   Change behavior for lower environments
				...
				if (accountPermissionLevel < 9) {
					try {
						...
						if (res != nullptr && res->next()) {
							...
							Time timeVal(sec);
							// Infinity:  Live server check  
							if (liveServer && timeVal.miliDifference() < 3600000) {
					...
					if (lastCreatedCharacter.containsKey(accID)) {
						...
						// Infinity:  Live server check  
						if (liveServer && lastCreatedTime.miliDifference() < 3600000) {
							ErrorMessage* errMsg = new ErrorMessage("Create Error", "You are only permitted to create one character per hour. Repeat attempts prior to 1 hour elapsing will reset the timer.", 0x0);
							client->sendMessage(errMsg);
	...
	// Infinity:  Custom New Player Broadcast Announcement
...
void PlayerCreationManager::addHair(CreatureObject* creature,
	...
	/* Infinity:  To Allow Chiss Race with Human hairstyle
