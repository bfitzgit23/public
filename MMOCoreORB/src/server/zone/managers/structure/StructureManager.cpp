...
StructureObject* StructureManager::placeStructure(CreatureObject* creature, const String& structureTemplatePath, float x, float y, int angle, int persistenceLevel) {
	...
	if (structureObject->isGuildHall()) {
		...
		if (ghost != nullptr)
			ownerAccountID = ghost->getAccountID();

		String pX = String::valueOf(ceil(x));
		String pZ = String::valueOf(ceil(z));
		String pY = String::valueOf(ceil(y));
...
int StructureManager::declareResidence(CreatureObject* player, StructureObject* structureObject, bool isCityHall) {
	...
	if (!isCityHall && !player->checkCooldownRecovery("declare_residence") && !ghost->isPrivileged()) {
		...
		float timeLeft = round(fabs(timeRemaining->miliDifference() / 1000.0f / 3600.0f * 100.0f)) / 100.f;   // Infinity:   Hours to two decimal places
	...
	uint64 coolDownTimer = 4 * 3600 * 1000;  //Infinity: 4 Hour Default cooldown on change in residence
	...
	if (galaxyName != "SWG Infinity") { //Infinity: on TC or dev box cooldown is 30 seconds
		coolDownTimer = 30 * 1000;  
	}

	player->addCooldown("declare_residence", coolDownTimer); 
...
void StructureManager::payMaintenance(StructureObject* structure, CreatureObject* creature, int amount) {
	...
	if (structure->isGuildHall() && amount >= 1) {

		uint32 creatureAccountID = ghost->getAccountID();
		uint32 ownerAccountID = 0;

		auto structureZone = structure->getZone();

		String zoneName = "Error - unknown";
		if (structureZone != nullptr)
			zoneName = structureZone->getZoneName();
		zoneName[0] = toupper(zoneName[0]);

		String ownerName = "Error - unknown";

		ManagedReference<CreatureObject*> ownerCreo = structure->getOwnerCreatureObject();

		if (ownerCreo != nullptr) {
			ownerName = ownerCreo->getFirstName();

			auto ownerGhost = ownerCreo->getPlayerObject();
			if (ownerGhost != nullptr) {
				ownerAccountID = ownerGhost->getAccountID();
			}
		}

		StringBuffer msg;

		msg << ") added ~" << amount << "~ credits in maintenance to a guildhall.  ";
		msg << "Owner: " << ownerName << " (AcctID:" << ownerAccountID;
		msg << ").  Zone: " << zoneName << ", coordinates (x=";
		msg << ceil(structure->getWorldPositionX()) << ", y=" << ceil(structure->getWorldPositionY()) << ", z=" << ceil(structure->getWorldPositionZ()) << ").";
...
void StructureManager::withdrawMaintenance(StructureObject* structure, CreatureObject* creature, int amount) {
	...
	StringIdChatParameter params("player_structure", "withdraw_maintenance_credits"); // You successfully withdraw %DI credits from the maintenance pool
	params.setDI(amount);
	...
	if (amount >= 1) {
		...
		auto ghost = creature->getPlayerObject();
		if (ghost != nullptr) {
			creatureAccountID = ghost->getAccountID();
		}
		...
		msg << ") withdrew ~" << amount << "~ credits in maintenance from a guildhall.  ";
		msg << "Owner: " << ownerName << " (AcctID:" << ownerAccountID;
		msg << ").  Zone: " << zoneName << ", coordinates (x=";
		msg << ceil(structure->getWorldPositionX()) << ", y=" << ceil(structure->getWorldPositionY()) << ", z=" << ceil(structure->getWorldPositionZ()) << ").";
