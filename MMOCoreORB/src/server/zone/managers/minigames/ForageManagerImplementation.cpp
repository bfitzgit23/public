...
void ForageManagerImplementation::startForaging(CreatureObject* player, int forageType) {
    ...
    //Infinity:  Reduce special action cost
...
void ForageManagerImplementation::finishForaging(CreatureObject* player, int forageType, float forageX, float forageY, const String& zoneName) {
	...
	case ForageManager::LAIR:
        ...
        chance = (int)(15 + (skillMod * 0.95));  //Infinity:  Improved chances for scout and lair foraging
		break;
	...
	default:
        skillMod = player->getSkillMod("foraging");
		chance = (int)(25 + (skillMod * 0.6f));  // 100% chance of mollusk or crustacean meat for master ranger with +25 foraging tape
		break;
	}
...
bool ForageManagerImplementation::forageGiveItems(CreatureObject* player, int forageType, float forageX, float forageY, const String& planet) {
	...
	//Determine how many items the player finds.  
    //InfinitY:  Added extra bonuses for ranger
    if (forageType == ForageManager::SCOUT) {
		if (player->hasSkill("outdoors_scout_camp_03") && System::random(5) == 0)  //Infinity:  1/6 chance, successive rolls have lower chance
			itemCount += 1;
		if (player->hasSkill("outdoors_scout_master") && System::random(6) == 0)
			itemCount += 1;
		if (player->hasSkill("outdoors_ranger_harvest_02") && System::random(7) == 0) 
			itemCount += 1;
		if (player->hasSkill("outdoors_ranger_master") && System::random(8) == 0)
			itemCount += 1;
	}
	...
	if (forageType == ForageManager::SHELLFISH){
		...
		if (forageGiveResource(trx, player, forageX, forageY, planet, resName)) {
            /*// Infinity:  Not going to duplicate the messages for shellfish
            if (mullosks)
	...
	if (forageType == ForageManager::SCOUT) {
		...
		for (int i = 0; i < itemCount; i++) {
			dice = System::random(209);     //Infinity:  Increased for treasure map (rare) chance, should be 10/209 chance
			level = 1;

			if (dice >= 0 && dice < 160) {
				lootGroup = "forage_food";
			}
            else if (dice >=160 && dice < 200) {
				lootGroup = "forage_bait";
			}
            else {  //200+ should be treasure maps
        ...
        else if (dice >= 40 && dice < 110) { //Resources.
			if (forageGiveResource(trx, player, forageX, forageY, planet, resName)) {
        ...
        else if (dice >= 110 && dice < 170) { //Average components.
			lootGroup = "forage_medical_component";
			level = 1;
		}
        else if (dice >= 170 && dice < 200) { //Good components.
			lootGroup = "forage_medical_component";
			level = 60;
		}
		...
		if (dice >= 0 && dice < 35) { // Live Creatures
			lootGroup = "forage_live_creatures";
		}
		else { //Infinity:   Increased chance of egg.   Dice is 35 or greater
			resName = "meat_egg";
			if (forageGiveResource(trx, player, forageX, forageY, planet, resName)) {
...
bool ForageManagerImplementation::forageGiveResource(TransactionLog& trx, CreatureObject* player, float forageX, float forageY, const String& planet, String& resType) {
    ...
    if (resource == nullptr) {  //This should never happen, but we like to be safe
    	StringBuffer message;
    	message << "Resource is nullptr";
		warning(message.toString());
        return false;
    }

     //Infinity:  Increased quantity for shellfish and egg harvesting
	int quantity = System::random(45);
    if (resType.beginsWith("meat_egg")) {                       //Infinity:  Eggs are hard to get
        quantity += player->getSkillMod("foraging");
    }
    else {
        quantity += player->getSkillMod("foraging") * 0.5f;     //Infinity:  Shellfish are easy to get
    }

	resourceManager->harvestResourceToPlayer(trx, player, resource, quantity);
	
	// Infinity:  Send System Message
    player->sendSystemMessage("You managed to locate " + String::valueOf(quantity) + " units of " + resource->getFinalClass() + ".");
