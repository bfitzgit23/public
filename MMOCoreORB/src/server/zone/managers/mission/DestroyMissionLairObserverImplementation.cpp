...
bool DestroyMissionLairObserverImplementation::checkForNewSpawns(TangibleObject* lair, TangibleObject* attacker, bool forceSpawn) {
	...
	if (spawnNumber == 4) {//Infinity:  Lots of custom stuff here
		...
		bossChance *= 0.5;  //Destroy mission lairs have half the boss mobile chance of while lairs

		if (bossChance == 0)
			bossChance = 10;	//Infinity:  Default boss chance for destroy mission lair is 10%
	...
	for(int i = 0; i < objectsToSpawn.size(); ++i) {
		...
		if (zoneServer != nullptr && zoneServer->getGalaxyName() != "SWG Infinity")
			tamingChance = Math::max(tamingChance, 0.50f);
