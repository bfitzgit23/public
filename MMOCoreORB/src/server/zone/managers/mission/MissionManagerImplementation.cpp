...
void MissionManagerImplementation::loadLuaSettings() {
	try {
		...
		destroyMissionGroupRewardFactor = lua->getGlobalLong("destroyMissionGroupRewardFactor");  //Infinity
...
void MissionManagerImplementation::handleMissionListRequest(MissionTerminal* missionTerminal, CreatureObject* player, int counter) {
	...
	if (missionTerminal->isBountyTerminal()) {
		...
		bool playerIsJedi = player->hasSkill("force_title_jedi_rank_02");   //Considered Jedi if they have Padawan skill box
		bool playerHasGodMode = false;
		ManagedReference<PlayerObject*> playerGhost = player->getPlayerObject();

		if (playerGhost != nullptr) {
			playerIsJedi = playerGhost->isJedi();
			playerHasGodMode = playerGhost->hasGodMode();
		}

		if (!(player->hasSkill("combat_bountyhunter_novice") || (player->hasSkill("mandalorian_master") && !playerIsJedi) || playerHasGodMode)) {
	...
	int maximumNumberOfItemsInMissionBag = 24;  //Infinity:  Increased mission bag size
...
void MissionManagerImplementation::handleMissionAccept(MissionTerminal* missionTerminal, MissionObject* mission, CreatureObject* player) {
	...
	//Limit to two missions (only one of them can be a bounty mission)  //Infinity: Allow 6 missions total
		...
		if (targetID != 0) {
			...
			if (bounty == nullptr || !isBountyValidForPlayer(player, bounty)) {
				...
				if (target != nullptr && target->isPlayerCreature()) {
					Logger::console.error("MissionManager::handleMissionAccept:  Hunter " + player->getDisplayedName() + " accepted bounty mission for player " + target->getDisplayedName());
				}
			}
		}
	}
...
void MissionManagerImplementation::handleMissionAbort(MissionObject* mission, CreatureObject* player) {
	...
	if (mission->getTypeCRC() == MissionTypes::BOUNTY) { 
		...
		if (target != nullptr && target->isPlayerCreature()) {
			Logger::console.error("MissionManager::handleMissionAbort:  Hunter " + player->getDisplayedName() + " dropped bounty mission for player " + target->getDisplayedName());
		}
	}
...
void MissionManagerImplementation::randomizeGeneralTerminalMissions(CreatureObject* player, int counter, bool slicer) {
	...
	for (int i = 0; i < bagSize; ++i) {
		...
		if (i < 18) { //Infinity:  Adjust number of missions displayed per page
			randomizeGenericDestroyMission(player, mission, Factions::FACTIONNEUTRAL);
		} else if (i < 35) { //Infinity:  Adjust number of missions displayed per page
			randomizeGenericDeliverMission(player, mission, Factions::FACTIONNEUTRAL);
		}

		if (slicer) {
			mission->setRewardCredits(mission->getRewardCredits() * 1.5f);
		}
...
void MissionManagerImplementation::randomizeGenericDestroyMission(CreatureObject* player, MissionObject* mission, const uint32 faction) {
	...
	int reward = destroyMissionBaseReward + ((diffDisplay * 0.5f + System::random(diffDisplay * 0.5f)) * destroyMissionGroupRewardFactor) + (difficultyLevel * destroyMissionDifficultyRewardFactor); //Infinity:  15 and 375

	reward = Math::min(reward, 50000);   //Infinity:  50,000 credit max payout
	...
	int factionPointsReward = (randomLairSpawn->getMinDifficulty() * 6) + System::random(50);  //Infinity:  Should payout 1850 on average
	...
	//Infinity:  CL and mob names on lair
	String groupSuffix;
	...
	mission->setMissionTitle("CL" + String::valueOf(diffDisplay), " Destroy the " + mobileName.replaceAll("_", " ") + groupSuffix);  //Infinity:  Custom CL and creature names
...
void MissionManagerImplementation::randomizeGenericBountyMission(CreatureObject* player, MissionObject* mission, const uint32 faction, Vector<ManagedReference<PlayerBounty*>>* potentialTargets) {
	...
	auto playerZone = player->getZone();
	if (playerZone == nullptr) {
		return;
	}
	...
	int level = 1;
	int randomTexts = 25;
	if (player->hasSkill("combat_bountyhunter_investigation_03") || (player->hasSkill("mandalorian_master") && !playerIsJedi) || playerHasGodMode) {
		level = 3;
	...
	if (playerTarget) {
		...
		if (target != nullptr) {
			...
			String creatureName = "";
			String missionTargetName = "Unidentified Player Target";
			String missionTitle = "Unidentified player target";
			String stfFile = "mission/mission_bounty_jedi";

			if (creature != nullptr)  {
				creatureName = creature->getFirstName() + " " + creature->getLastName();
				creatureName = creatureName.trim();

				bool targetIsPadawan = creature->hasSkill("force_title_jedi_rank_02");   //Considered Jedi if they have Padawan skill box
				bool targetHasJediWeapon = false;

				ManagedReference<WeaponObject*> weapon = creature->getWeapon();

				if (weapon != nullptr && weapon->isJediWeapon()) {
					targetHasJediWeapon = true;
				}
				...
				if (targetGhost != nullptr) {
					if (ConfigManager::instance()->getBool("Core3.MissionManager.AnonymousBountyTerminals", false)) {
						...
						if (targetIsPadawan) {
							...
							if (rank >= 6) { //FRS rank 6+
								missionTargetName = creatureName;
								mission->setMissionTitle("Player Bounty", " Eliminate the Jedi player target " + creatureName + " (Rank " + String::valueOf(rank) + ")");
								/*
								if (playerHasGodMode)
									mission->setMissionTitle("Player Bounty", " Eliminate the Jedi player target " + creatureName + " (Rank " + String::valueOf(rank) + ")");
								else
									mission->setMissionTitle("Player Bounty", " Eliminate the Jedi player target " + creatureName);
								*/
							}
							else if (rank <= 5 && rank >= 0) {  //FRS rank 0-5
								if (playerHasGodMode)
									mission->setMissionTitle("Player Bounty", " Eliminate the Jedi player target " + creatureName + " (Rank " + String::valueOf(rank) + ")");
								else
									mission->setMissionTitle("Player Bounty", " Eliminate unidentified Jedi player target");
							}
							else if (creature->hasSkill("combat_bountyhunter_investigation_03") ) {
								stfFile = "mission/mission_bounty_nonjedi";
								if (playerHasGodMode)
									mission->setMissionTitle("Player Bounty", " Eliminate the non-Jedi player target " + creatureName);
								else
									mission->setMissionTitle("Player Bounty", " Eliminate unidentified non-Jedi player target");
							}
							else {
								if (playerHasGodMode)
									mission->setMissionTitle("Player Bounty", " Eliminate the Jedi player target " + creatureName + " (Rank " + String::valueOf(rank) + ")");
						...
						missionTargetName = creatureName;
						if (targetIsPadawan) {
							if (creature->hasSkill("combat_bountyhunter_investigation_03")) {
								stfFile = "mission/mission_bounty_nonjedi";	
								mission->setMissionTitle("Player Bounty", " Eliminate the non-Jedi player target " + creatureName);
							}
							else {
			...
			mission->setMissionTargetName(missionTargetName); 	//Infinity
			mission->setRewardCredits(target->getReward()); 	//Infinity
			...
			//mission->setMissionTitle(stfFile, "m" + String::valueOf(randTexts) + "t");  //Infinity
		...
		if (playerHasGodMode && !player->hasSkill("combat_bountyhunter_novice")) { //Infinity:  Admins only want to see player missions
			return;
		}
...
bool MissionManagerImplementation::isBountyValidForPlayer(CreatureObject* player, PlayerBounty* bounty) {
	...
	*/
	...
	if (creature->isDead()) {  //Infinity:  Don't include corpses
		return false;
	}

	if (ConfigManager::instance()->getBool("Core3.MissionManager.PlayerBountyMarkVictoryCooldown", false) && !creature->checkCooldownRecovery("BountyMarkVictory")) { //30 minute cooldown after killing BH 
		return false;
	}
	...
	if (targetGhost == nullptr || targetGhost->hasGodMode() || targetGhost->getVisibility() < terminalVisibilityThreshold) {
		return false;
	}
	...
	//Must be admin, Death Watch, or BH Investigation 3 to see players
	if (!(playerGhost->hasGodMode() || player->hasSkill("combat_bountyhunter_investigation_03") || player->hasSkill("mandalorian_master")))
		return false;

	//Checks for non-admin Mandos that do not have BH Investigation 3
	if (!playerGhost->hasGodMode() && player->hasSkill("mandalorian_master") && !player->hasSkill("combat_bountyhunter_investigation_03")) {
		...
		if (creature->hasSkill("force_title_jedi_rank_02")) {
			...
			if (targetHasJediWeapon) {  //If holding a lightsaber, Mando cannot hunt
				return false;
			}
			else {  //Check elapsed time since last GCW TEF and number of Jedi skillboxes

				float gcwTefHours = targetGhost->getLastGcwPvpCombatActionTimestamp().miliDifference() / 1000.0f / 3600.0f;
				int skillBoxesJedi = 49;  //Default is full template

				SkillManager* skillManager = SkillManager::instance();
				if (skillManager != nullptr) {
					skillBoxesJedi = skillManager->getSpecificSkillCount(creature,"force_discipline_");
				}

				//If Last GCW TEF happened over 24 hours before or Player has at least two full Jedi trees, Mando cannot hunt
				if (gcwTefHours > 24 || skillBoxesJedi >= 36 ) {
					return false;
				}
			}
		}
	...
	/* Old checks
	//Infinity:  Jedi-Mando checks
	if (!playerGhost->hasGodMode() && !player->hasSkill("combat_bountyhunter_investigation_03") && player->hasSkill("mandalorian_master") && !creature->hasSkill("combat_bountyhunter_investigation_03") &&
			(creature->hasSkill("force_title_jedi_rank_02") && (creature->hasSkill("force_discipline_healing_novice") || creature->hasSkill("force_discipline_powers_novice") ||
			creature->hasSkill("force_discipline_enhancements_novice") || creature->hasSkill("force_discipline_light_saber_novice") || creature->hasSkill("force_discipline_defender_novice"))))
		return false;
	*/

	uint64 accountId = playerGhost->getAccountID();

	if (!ConfigManager::instance()->getBool("Core3.MissionManager.EnableSameAccountBountyMissions", false) && targetGhost->getAccountID() == accountId) {
		return false;
	}
	...
	if (!ConfigManager::instance()->getBool("Core3.MissionManager.PrivateStructureJediMissions", false)) {
		if (building != nullptr && (building->isPrivateStructure() || building->isOnBanList(player)))   //Infinity:  Check if BH is on ban list for structure also
			return false;
	}

	//Infinity:  BH Target checks
	...
	if (zone != nullptr) {
		...
		if (zoneName == "naboo") {   //Check to see if target is unattackable in Highgarden neutral zone
			if (targetX >= -5120 && targetX <= -4943 && targetY >= 2802 && targetY <= 2997) {
				return false;			
			}
		}

		if (zoneName == "corellia") {   //Check to see if target is unattackable in Swamptown neutral zone
			if (targetX >= 1400 && targetX <= 1650 && targetY >= 3800 && targetY <= 4100) {
				return false;			
			}
		}

		if (zoneName == "endor") {   //Check to see if target is unattackable on Rikers Island
			if (targetX >= -7074 && targetX <= -7006 && targetY >= -7074 && targetY <= -7006) {
				return false;
			}
		}

		/*
		// Let's check to see if in stadium and not show them on BH terms if they are
		bool inStadium = creature->getScreenPlayState("stadium:insideStadium") & 1;
		if (inStadium) {
			return false;
		}
		*/
			...
			if (terrainManager != nullptr) {
				...
				if ((heightDifference) > 3.0f && building == nullptr) { // Over 3 meters above terrain and not in building, then check nearby objects for structure
					...
					if (closeObjectsVector == nullptr) {
						...
						zone->getInRangeObjects(targetX, targetY, 25, &nearbyObjects, true, true);
					}
					else {
					...
					for(int i = 0; i < nearbyObjects.size(); ++i) {
						...
						if (scno != nullptr && scno->isBuildingObject()) {
							...
							if (building != nullptr && building->isPrivateStructure() && building->isInRange(creature, 25.f)) {
	...
	//Infinity: Let's check to see if in stadium and not show them on BH terms if they are
	bool inStadium = creature->getScreenPlayState("stadium:insideStadium") & 1;
	if (inStadium) {
		return false;
	}
	...
	for (int j = 0; j < hunters->size(); j++) {
		...
		if (hunter != nullptr) {
			ManagedReference<PlayerObject*> hunterGhost = hunter->getPlayerObject();
	...
	if (ConfigManager::instance()->getBool("Core3.MissionManager.PlayerBountyCooldown", false)) {
		int cooldownTime = ConfigManager::instance()->getInt("Core3.MissionManager.PlayerBountyCooldownTime", 3600000); // 1 hour default
	...
	if (creature != nullptr) {
		...
		if (mission != nullptr) {
			...
			if (objective != nullptr) {
				...
				if (ConfigManager::instance()->getBool("Core3.MissionManager.PlayerBountyCooldown", false) && targetID > 0) {
					PlayerBounty* playerBounty = playerBountyList.get(targetID);
...
bool MissionManagerImplementation::sendPlayerBountyDebug(CreatureObject* creature, CreatureObject* target) {
	...
	box->setPromptTitle("Player Visibility");
	...
	if (playerBounty == nullptr) {
		...
		int skillsReward = targetGhost->calculateBhReward();
		int playerBountyReward = playerBounty->getReward() - skillsReward;
		promptText += "Skills Bounty Reward: \t" + String::withCommas(skillsReward) + "\n";
		promptText += "Player Bounty Reward: \t" + String::withCommas(playerBountyReward) + "\n";
		promptText += "Total Bounty Reward: \t" + String::withCommas(playerBounty->getReward()) + "\n";
...
int MissionManagerImplementation::getPlayerBounty(uint64 targetId) {
	...
	if (playerBountyList.contains(targetId)) {
		//info("Player found:" + String::valueOf(targetId) + " returning reward =" + String::valueOf(playerBountyList.get(targetId)->getReward()),true);
		return playerBountyList.get(targetId)->getReward();
	}
	else{
		//info("Player not found, no valid bounty.",true);
		return 0;
	}
}