...
public:
	...
	const static int FORCESHIELD = 0x2;
	const static int FORCEFEEDBACK = 0x3;
	const static int FORCEABSORB = 0x4;
	...
	// toHitChance
	const static constexpr float toHitScale = 50.f;		//Infinity:  REVERTED Lowered from 50 to 20 to increase the weight of the difference in accuracy and defense
	const static constexpr float toHitBase = 75.f;		//This is hit chance if attackerAccuracy = targetDefense
...
public:
    ...
    ArmorObject* getArmorObject(CreatureObject* defender, uint8 hitLocation) const;     //Infinity:  Moved from protected section
    float getArmorPiercing(TangibleObject* defender, int armorPiercing) const;          //Infinity:  Moved from protected section
	...
	*/
	...
	float getWeaponPostureModifier(uint32 attackWeaponMask) const {
		switch(attackWeaponMask) {
			case WeaponType::RIFLEWEAPON: return 1.2f;			//Infinity:  previously 2.5
			case WeaponType::CARBINEWEAPON: return 1.15f;		//Infinity:  previously 2.0
			case WeaponType::PISTOLWEAPON: return 1.1f;			//Infinity:  previously 1.5
			case WeaponType::SPECIALHEAVYWEAPON: return 1.2f;	//Infinity:  previously 3.0
