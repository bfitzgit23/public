...
bool GCWManagerImplementation::canUseTerminals(CreatureObject* creature, BuildingObject* building, SceneObject* terminal) {
	...
	if (building->getPvpStatusBitmask() & CreatureFlag::OVERT) {
		...
		if (creature->getFactionStatus() != FactionStatus::OVERT) {  //Infinity:  Must be overt to bust base
			creature->sendSystemMessage("@hq:declared_only"); // Only Special Forces personnel may access this terminal!
			return false;
		}
	}
...
bool GCWManagerImplementation::canStartSlice(CreatureObject* creature, TangibleObject* tano) {
	...
	if (!areOpposingFactions(creature->getFaction(), building->getFaction())) {
		...
		return false;
	} else if (creature->getSkillMod("slicing") < 15) {
...
void GCWManagerImplementation::doBaseDestruction(BuildingObject* building) {
	...
	int faction = building->getFaction(); //Infinity Custom 
	bool pveBase = !(building->getPvpStatusBitmask() & CreatureFlag::OVERT);
	auto zone = building->getZone();
	int posX = building->getPositionX();
	int posY = building->getPositionY();
	...
	if (baseType == PLAYERFACTIONBASE && pveBase && zone != nullptr) {

		SortedVector<ManagedReference<QuadTreeEntry*> > closeObjects;
		zone->getInRangeObjects(posX, posY, 128.0f, &closeObjects, true);

		for (int i = 0; i < closeObjects.size(); ++i) {
			ManagedReference<SceneObject*> object = cast<SceneObject*>(closeObjects.get(i).get());

			if (object == nullptr || !object->isAiAgent() || object->isPet())
				continue;

			ManagedReference<AiAgent*> agent = cast<AiAgent*>(object.get());

			if (agent != nullptr && agent->getFaction() == faction) {
				Locker locker(agent);
...
void GCWManagerImplementation::performCheckWildContrabandScanTask() {
	...
	for (int i = 0; i < playerList.size(); i++) {
		...
		ManagedReference<CreatureObject*> player = object->asCreatureObject();
...
void GCWManagerImplementation::listBases(CreatureObject* creature, bool destroy, bool pveOnly) {
	...
	for (int i = 0; i < gcwBaseList.size(); i++) {
		...
		if (destroy && building->isGCWBase()) {
			if (!pveOnly || (pveOnly && !(building->getPvpStatusBitmask() & CreatureFlag::OVERT))) {
				doBaseDestruction(building);
			}
			body << "******* DESTROYED *******" << endl;
		}
...
bool GCWManagerImplementation::tooCloseToNpcCity(int x, int y) {
	if (zone == nullptr)
		return true;

	SortedVector<QuadTreeEntry*> inRangeObjects;
	zone->getInRangeObjects(x, y, 750, &inRangeObjects, true, false);
	...
	for (int i = 0; i < inRangeObjects.size(); ++i) {
		...
		ManagedReference<CityRegion*> cityRegion = scene->getCityRegion().get();

		if (cityRegion != nullptr) {
			if (cityRegion->isClientRegion()) {
				return true;
			}
		}
	}
