...
void CityManagerImplementation::sendCityReport(CreatureObject* creature, const String& planetName, byte rank) {
	...
	report << "oid, mayor, cityhallid, regions, citizens, civicstructures, totalstructures, treasury, Loc, Next Update, next citizen Assessment (if pending)" << endl;
	...
	for (int i = 0; i < cities.size(); ++i) {
		...
		String mayorName = String::valueOf(city->getMayorID());
		...
		if (zoneServer != nullptr) {
			...
			if (mayorObject != nullptr && mayorObject->isPlayerCreature()) {
				...
				ManagedReference<CreatureObject*> mayor = mayorObject.castTo<CreatureObject*> ();
				mayorName = mayor->getFirstName();
			}
		}

		report << ", " << String::valueOf(city->getObjectID()) << ", " << mayorName;
...
void CityManagerImplementation::sendCityStatusReport(CreatureObject* creature, const String& planetName) {
	...
	report << "Max number of cities at rank 5 and above: " << String::valueOf(citiesAllowed->get(4)) << endl;
	report << "Max number of cities at rank 6: " << String::valueOf(citiesAllowed->get(5)) << endl;
...
void CityManagerImplementation::sendBannedReport(CityRegion* city, CreatureObject* creature, SceneObject* terminal) {
	PlayerObject* ghost = creature->getPlayerObject();
	...
	//Check if they already have the window open.
	if (ghost->hasSuiBoxWindowType(SuiWindowType::CITY_BANNED_REPORT))
		return;

	ManagedReference<SuiListBox*> listbox = new SuiListBox(creature, SuiWindowType::CITY_BANNED_REPORT);
	listbox->setPromptTitle("@city/city:banned_list_t");  //Banned Player Report
	listbox->setPromptText("@city/city:banned_list_d");   //The following is a list players who are banned from the city.
	listbox->setUsingObject(terminal);
	listbox->setForceCloseDistance(16.f);
	listbox->addMenuItem("@city/city:banned_player_prompt");  //Banned Players:

	CitizenList* bannedList = city->getBannedList();

	for (int i = 0; i < bannedList->size(); ++i) {
		ManagedReference<SceneObject*> player = zoneServer->getObject(bannedList->get(i));

		if (player != nullptr) {
			String name = "@city/city:default \t" + player->getDisplayedName();
