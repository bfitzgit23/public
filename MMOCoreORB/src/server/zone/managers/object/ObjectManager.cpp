...
void ObjectManager::registerObjectTypes() {
	...
	// Infinity:   Mabari/Padded Armor belts and some Bandoliers to still be clothing
...
SceneObject* ObjectManager::cloneObject(SceneObject* object, bool makeTransient) {
	...
	uint64 existingOid = object->getObjectID();
	...
	if (clonedObject == nullptr) {
		error("Count not clone object.");
		error("cloneObject - Bad CRC for OID = " + String::valueOf(existingOid) + ", CRC = 0x" + String::hexvalueOf(serverCRC) +  ", database = " + databaseName + ".db");
		return nullptr;
	}
...
Reference<DistributedObjectStub*> ObjectManager::loadPersistentObject(uint64 objectID) {
	...
	try {
		if (Serializable::getVariable<uint32>(serverObjectCrcHashCode, &serverObjectCRC, &objectData)) {
			...
			if (object == nullptr) {
				error("serverObjectCrcHashCode - could not load object from database");
				String JSONdump = "*** ERROR ***";

				if (Serializable::getVariable<String>(_classNameHashCode, &className, &objectData)) {			
					...
					if (pod != nullptr) {
						...
						try {
							...
							thisObject[String::valueOf(objectID).toCharArray()] = j;

							JSONdump = thisObject.dump();
				...
				String databaseName;
				database->getDatabaseName(databaseName);
				error("loadPersistentObject - Bad CRC for OID = " + String::valueOf(objectID) + ", CRC = 0x" + String::hexvalueOf(serverObjectCRC) +  ", database = " + databaseName + ".db");
				error("JSON = " + JSONdump);

				if (databaseName == "draftschematics") {
					error("Deleting schematic object with bad CRC from database");
					//database->deleteData(objectID);
				}

				if (databaseName == "sceneobjects" && JSONdump.contains("treasure_map_base")) {
					error("Deleting scene object with bad CRC from database");
					//database->deleteData(objectID);
				}
			...
			if (object == nullptr) {
				error("_classNameHashCode - could not load object from database");
				String JSONdump = "*** ERROR ***";
...
void ObjectManager::deSerializeObject(ManagedObject* object, ObjectInputStream* data) {
	...
	//	uint32 lastSaveCRC = managedObject->getLastCRCSave();
		...
		if (sceno) {
			...
			error() << "could not deserialize scene object of type: " << sceno->getGameObjectType() << " oid: " << sceno->getObjectID() << " from DB: " << dbName;
		} else {
...
SceneObject* ObjectManager::createObject(uint32 objectCRC, int persistenceLevel, const String& database, uint64 oid, bool initializeTransientMembers) {
	...
	if (object == nullptr) {
		...
		error("createObject - Bad CRC for OID = " + String::valueOf(oid) + ", CRC = 0x" + String::hexvalueOf(objectCRC) +  ", database = " + database + ".db");
		return nullptr;
	}
