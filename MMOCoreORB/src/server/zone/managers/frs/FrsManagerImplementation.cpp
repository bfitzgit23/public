...
void FrsManagerImplementation::initialize() {
	...
	if (miliDiff >= maintenanceInterval) {
		...
		StringBuffer msg;
		msg << "FRS Maintenance:  lastTick = " << lastTick << ", miliDiff = " << miliDiff << ", maintenanceInterval = " << maintenanceInterval << ".  Running FRS maintenance task NOW.";
		error(msg.toString());
		...
		StringBuffer msg;
		msg << "FRS Maintenance:  lastTick = " << lastTick << ", miliDiff = " << miliDiff << ", maintenanceInterval = " << maintenanceInterval;
		msg << ".  Scheduling maintenance task in " << (maintenanceInterval - miliDiff)/1000 << " seconds.";
		error(msg.toString());
	}
...
void FrsManagerImplementation::validatePlayerData(CreatureObject* player, bool verifyBan) {
	...
	if (realPlayerRank != curPlayerRank) {
		if (realPlayerRank == -1 && (councilType == COUNCIL_LIGHT || councilType == COUNCIL_DARK)) {
			...
			StringBuffer msg;
			msg << "validatePlayerData() - Rank mismatch for player " << player->getFirstName() << ".  Calling setPlayerRank";
			error(msg);
	...
	if (realPlayerRank >= 0 && (councilType == COUNCIL_LIGHT || councilType == COUNCIL_DARK)) {
		...
		// Infinity:  Guardian at rank 8
		if (realPlayerRank >= 8 && !player->hasSkill("force_title_jedi_rank_04"))
			player->addSkill("force_title_jedi_rank_04", true);
		// Infinity:  Master at rank 11
		if (realPlayerRank >= 11 && !player->hasSkill("force_title_jedi_master"))
...
void FrsManagerImplementation::setPlayerRank(CreatureObject* player, int rank) {
	...
	StringBuffer msg;
	msg << "setPlayerRank for " << player->getFirstName() << " ID: " << player->getObjectID() << " Current FRS Rank = " << curRank << " New FRS Rank = " << rank << " Current FRS XP = " << curExperience;
	error(msg);
...
void FrsManagerImplementation::removeFromFrs(CreatureObject* player) {
	...
	//Infinity:  Remove force progression and FRS XP
	auto zoneServer = this->zoneServer.get();
	if (zoneServer != nullptr) {

		SkillManager* skillManager = zoneServer->getSkillManager();
		if (skillManager != nullptr) {
			//Infinity:  Remove force progression
			if (player->hasSkill("force_title_jedi_master"))
				skillManager->surrenderSkill("force_title_jedi_master", player, true, false);

			if (player->hasSkill("force_title_jedi_rank_04"))
				skillManager->surrenderSkill("force_title_jedi_rank_04", player, true, false);

			if (player->hasSkill("force_title_jedi_rank_03"))
				skillManager->surrenderSkill("force_title_jedi_rank_03", player, true, false);
		...
		if (frsXP > 0) {
			...
			if (playerManager != nullptr)
				playerManager->awardExperience(player, "force_rank_xp", frsXP * -1, true, 1.0f, false);  //Don't use XP modifiers
		}
	}
	...
	//Infinity:   Why not do all these here?
...
void FrsManagerImplementation::handleSkillRevoked(CreatureObject* player, const String& skillName) {
	...
	if (skillName == "force_title_jedi_rank_03" || skillName == "force_rank_light_novice" || skillName == "force_rank_dark_novice") {
		removeFromFrs(player);
		return;
	}
	...
	if (skillRank < 0) {
		...
		if (skillRank == 1) {
			...
			if (frsXP > 9999) {
				...
				if (playerManager != nullptr)
					playerManager->awardExperience(player, "force_rank_xp", (frsXP - 9999) * -1, true, 1.0f, false);  //Don't use XP modifiers
			}
		}
...
void FrsManagerImplementation::updatePlayerSkills(CreatureObject* player) {
	...
	for (int i = 11; i >= 0; i--) {
		...
		if (playerRank >= rank) {
			...
			// Infinity:  8 and 11
			if (rank == 8 && !player->hasSkill("force_title_jedi_rank_04"))
				player->addSkill("force_title_jedi_rank_04", true);
			if (rank == 11 && !player->hasSkill("force_title_jedi_master"))
				player->addSkill("force_title_jedi_master", true);
		} else {
			if (player->hasSkill(rankSkill))
...
void FrsManagerImplementation::adjustFrsExperience(CreatureObject* player, int amount, bool sendSystemMessage) {
	...
	// Infinity:  Custom values and checks for autopromote
	...
	if (amount > 0) {
		...
		if (ghost->hasCappedExperience("force_rank_xp")) {//Infinity:  Is capped for rank 11?
		...
		if (councilType == COUNCIL_LIGHT) {
			...
			if (reqXp > curExperience) {
				...
				Core::getTaskManager()->scheduleTask([strongMan, strongRef] () {
					Locker locker(strongRef);
					strongMan->validateFrsExperience(strongRef, true);
				}, "ValidateFrsExperienceTask", 250);
			}
		...
		/* Infinity:   Recursively demote above
		int reqXp = rankingData->getRequiredExperience();
...
void FrsManagerImplementation::deductMaintenanceXp(CreatureObject* player) {
	...
	int frsXP = ghost->getExperience("force_rank_xp");
	if (rank == 0 && frsXP < 10000)
		return;

	// Infinity:  Custom login and visibility checks for increased maintenance
	uint64 currentTimeMili = Time().getMiliTime();
	...
	uint64 loginDelta = currentTimeMili - lastLoginTimeMili;
	...
	uint64 bountyDelta = currentTimeMili - lastBountyDeathMili;

	float visibility = ghost->getVisibility();

	bool loginPenalty = false;
	bool visibilityPenalty = false;	
	...
	if (!ghost->isOnline() && loginDelta > maintenanceInterval) { //Extra penalty if you haven't logged in during the past maintenance cycle
		loginPenalty = true;
		//error("FRS:  Login penalty for player " + player->getFirstName() + ". loginDelta = " + String::valueOf(loginDelta) + ", maintenance interval = " + String::valueOf(maintenanceInterval) + ".");
	}

	if (visibility < 1500 && bountyDelta > maintenanceInterval) {  //Extra penalty if visibility under 1500 (terminal threshold) and no BH death during maintenance interval
		visibilityPenalty = true;
		error("FRS:  Visibility penalty for player " + player->getFirstName() + ". visibility = " + String::valueOf(visibility) + 
			", lastBountyDeathMili = " + String::valueOf(lastBountyDeathMili) +
			", bountyDelta = " + String::valueOf(bountyDelta) +
			", maintenance interval = " + String::valueOf(maintenanceInterval) + ".");
	}

	int maintXp = 0;

	switch (frsXP) {
		case 10000 ... 19999:  // Rank 1
			maintXp = baseMaintCost * 1;
			break;
		case 20000 ... 29999:  // Rank 2
			maintXp = baseMaintCost * 2;
			break;
		case 30000 ... 64999:  // Rank 3
			maintXp = baseMaintCost * 3;
			break;
		case 65000 ... 149999:  // Rank 4
			maintXp = baseMaintCost * 4;
			break;
		case 150000 ... 224999:  // Rank 5
			maintXp = baseMaintCost * 5;
			break;
		case 225000 ... 324999:  // Rank 6
			maintXp = baseMaintCost * 6;
			break;
		case 325000 ... 449999:  // Rank 7
			maintXp = baseMaintCost * 7;
			break;
		case 450000 ... 599999:  // Rank 8
			maintXp = baseMaintCost * 8;
			break;
		case 600000 ... 774999:  // Rank 9
			maintXp = baseMaintCost * 9;
			break;
		case 775000 ... 999999:  // Rank 10
			maintXp = baseMaintCost * 10;
			break;
		case 1000000 ... 1250000:  // Rank 11
			maintXp = baseMaintCost * 11;
			break;
		default:
			maintXp = 0;
			error("Error in FRS XP for rank maintenance deduction.");						
			break;
	}

	if (loginPenalty || visibilityPenalty)  // 5X with penalty
		maintXp *= 5;
	...
	if (ConfigManager::instance()->getBool("Core3.FrsManager.ImmediateMaintXpDeduction", false)) {  //Immediate deduction

		Locker clocker(managerData, player);
		uint64 playerID = player->getObjectID();
		int curDebt = managerData->getExperienceDebt(playerID);

		// Infinity:  Custom email
		StringBuffer msg;

		if (curDebt <= 0) { //No Debt from previous configuration
			msg << "You have lost " << String::withCommas(maintXp) << " Force Rank experience. All members of Rank 1 or higher must pay experience each week to remain in their current positions." << endl;
		}
		else {
			msg << "You have lost " << String::withCommas(maintXp) << " Force Rank experience. This includes ";
			msg << String::withCommas(curDebt) << " previously banked experience debt.  All members of Rank 1 or higher must pay experience each week to remain in their current positions." << endl;
			managerData->removeExperienceDebt(playerID);
		}

		clocker.release();

		if (loginPenalty && visibilityPenalty) {
			msg << endl << "You did not login during the last week and did not meet visibility requirements, so your FRS maintenance has been increased by 5X.";
		}
		else if (loginPenalty) {
			msg << endl << "You did not login during the last week, so your FRS maintenance has been increased by 5X.";
		}
		else if (visibilityPenalty) {
			msg << endl << "You did not meet visibility requirements, so your FRS maintenance has been increased by 5X.";
		}

		if (chatManager != nullptr)
			chatManager->sendMail("Enclave Records", "@force_rank:xp_maintenance_sub", msg.toString(), player->getFirstName());
		...
		// Infinity:  Custom email
		StringBuffer msg;
		msg << "You have lost " << String::withCommas(maintXp) << " Force Rank experience. All members of Rank 1 or higher must pay experience each week to remain in their current positions.";
		msg << "  (Note: This loss may not take effect until your next login.)" << endl;
...
int FrsManagerImplementation::getRequiredXP(int councilType, int rank) {
	...
	if (councilType == COUNCIL_LIGHT)
		...
		rankingData = darkRankingData.get(rank);
	if (rankingData == nullptr) {
		error("getRequiredXP - rankingData = nullptr");
		return 0;
	}
...
void FrsManagerImplementation::validateFrsExperience(CreatureObject* player, bool sendSystemMessage) {
	if (player == nullptr)
		return;
	...
	if (councilType == COUNCIL_LIGHT) {
		...
		if (reqXp > curExperience) {
			StringBuffer msg;
			msg << "validateFrsExperience() - Demote Light Jedi " << player->getFirstName() << " ID: " << player->getObjectID() << " Current FRS Rank: " << rank <<  " Current FRS XP: " << curExperience;
			error(msg);
			demotePlayer(player);
		...
		if (reqXp > curExperience) {
			StringBuffer msg;
			msg << "validateFrsExperience() - Demote Dark Jedi " << player->getFirstName() << " ID: " << player->getObjectID() << " Current FRS Rank: " << rank <<  " Current FRS XP: " << curExperience;
			error(msg);
			demotePlayer(player);
