...
#ifdef DEBUG_GROUPS
	info(true) << "Player: " << creature->getDisplayedName() << " joining group of " << leader->getDisplayedName();
#endif
...
void GroupManager::createGroup(CreatureObject* leader, CreatureObject* creature) {
	...
	if (leaderGhost != nullptr)	{
		String typeSelected = leaderGhost->getScreenPlayData("group_message_selection", "typeSelected");
		typeSelected = (typeSelected == "") ? "All" : typeSelected;
		leader->sendSystemMessage("You are currently receiving the following harvest and loot group system messages:  " + typeSelected);
		}
	...
	if (creature->isPlayerCreature()) {
		...
		if (playerGhost != nullptr) {
			//Infinity:  Group message selection code
			String typeSelected = playerGhost->getScreenPlayData("group_message_selection", "typeSelected");
			typeSelected = (typeSelected == "") ? "All" : typeSelected;
			creature->sendSystemMessage("You are currently receiving the following harvest and loot group system messages:  " + typeSelected);
...
void GroupManager::leaveGroup(ManagedReference<GroupObject*> group, CreatureObject* creature) {
	// Pre: player locked
	// Post: player locked
	if (group == nullptr || creature == nullptr)
		return;
...
void GroupManager::disbandGroup(ManagedReference<GroupObject*> group, CreatureObject* player) {
	...
	if (group == nullptr || player == nullptr)
		return;
...
void GroupManager::kickFromGroup(ManagedReference<GroupObject*> group, CreatureObject* player, CreatureObject* memberToKick) {
	...
	if (group == nullptr || player == nullptr || memberToKick == nullptr)
		return;
...
void GroupManager::changeLootRule(GroupObject* group, int newRule) {
	...
	void GroupManager::createLottery(GroupObject* group, AiAgent* corpse) {
		...
		ManagedReference<SceneObject*> lootContainer = corpse->getSlottedObject("inventory");
		...
		for (int i = 0; i < lootContainer->getContainerObjectsSize(); ++i) {
			ManagedReference<SceneObject*> lootItem = lootContainer->getContainerObject(i);
			if (lootItem == nullptr)
				continue;
	...
	void GroupManager::transferLoot(GroupObject* group, CreatureObject* winner, SceneObject* object, bool stillGrouped) {
		...
		//Transfer the item to the winner.
		ManagedReference<SceneObject*> winnerInventory = winner->getSlottedObject("inventory");
