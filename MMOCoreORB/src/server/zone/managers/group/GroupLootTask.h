...
public:
	...
	void run() {
		...
		if (lootAll) {
			...
			if (group->getLootRule() == GroupManager::MASTERLOOTER && lootContainer->getContainerObjectsSize() <= 0) {
				...
				group->sendSystemMessage(msg, true, 2);   //Infinity:  notifyLeader = true, messageType 1 = Harvest, 2 = Loot
			}
		} else {
			gclocker.release();
	...
	void splitCredits() {
		...
		group->sendSystemMessage(lootMember, player, 2);   //Infinity:  messageType 1 = Harvest, 2 = Loot
		...
		for (int i = 0; i < payees.size(); ++i) {
			ManagedReference<CreatureObject*> payee = payees.get(i);
