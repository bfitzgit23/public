...
Vector<WorldCoordinates>* PathFinderManager::findPathFromWorldToCell(const WorldCoordinates& pointA, const WorldCoordinates& pointB, Zone *zone) {
	...
	if (pathToCell == nullptr) {
		...
		error() << "getPath (worldtocell) from " << exteriorNode << " to " << nearestInteriorNode
			   	<< " is nullptr for building " << templateObject->getFullTemplateString() << " OID: " << (building != nullptr ? String::valueOf(building->getObjectID()) : "?")
				<< " from " << pointA << " to " << pointB << " in zone " << zoneName;
...
Vector<WorldCoordinates>* PathFinderManager::findPathFromCellToWorld(const WorldCoordinates& pointA, const WorldCoordinates& pointB, Zone *zone) {
	...
	if (exitPath == nullptr) {
		...
		error() << "getPath getPath (celltoworld) from " << exitNode << " to " << exteriorNode
				<< " exitpath is nullptr for building " << templateObject->getFullTemplateString() << " OID: " << (building != nullptr ? String::valueOf(building->getObjectID()): "?")
				<< " from " << pointA << " to " << pointB << " in zone " << zoneName;
...
Vector<WorldCoordinates>* PathFinderManager::findPathFromCellToDifferentCell(const WorldCoordinates& pointA, const WorldCoordinates& pointB) {
	...
	if (nodes == nullptr) {
		...
		float x = building1 != nullptr ? building1->getWorldPositionX() : 0;
		float y = building1 != nullptr ? building1->getWorldPositionY() : 0;

		log() << "getPath (celltodifferentcell) Could not find path from " << source
				<< " to " << target << " in building: "
				<< templateObject->getFullTemplateString() 
				<< " Building OID: " << (building1 != nullptr ? String::valueOf(building1->getObjectID()) : "?")
				<< " Source cell OID: " << (ourCell != nullptr ? String::valueOf(ourCell->getObjectID()) : "?")
				<< " Destination cell OID: " << (targetCell != nullptr ? String::valueOf(targetCell->getObjectID()) : "?")
				<< " from " << pointA << " to " << pointB << " in zone " << zoneName
				<< " World coords:  " << x << ", " << y;
	...
	if (nodes->size() == 1) {
		...
		error() << "getPath (celltodifferentcell) nodes->size() == 1 from " << source << " to " << target
...
Vector<WorldCoordinates>* PathFinderManager::findPathFromCellToCell(const WorldCoordinates& pointA, const WorldCoordinates& pointB) {
	...
	if (res == -1) {
		...
		if (trianglePath != nullptr) {	//Infinity:  This should prevent memory leak
			delete trianglePath;
			trianglePath = nullptr;
		}
