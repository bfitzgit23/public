...
void ResourceSpawner::shiftResources() {
	...
	dumpResources();
	ghDumpAll();  //Infinity:  Write allresources to Galaxy Harvester output file
}
...
void ResourceSpawner::sendSampleResults(TransactionLog& trx, CreatureObject* player, const float density, const String& resname) const {
	...
	// Lower skill levels mean you can't sample lower concentrations
	...
	int unitsExtracted = maxUnitsExtracted * (float(surveySkill) / 100.0f) * samplingMultiplier * cityMultiplier * 1.8f;  //Infinity: increase, previously 2.0, reduced by 10%
	int xpcap = 100;  //Infinity: 40 to 100
	...
	if (session->tryGamble()) {
		...
		session->clearGamble();
		xpcap = 150;  //Infinity:  Increased cap, but mini-game is disabled.
	}
