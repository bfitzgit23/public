...
void DnaManager::generationalSample(PetDeed* deed, CreatureObject* player, int quality) {
	...
	//Infinity:  Not using creature emplate of clone
...
#ifdef DEBUG_GENERATION_SAMPLE
	//info(true) << "Ferocity: " << ferocity;
	info(true) << "Creature Level: " << creatureLevel;
	...
	//Infinity:  Save source name for custom object name
	String sourceName = deed->getTemplateName();
	prototype->setSource(sourceName);

	String displayName = StringIdManager::instance()->getStringId(sourceName.hashCode()).toString();

	if (displayName.beginsWith("a ")) {
		displayName = displayName.subString(2,displayName.length());
	}
	else if (displayName.beginsWith("an ")) {
	...
	//Infinity:  Custom object name
	prototype->setCustomObjectName(qualityPrefix + " DNA Sample: " + displayName + " (CL" + String::valueOf(creatureLevel) + ")", false);

	//Infinity:  Set Quality and Creature level
...
void DnaManager::generateSample(Creature* creature, CreatureObject* player, int quality){
	...
	int endurance = Genetics::randomizeValue(750 * creatureLevel / 95.0f, quality); //Infinity
	...
	if (nameId->getFile().isEmpty() || nameId->getStringID().isEmpty()) {
		sourceName = creature->getCreatureName().toString();
	}
	else {
		sourceName = nameId->getFullPath();
	}
	prototype->setSource(sourceName);  //Infinity
	...
	switch (type) {
		case HIT_LEVEL:
			...
			for (int i = 0; i < dnaHit.size(); i++) {
				float lvminus = 100.0f, lvplus = 300.0f;
		...
		case DPS_LEVEL:
			for (int i = 0; i < dnaDPS.size(); i++) {
				...
				if (value >= (lvminus + lv) / 2.00f && value <= (lvplus + lv) / 2.00f) {
					rc = i;
					break;
				}
			}
