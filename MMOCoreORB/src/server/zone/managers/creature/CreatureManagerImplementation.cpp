...
CreatureObject* CreatureManagerImplementation::spawnCreature(uint32 templateCRC, float x, float z, float y, uint64 parentID, float directionDegrees) {
	...
	placeCreature(creature, x, z, y, parentID, directionDegrees);
...
SceneObject* CreatureManagerImplementation::spawnLair(unsigned int lairTemplate, int difficultyLevel, int difficulty, float x, float z, float y, float size) {
	...
	const VectorMap<String, int>* mobiles = lairTmpl->getMobiles();  //Infinity:  Using mobiles vector map instead
	...
	//Infinity:  Custom lair condition based on highest CL mob in lair
	int highestCL = 0;
	for (int i = 0; i < mobiles->size(); i++) {

		String templateName = mobiles->elementAt(i).getKey();
		CreatureTemplate* creatureTemplate = CreatureTemplateManager::instance()->getTemplate(templateName);

		if (creatureTemplate == nullptr)
			continue;
		int currentCL = creatureTemplate->getLevel();
		if (currentCL > highestCL) {
			highestCL = currentCL;
		}
	}

	int condition = 0;
	if (highestCL > 100) {  //Decrease the condition increase over 100 CL
		condition = (100 * (1000 + System::random(250))) + ((highestCL - 100) * (500 + System::random(250)));
	}
	else if (highestCL > 0 && highestCL < 100) {
		condition = highestCL * (1000 + System::random(250));
	}
	else { //Couldn't find a CL, use default method
		error("Couldn't find a mob CL in lair template.");
		condition = difficultyLevel * (1000 + System::random(200));
	}
	...
	building->setMaxCondition(condition);  //Infinity:  Custom lair condition based on highest CL mobile (not boss)	
...
CreatureObject* CreatureManagerImplementation::spawnCreatureAsBaby(uint32 templateCRC, float x, float z, float y, uint64 parentID, float directionDegrees) {
	CreatureTemplate* creoTempl = creatureTemplateManager->getTemplate(templateCRC);
	...
	//error("Spawning a baby of type " + templateToSpawn + ", templateCRC = " + String::hexvalueOf(templateCRC) + ", objectCRC = " + String::hexvalueOf(objectCRC));
	//error("x = " + String::valueOf(x) + ", z = " + String::valueOf(z) + ", y = " + String::valueOf(y));
	...
	//error("x = " + String::valueOf(x) + ", z = " + String::valueOf(z) + ", y = " + String::valueOf(y) + ", parentID = " + String::valueOf(parentID) + ", creO = " + String::valueOf(creO));
	...
	//error("Final direction before placement = " + String::valueOf(directionDegrees));

	placeCreature(creO, x, z, y, parentID, directionDegrees);

	//creO->updateDirection(Math::deg2rad(directionDegrees));  

	//error("Final direction after placement = " + String::valueOf(creO->getDirectionAngle()));

	return creO;
}

CreatureObject* CreatureManagerImplementation::spawnCreatureAsEventMob(uint32 templateCRC, int level, float x, float z, float y, uint64 parentID, float directionDegrees) {
	CreatureTemplate* creoTempl = creatureTemplateManager->getTemplate(templateCRC);
...
CreatureObject* CreatureManagerImplementation::spawnCreature(uint32 templateCRC, uint32 objectCRC, float x, float z, float y, uint64 parentID, bool persistent, float directionDegrees) {
	CreatureTemplate* creoTempl = creatureTemplateManager->getTemplate(templateCRC);
...
void CreatureManagerImplementation::placeCreature(CreatureObject* creature, float x, float z, float y, uint64 parentID, float directionDegrees) {
	if (creature == nullptr)
		return;
	...
	if (creature->isAiAgent()) {
		...
		aio->setHomeLocation(x, z, y, cellParent, directionDegrees);
		aio->setNextStepPosition(x, z, y, cellParent);
	}

	creature->initializePosition(x, z, y);
	creature->setDirection(Math::deg2rad(directionDegrees));
...
int CreatureManagerImplementation::notifyDestruction(TangibleObject* destructor, AiAgent* destructedObject, int condition, bool isCombatAction) {
	...
	try {
		...
		//Infinity:  Reworked to remove the destructed object as defender from attackers in threatmap
		for (int i = 0; i < copyThreatMap.size(); ++i) {
			...
			if (attacker->isPlayerCreature() && observers.size() > 0) {
				attackerCreo->notifyObservers(ObserverEventType::QUESTKILL, destructedObject);
			}

			if (attacker == destructor || !(attackerCreo->isPlayerCreature() || attackerCreo->isPet())) {  //Infinity:  Don't check destructor they are handled later.  Only check players/pets.
				continue;
			}

			ManagedReference<CreatureObject*> strongAttacker = attackerCreo;
			ManagedReference<AiAgent*> strongDestructed = destructedObject;

			//Logger::console.error("Scheduling RemoveDefenderAndClearCombatTask for attacker " + attackerCreo->getDisplayedName());

			Core::getTaskManager()->scheduleTask([strongAttacker, strongDestructed] () {

				if (strongAttacker == nullptr || strongDestructed == nullptr) {
					return;
				}

				Locker locker(strongAttacker);
				if (strongAttacker->hasDefender(strongDestructed)) {

					//Logger::console.error("Removing defender " + strongDestructed->getDisplayedName() + " from attacker " + strongAttacker->getDisplayedName());
					strongAttacker->removeDefender(strongDestructed);

					// Finally if the attacker has no more defenders, clear their combat state
					if (!strongAttacker->hasDefenders()) {
						//Logger::console.error("Clearing combat state on attacker " + strongAttacker->getDisplayedName());
						strongAttacker->clearCombatState(false);
					}
				}
			}, "RemoveDefenderAndClearCombatTask", 200);
		}
			...
			if (player->isPlayerCreature()) {
				if (!destructedObject->isEventMob()) {
					if (player->isGrouped()) {
						...
						if (group != nullptr) {
							for (int i = 0; i < group->getGroupSize(); i++) {
								...
								if (groupMember->isPlayerCreature()) {
									...
									groupMember->notifyObservers(ObserverEventType::KILLEDCREATURE, destructedObject, destructorObjectID);
								}
							}
						}
					}
						...
						player->notifyObservers(ObserverEventType::KILLEDCREATURE, destructedObject, destructorObjectID);
					}
				}

				FactionManager* factionManager = FactionManager::instance();

				if (factionManager != nullptr && !destructedObject->getFactionString().isEmpty() && !destructedObject->isEventMob()) {
						...
						if (group != nullptr) {
							...
							for (int i = 0; i < group->getGroupSize(); i++) {
								...
								if (groupMember != nullptr && groupMember->isPlayerCreature() ) {
									...
									if (groupMember->isInRange(destructedObject, 128.0f) && copyThreatMap.contains(groupMember.get())) {
		...
		bool dnaDeath = false;  //Infinity:   Let's check for DNA death and not award XP in that case
		if (destructedObject->isCreature()) {
			ManagedReference<Creature*> destructedCreature = cast<Creature*>(destructedObject);
			if (destructedCreature != nullptr && destructedCreature->getDnaState() == DNADEATH)
				dnaDeath = true;
		}

		if (!dnaDeath && playerManager != nullptr)
		...
		shouldRescheduleCorpseDestruction = playerManager != nullptr && playerManager->shouldRescheduleCorpseDestruction(player, destructedObject);
	} catch (...) {
...
void CreatureManagerImplementation::droidHarvest(Creature* creature, CreatureObject* droid, int selectedID, int harvestBonus) {
	...
	quantity = Math::max(quantity, 100.0f); //Infinity:  Minimum 100 units of meat, hide, or bone

	int ownerSkill = owner->getSkillMod("creature_harvesting");
	int quantityExtracted = int(quantity * ownerSkill / 100.0f);
	...
	if (owner->isGrouped()) {
		...
		if (group != nullptr) {
			...
			modifier = group->getGroupHarvestModifier(owner);
	...
	if (owner->isGrouped()) {
		...
		if (group != nullptr) {
			...
			group->broadcastMessage(owner, sysMessage, true, 1);	//Infinity:  messageType 1 = Harvest, 2 = Loot		
		}
	}
	...
	if (!creature->hasLoot() && creature->getBankCredits() < 1 && creature->getCashCredits() < 1 && playerManager != nullptr && !playerManager->canGroupMemberHarvestCorpse(owner, creature)) {
...
void CreatureManagerImplementation::harvest(Creature* creature, CreatureObject* player, int selectedID) {
	...
	//Infinity:  Increase harvesting ranged based on creature template radius
	float adjustedRange = 7.0f + Math::min(creature->getTemplateRadius() / 1.5f, 18.0f);
	if (!player->isInRange(creature, adjustedRange))
		return;
	...
	if (player->isGrouped()) {
		...
		ManagedReference<GroupObject*> group = player->getGroup();
		if (group != nullptr) {
			modifier = group->getGroupHarvestModifier(player);
			quantityExtracted = (int)(quantityExtracted * modifier);
		}
	}
	...
	if (player->isGrouped()) {
		...
		if (group != nullptr) {
			...
			ChatSystemMessage* sysMessage = new ChatSystemMessage(bonusMessage);
			group->broadcastMessage(player, sysMessage, false, 1);	//Infinity:  messageType 1 = Harvest, 2 = Loot	
	...
	if (!creature->hasLoot() && creature->getBankCredits() < 1 && creature->getCashCredits() < 1 && playerManager != nullptr && !playerManager->canGroupMemberHarvestCorpse(player, creature)) {
		Reference<DespawnCreatureTask*> despawn = creature->getPendingTask("despawn").castTo<DespawnCreatureTask*>();
...
void CreatureManagerImplementation::tame(Creature* creature, CreatureObject* player, bool force, bool adult) {
	...
	auto playerManager = zoneServer->getPlayerManager();
	if (playerManager != nullptr) {
		maxStoredPets += playerManager->getBaseStoredCreaturePets();
	}
	...
	if (zoneServer->getGalaxyName() != "SWG Infinity") { // Infinity:  Let's making taming easy on TC
		force = true;
	}
	...
	uint32 pvpMask = creature->getPvpStatusBitmask();							//Infinity:  Save original pvpStatusBitamsk in case it was changed by screenplays, etc.

	uint32 creatureMask = creature->getCreatureBitmask();							//Infinity:  Save original bitmask in case it was changed by screenplays, etc.
	...
	Reference<TameCreatureTask*> task = new TameCreatureTask(creature, player, pvpMask, creatureMask, force, adult);
