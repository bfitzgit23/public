...
void PetManagerImplementation::handleChat(CreatureObject* speaker, AiAgent* pet, const String& message) {
	...
	/* Infinity:  1 second cooldown on tellpet
	if (!pet->checkCooldownRecovery("tellpet"))
		return;
	pet->updateCooldownTimer("tellpet", 1000); // 1 second
	*/
...
bool PetManagerImplementation::handleCommandTraining(CreatureObject* speaker, AiAgent* pet, const String& message) {
	...
	auto zoneServer = speaker->getZoneServer();  //Infinity:  Let's get this for later and nullcheck it
	...
	if (pcd->hasTrainedCommandString(message)) {
		if (pet->getOptionsBitmask() & OptionBitmask::CONVERSE) {
			...
			zoneServer->getChatManager()->broadcastChatMessage(pet, chat, 0, 0, 0);
		} else {
	...
	if (petType == CREATUREPET) {
		...
		if (!alreadyTrained) {
			...
			int roll = System::random(skill + level/5.0f);  //Infinity: Level / 5 to increase change of successful training attempt

			if (skill > roll || zoneServer->getGalaxyName() != "SWG Infinity")  //Infinity:  Train command on first try on lower environments
				success = true;
	...
	if (numberOfSubStrings > 1) {
		...
		if (nameManager->validateName(parsedName) != NameManagerResult::ACCEPTED) {
			speaker->sendSystemMessage("Pet name was rejected by the name filter.");
			return true;
		}
...
void PetManagerImplementation::enqueuePetCommand(CreatureObject* player, AiAgent* pet, uint32 command, const String& args, bool selfTarget) {
	...
	// CreatureObject* pet, uint32 command, const String& args, uint64 target, int priority = QueueCommand::NOCOMBATQUEUE (-1)
