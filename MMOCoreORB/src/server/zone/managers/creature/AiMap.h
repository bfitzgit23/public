...
public:
	...
	Behavior* getTemplate(uint32 bitMask, BehaviorTreeSlot treeID, uint64 customMapName = 0) {
		...
		for (uint32 mask = CreatureFlag::LASTAIMASK; ; mask = mask >> 1) {  //Changed loop and condition to use all 32 bits
				
			//Logger::console.error("Mask = 0x" + String::hexvalueOf(mask));	
