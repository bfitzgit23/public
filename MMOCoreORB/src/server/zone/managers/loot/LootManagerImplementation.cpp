...
void LootManagerImplementation::initialize() {
	...
	info("Loaded " + String::valueOf(lootableJediMods.size()) + " lootable jedi stat mods.");   //Infinity:  Custom Jedi mods
	info("Loaded " + String::valueOf(lootableRareMods.size()) + " lootable rare stat mods.");   //Infinity:  Custom tape group
...
bool LootManagerImplementation::loadConfigData() {
    ...
    auto lootChanceMultiplier = ConfigManager::instance()->getFloat("Core3.LootManager.LootChanceMultiplier", 1.0f);
	yellowChance = lua->getGlobalFloat("yellowChance") / lootChanceMultiplier;
	yellowModifier = lua->getGlobalFloat("yellowModifier");
	exceptionalChance = lua->getGlobalFloat("exceptionalChance") / lootChanceMultiplier;
	exceptionalModifier = lua->getGlobalFloat("exceptionalModifier");
	legendaryChance = lua->getGlobalFloat("legendaryChance") / lootChanceMultiplier;
	...
	modsTable = lua->getGlobalObject("lootableJediStatMods");  //Infinity:  Custom Jedi mods
	loadLootableMods( &modsTable, &lootableJediMods );

	modsTable = lua->getGlobalObject("lootableRareStatMods");  //Infinity:  Custom tape group
	loadLootableMods( &modsTable, &lootableRareMods );
...
TangibleObject* LootManagerImplementation::createLootObject(TransactionLog& trx, const LootItemTemplate* templateObject, int level, bool maxCondition) {
	...
	if (System::random(legendaryChance) >= legendaryChance - adjustment) {
		...
		if (!prototype->isAttachment()) { // Don't add exceptional/legendary text to SEAs, will screw up the name
	...
	if (prototype->isLightsaberCrystalObject()) {
		...
		if (crystal != nullptr) {
			...
			//Infinity:  Enable rare modifications to crystals
	...
	for (int i = 0; i < craftingValues->getTotalExperimentalAttributes(); ++i) {
		...
		if (attribute == "maxrange" || attribute == "midrange" || attribute == "zerorangemod" || attribute == "maxrangemod" || attribute == "forcecost" || attribute == "color") {//Infinity: Don't roll on color crystal color
			continue;
		}
		...
		if (attribute == "useCount" || attribute == "quantity" || attribute == "charges" || attribute == "uses" || attribute == "charge") { //Infinity:  Let's adjust these values by yellow modifier only

			if (excMod > 1.0f) { // Only adjust these attributes by yellow modifier
				min *= yellowModifier;
				max *= yellowModifier;
			}	
		}
...
String LootManagerImplementation::getRandomLootableMod( unsigned int sceneObjectType, const String& lootTemplateName) {//Infinity:  Support for custom Jedi skill mod attachments
	...
	//Infinity:
	// Infinity: Copying how clothing attachments work below
	else if( sceneObjectType == SceneObjectType::ARMORATTACHMENT && lootTemplateName == "attachment_rare_armor"){
		return lootableRareMods.get(System::random(lootableRareMods.size() - 1));
	}
	...
	// Infinity:  Here we piggyBack off how clothingAttachments work currently in order to make custom clothingAttachment group!
	...
	// Infinity:  doing the same as jedi tapes above for this new group
	else if( sceneObjectType == SceneObjectType::CLOTHINGATTACHMENT && lootTemplateName == "attachment_rare_clothing"){
		return lootableRareMods.get(System::random(lootableRareMods.size() - 1));
	}
...
TangibleObject* LootManagerImplementation::createLootAttachment(const LootItemTemplate* templateObject, const String& skillMod, int skillModValue, bool ignoreChecks) {

	if (!ignoreChecks) {
		//SEA Value range checks
		if (skillModValue > 25) {
			skillModValue = 25;
		}
		else if (skillModValue < 1) {
			skillModValue = 1;
		}
	}
	...
	if (prototype == nullptr || !prototype->isAttachment()) {
		error("Could not create loot attachment object: " + directTemplateObject);
		return nullptr;
	}
	...
	if (attachment->isArmorAttachment()) {
		if (!ignoreChecks && !isValidAttachment("AA", skillMod)) {
			Logger::console.error("Invalid AA: " + skillMod);
			return nullptr;
		}
		attachmentType = "[AA] ";
	}
	else if (attachment->isClothingAttachment()) {
		if (!ignoreChecks && !isValidAttachment("CA", skillMod)) {
			Logger::console.error("Invalid CA: " + skillMod);
			return nullptr;
		}
		attachmentType = "[CA] ";
	}
	...
	// Infinity:   We want to override the UpdateCraftingValues here for Attachments to enable us
	// to create new specific loot groups for attachments!
	attachment->updateAttachmentValues(skillMod, skillModValue);
	...
	for(int i = 0; i < mods->size(); ++i) {
		...
		if (currentValue > highest){
			...
			attachmentName.setStringId("stat_n", key);
			prototype->setObjectName(attachmentName, false);
			String displayedName = prototype->getDisplayedName() != "" ? prototype->getDisplayedName() : skillMod;
			attachmentCustomName = attachmentType + displayedName + " " + String::valueOf(currentValue);
		}
	}
	prototype->setCustomObjectName(attachmentCustomName, false);
