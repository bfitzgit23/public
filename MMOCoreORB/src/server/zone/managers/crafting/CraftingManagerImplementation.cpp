...
int CraftingManagerImplementation::calculateExperimentationSuccess(CreatureObject* player,
	...
	bool testAmazing = false; //Infinity:  For test servers
	auto zoneServer = player->getZoneServer();
	if (zoneServer != nullptr) {
		String galaxyName = zoneServer->getGalaxyName();
		if (galaxyName != "SWG Infinity") { //Infinity: Amazing with crafting food/drink on test servers
			testAmazing = (expbonus > 0);
		}
	}

	if((luckRoll > ((95 - expbonus) - forceSkill) || testAmazing))
