...
/*	Infinity:  Allow creatures to inherit AOE specials from DNA
	if (effectiveSpecial.contains("creature"))
		effectiveSpecial = "defaultattack";  
*/	
	int roll = System::random(2000);
	// roll now determined by template quality
	// we roll 0-2000 if that number is < quality * 100 i.e. VHQ 100 VLQ 700 if we get less than the odds we don't stick the special
	// VHQ has a 95% chance to stick the special, VLQ has a 65% chance. 
	if (roll < odds ) {
		effectiveSpecial = "defaultattack";
	}
...
void GeneticLabratory::recalculateResistances(CraftingValues* craftingValues, float fortDiff) {
	...
	bool lightsaberSpec = (craftingValues->getMinValue("lightsabereffectiveness") == 1);
	...
	if (!kineticSpec) {
		newValue = (!armorReset ? Math::min((craftingValues->getCurrentValue("dna_comp_armor_kinetic") + effectivenessBonus), 80.f) : 0.f);
	...
	if (!energySpec) {
		newValue = (!armorReset ? Math::min((craftingValues->getCurrentValue("dna_comp_armor_energy") + effectivenessBonus), 80.f) : 0.f);
	...
	if (!lightsaberSpec) {
		newValue = (!armorReset ? Math::min((craftingValues->getCurrentValue("dna_comp_armor_lightsaber") + effectivenessBonus), 35.f) : 0.f);

#ifdef DEBUG_GENETIC_LAB
		info(true) << "Lightsaber Current: " << craftingValues->getCurrentValue("dna_comp_armor_lightsaber") << " Lightsaber New: " << newValue;
#endif

		craftingValues->setCurrentValue("dna_comp_armor_lightsaber", newValue);
	}
...
void GeneticLabratory::setInitialCraftingValues(TangibleObject* prototype, ManufactureSchematic* manufactureSchematic, int assemblySuccess) {
...
#ifdef DEBUG_GENETIC_LAB
	info(true) << "AssemblySuccess = " << assemblySuccess << ", Assembly Modifier: " << modifier;
#endif
	...
	*/
	...
	float currentPercentage = 0.0f, maxPercentage = 0.0f, maxValue = 1000.0f, capValue = 0.0f, initialValue = 0.0f, rangeValue = 0.0f, fortitude = 0.0f;
	bool hidden = false;
...
#ifdef DEBUG_GENETIC_LAB
		info(true) << "Current Percentage: " << currentPercentage * 100.0f << " Max Percentage: " << 100.f;
...
#ifdef DEBUG_GENETIC_LAB
	info(true) << "Experimentation Result = " << experimentationResult << ", Experimentation Modifier: " << modifier;
#endif
