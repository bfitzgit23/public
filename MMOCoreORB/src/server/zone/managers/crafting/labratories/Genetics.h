...
public:
	...
	const static constexpr float LIGHTSABER_MAX = 35.f;

	// Hardiness and Fortitude
	...
	*/
	...
	static float initialValue(float maxValue) {
		return maxValue * 0.74f;  // Infinity
	}
	...
	static int damageToValue(float dps, int quality) {
		...
		// Infinity:  
		int base = round(((dps - 15.0) / (735.0)) * 1000.0);
		return randomizeValue(base, quality);
	}
	...
	static int ferocityToValue(int level, int quality) {
		...
		// Infinity:  Ferocity value
		int base = level * 100 + System::random(5);
		return randomizeValue(base, quality);
	}

	// convert acceleration
	static int accelerationToValue(float speed, int quality) {
		int base = round(((speed - 1.0) / (4.0)) * 1000.0 * 2.0);
		return randomizeValue(base, quality);
	}
	...
	static int calculatePetLevel(GeneticComponent* pet) {
		...
		float levelFloat = ((statLevel + regenerationLevel + damageLevel/6.0f + hitLevel/3.0f + defenseLevel/3.0f + armorLevel/5.0f) / 13.0f) - 11.2f;
	...
	static int calculateAgentLevel(int health, float dps, float hit, int regen, int armor, float effective, float kin, float eng, float bla, float heat, float cold, float elec, float acid, float stun, float lightsaber) {
		float statLevel = generateStatLevel(health);
		float damageLevel = generateDamageLevel(dps);
		float hitLevel = generateHitLevel(hit);
		float defenseLevel = hitLevel;
		float regenerationLevel = generateRegenLevel(regen);
		float armorLevel = generteArmorLevel(armor, effective);
		float armorBase = DnaManager::instance()->valueForLevel(DnaManager::ARM_LEVEL, armorLevel);
		float baseLevel = (((statLevel) + (damageLevel) + (regenerationLevel) + (hitLevel)) / 19.0f) + 0.5;
	...
	static int calculateArmorValue(GeneticComponent* pet, int armorLevel, int baseLevel, int armorBase) {
		...
		resistanceLevel += resistMath(pet->getSaber(), armorRating, eff, false, 2.0, 1);
	...
	static float calculateDPSLevel(float maxDamage, float minDamage) {
		float dps = (maxDamage + minDamage) / 2.0f;
	...
	static int calcArmorLevelByStats(int armorRating, int armorLevel, int baseLevel, int armorBase, float kinetic, float energy, float blast, float heat, float cold, float electricity, float acid, float stun, float lightsaber) {
		int level = armorLevel + 1;
		if (level < baseLevel)
			level = baseLevel;
		int eff = armorBase / 50;
		...
		resistanceLevel += resistMath(lightsaber, armorRating, eff, false, 2.0, 1);
		level += ((float)resistanceLevel) / 10.0;
		return level;
	}

	static float generateStatLevel(int health) {
		return (DnaManager::instance()->levelForScore(DnaManager::HAM_LEVEL, health) + 1) * 2.0f; //Infinity:  6.0 to 2.0
	}
