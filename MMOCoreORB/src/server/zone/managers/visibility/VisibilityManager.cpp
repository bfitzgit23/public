...
void VisibilityManager::decreaseVisibility(CreatureObject* creature) {
	...
	if (ghost != nullptr) {
		...
		int startingVisibility = ghost->getVisibility();

		if (startingVisibility > 0) {
			...
			if (ghost->getVisibility() <= visibilityDecrease) {
				...
				ghost->setVisibility(startingVisibility - visibilityDecrease);
			}
		}
	}
}
...
void VisibilityManager::clearVisibility(CreatureObject* creature) {
	...
	if (ghost != nullptr  && !ghost->hasGodMode()) {
		//info("[clear visibility] Clearing visibility for player " + String::valueOf(creature->getObjectID()), true);
