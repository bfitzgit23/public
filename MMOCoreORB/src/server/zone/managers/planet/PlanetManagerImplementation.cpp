...
Reference<SceneObject*> PlanetManagerImplementation::loadSnapshotObject(WorldSnapshotNode* node, WorldSnapshotIff* wsiff, int& totalObjects) {
	...
	if (object == nullptr) {
		...
		msg << "\n";
		msg << "*********************************************************************************************************************************************************************" << "\n";
		msg << "Error loading snapshot object with template " << serverTemplate << ", hashcode = 0x" << String::hexvalueOf(serverTemplate.hashCode()) << ".\n";
		msg << "Additional information will likely be available from the zoneserver attempting to load the object.\n";
		msg << "*********************************************************************************************************************************************************************";
		error(msg.toString());
		return nullptr;
	}
...
bool PlanetManagerImplementation::validateClientCityInRange(CreatureObject* creature, float x, float y) {
	...
	for (int i = 0; i < regionMap.getTotalCityRegions(); ++i) {
		...
		for (int j = 0; j < cityRegion->getRegionsCount(); ++j) {
			...
			// Infinity:  Let's account for Highgarden
			if (range > 1500)
				range = 1500;
