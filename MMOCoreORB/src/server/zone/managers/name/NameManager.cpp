...
int NameManager::validateCityName(const String& name) const {
	...
	//Infinity:  Require first letter to be uppercase
	if (!isupper(name[0]))
		return NameManagerResult::DECLINED_SYNTAX;

	int consecutiveCount = 1;
	for (int i = 1; i < name.length(); ++i) {
		if (toupper(name[i]) == toupper(name[i - 1])) {
			consecutiveCount++;
			if (consecutiveCount >= 3) {
				return NameManagerResult::DECLINED_SYNTAX;; //Infinity:  3+ consecutive letters not allowed
			}
		} else {
			consecutiveCount = 1; // Reset count if consecutive sequence is broken
		}
	}
