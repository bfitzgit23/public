...
bool SkillManager::awardSkill(const String& skillName, CreatureObject* creature, bool notifyClient, bool awardRequiredSkills, bool noXpRequired, bool regrant) {
	...
	if (ghost != nullptr) {
		...
		if (!regrant) {
			...
			if (skill->getSkillName() == "force_title_jedi_rank_02") {
				if (missionManager != nullptr)
					missionManager->addPlayerToBountyList(creature->getObjectID(), ghost->calculateBhRewardWithExisting());
			} else if (skill->getSkillName().contains("force_discipline")) {
				if (missionManager != nullptr)
					missionManager->updatePlayerBountyReward(creature->getObjectID(), ghost->calculateBhRewardWithExisting());
		...
		creature->notifyObservers(ObserverEventType::SKILLAWARDED, nullptr, skillName.hashCode());
	}
...
bool SkillManager::surrenderSkill(const String& skillName, CreatureObject* creature, bool notifyClient, bool checkFrs, bool regrant) {
	...
	if (!regrant) {
		...
		creature->notifyObservers(ObserverEventType::SKILLSURRENDERED, nullptr, skillName.hashCode());
	}
...
void SkillManager::surrenderAllSkills(CreatureObject* creature, bool notifyClient, bool removeForceProgression) {
	...
	for (int i = 0; i < copyOfList.size(); i++) {
		...
		if (!removeForceProgression and skillName.contains("force_"))
			continue;
...
//Infinity:  Will return a hashcode skills with Force Sensitive option
uint32 SkillManager::getSkillsHash(CreatureObject* creature, bool includeForceSensitive) {
	...
	for (int i = 0; i < skills->size(); ++i) {
		...
		if (!includeForceSensitive && skillName.beginsWith("force_sensitive")) { //Default is to include
			continue;
		}
