...
void ZoneServerImplementation::printInfo() {
	...
	float packetloss;
	if (totalResentPackets + totalSentPackets == 0) //Infinity:  No packets sent yet, avoid divide by zero
		packetloss = 0.0f;
	else
		packetloss = (100.f * totalResentPackets) / (totalResentPackets + totalSentPackets);
