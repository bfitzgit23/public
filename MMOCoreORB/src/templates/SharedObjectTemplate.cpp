...
void SharedObjectTemplate::loadDerv(IffStream* stream) {
	...
	if (luaObject != nullptr) {
        ...
        static const Vector<String> templatesToIgnore =
        { 
            "base_vehicle",
            "tangible_base_vehicle",
            "vehicle_base"
        };
        if (!templatesToIgnore.contains(getTemplateFileName())) {
...
void SharedObjectTemplate::readObject(IffStream* iffStream) {
	...
	if (nextType != 'SHOT') {
        ...
        if (nextType != 'STAT' && nextType != 'SSHP' && !templatesToIgnore.contains(getTemplateFileName())) { //Infinity:  Let's skip the warning message on specific examples
