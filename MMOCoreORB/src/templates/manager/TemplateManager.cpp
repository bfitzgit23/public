...
void TemplateManager::loadLuaTemplates() {
	...
	try {
		...
		error() << "error in loadLuaTemplates()";
		error(e.getMessage());
		e.printStackTrace();
...
AppearanceTemplate* TemplateManager::instantiateAppearanceTemplate(IffStream* iffStream) {
	...
	try {
		...
		error() << "error in instantiateAppearanceTemplate(), formType = " << formType << ".  filename = " << iffStream->getFileName();
		error(e.getMessage());
		e.printStackTrace();
	}
	...
	if (interior == nullptr) {
		...
		if (iffStream != nullptr) {
			try {
				...
				error() << "error in getInteriorLayout(), fileName = " << fileName;
				delete interior;
				interior = nullptr;
			}
